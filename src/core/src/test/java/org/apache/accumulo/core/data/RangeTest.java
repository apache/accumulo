...
public class RangeTest extends TestCase {
            ...
            for (boolean b2 : new boolean[] {true, false})
                for (boolean b3 : new boolean[] {true, false})
                    for (boolean b4 : new boolean[] {true, false}) {
                        ...
                        List<Range> rl = nrl(new Range(new Key(new Text("a")), b1, new Key(new Text("m")), b2), new Range(new Key(new Text("b")), b3, new Key(
                                new Text("n")), b4));
                        List<Range> expected = nrl(new Range(new Key(new Text("a")), b1, new Key(new Text("n")), b4));
                        check(Range.mergeOverlapping(rl), expected);
                        
                        rl = nrl(new Range(new Key(new Text("a")), b1, new Key(new Text("m")), b2), new Range(new Key(new Text("a")), b3,
                                new Key(new Text("n")), b4));
                        expected = nrl(new Range(new Key(new Text("a")), b1 || b3, new Key(new Text("n")), b4));
                        check(Range.mergeOverlapping(rl), expected);
                        
                        rl = nrl(new Range(new Key(new Text("a")), b1, new Key(new Text("n")), b2), new Range(new Key(new Text("b")), b3,
                                new Key(new Text("n")), b4));
                        expected = nrl(new Range(new Key(new Text("a")), b1, new Key(new Text("n")), b2 || b4));
                        check(Range.mergeOverlapping(rl), expected);
                        
                        rl = nrl(new Range(new Key(new Text("a")), b1, new Key(new Text("n")), b2), new Range(new Key(new Text("a")), b3,
                                new Key(new Text("n")), b4));
    ...
    public void testBug1() {
        ...
        Range scanRange = new Range(new Key("10;007cdc5b0".getBytes(), "~tab".getBytes(), "~pr".getBytes(), "".getBytes(), 130962, false), false, new Key(
                new Text("10<")).followingKey(PartialKey.ROW), false);
