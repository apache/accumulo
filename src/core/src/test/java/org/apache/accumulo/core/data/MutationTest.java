...
public class MutationTest extends TestCase{
	...
	private Text nt(String s){return new Text(s);}
	private Value nv(String s){return new Value(s.getBytes());}
	...
	/**
	 * Test for regression on bug 3422.  If a {@link Mutation} object is
	 * reused for multiple calls to readFields, the mutation would previously
	 * be "locked in" to the first set of column updates (and value lengths).
	 * Hadoop input formats reuse objects when reading, so if Mutations are
	 * used with an input format (or as the input to a combiner or reducer)
	 * then they will be used in this fashion.
	 */
