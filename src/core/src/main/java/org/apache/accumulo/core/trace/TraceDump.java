...
public class TraceDump {
    ...
    static {
        for (Option opt : new Option[] { LIST_SPANS, START_TIME, END_TIME,
            DUMP_TRACE, INSTANCE_URL }) {
    ...
    private static int listSpans(CommandLine commandLine) throws Exception {
        ...
        for (Entry<Key, Value> entry : scanner) {
            ...
            out.println(String.format("%016x %s %5d %s", span.traceId, TraceFormatter.DATE_FORMAT.format(new Date(span.getStart())), span.stop
                    - span.start, span.description));
        }
        return 0;
    }
    ...
    public static int printTrace(Scanner scanner, final Printer out) {
        ...
        Set<Long> visited = tree.visit(new SpanTreeVisitor() {
            ...
            public void visit(int level, RemoteSpan parent, RemoteSpan node, Collection<RemoteSpan> children) {
                ...
                out.print(String.format(fmt, node.stop - node.start, node.start
                        - finalStart, "", node.svc, node.sender, node.description));
            }
        });
