...
public class FileUtil {
     ...
     * 
     *            ISSUES : This method used the index files to find the mid point. If the map files have different index intervals this method will not return
     *            an accurate mid point. Also, it would be tricky to use this method in conjunction with an in memory map because the indexing interval is
     *            unknown.
     */
    public static SortedMap<Double,Key> findMidPoint(FileSystem fs, AccumuloConfiguration acuConf, Text prevEndRow, Text endRow, Collection<String> mapFiles,
        ...
        try {
            ...
            if (numKeys == 0) {
                if (useIndex) {
                    log.warn("Failed to find mid point using indexes, falling back to data files which is slower. No entries between " + prevEndRow + " and "
                            + endRow + " for " + mapFiles);
    ...
    private static long countIndexEntries(AccumuloConfiguration acuConf, Text prevEndRow, Text endRow, Collection<String> mapFiles, boolean useIndex,
        ...
        for (String path : mapFiles) {
            ...
            try {
                ...
                else reader = FileOperations.getInstance().openReader(path, new Range(prevEndRow, false, null, true), LocalityGroupUtil.EMPTY_CF_SET, false,
                        fs, conf, acuConf);
            ...
            else readers.add(FileOperations.getInstance().openReader(path, new Range(prevEndRow, false, null, true), LocalityGroupUtil.EMPTY_CF_SET, false, fs,
                    conf, acuConf));
