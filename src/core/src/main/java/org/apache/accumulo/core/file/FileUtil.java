...
public class FileUtil {
	...
	private static String createTmpDir(AccumuloConfiguration acuConf, FileSystem fs)
	throws IOException
	{
	...
	public static Collection<String> reduceFiles(AccumuloConfiguration acuConf, Configuration conf, FileSystem fs, Text prevEndRow, Text endRow, Collection<String> mapFiles, int maxFiles, String tmpDir, int pass)
	throws IOException
	{
	...
	public static SortedMap<Double, Key> findMidPoint(FileSystem fs, AccumuloConfiguration acuConf, Text prevEndRow, Text endRow, Collection<String> mapFiles, double minSplit)
	throws IOException
	{
	 ...
	 *   If the map files have different index intervals this method will not return an accurate mid point.
	 *   Also, it would be tricky to use this method in conjunction with an in memory map because the indexing interval is unknown.
	 */
	public static SortedMap<Double, Key> findMidPoint(FileSystem fs, AccumuloConfiguration acuConf, Text prevEndRow, Text endRow, Collection<String> mapFiles, double minSplit, boolean useIndex)
	throws IOException
	{
		Configuration conf = CachedConfiguration.getInstance();
		...
		try {
			...
			{
				if(lastKey != null 
						&& !lastKey.equals(mmfi.getTopKey(), PartialKey.ROW)
	...
	private static void cleanupIndexOp(AccumuloConfiguration acuConf, String tmpDir, FileSystem fs,
		...
		for (String path : mapFiles)
		{
			...
			try {
				if(useIndex)
					...
					reader = FileOperations.getInstance().openReader(path, new Range(prevEndRow, false, null, true), LocalityGroupUtil.EMPTY_CF_SET, false, fs, conf, acuConf);
	...
	public static WritableComparable<Key> findLastKey(FileSystem fs, AccumuloConfiguration acuConf, Collection<String> mapFiles)
	throws IOException
	{
		Key lastKey = null;
	...
	{
		public MLong(long i) { l = i; }
		long l;
	}
	
	public static Map<KeyExtent, Long> estimateSizes(AccumuloConfiguration acuConf, Path mapFile, long fileSize, List<KeyExtent> extents, Configuration conf, FileSystem fs)
	throws IOException
	{
