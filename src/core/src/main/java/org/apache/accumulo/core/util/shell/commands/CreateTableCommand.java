...
public class CreateTableCommand extends Command {
	...
	public int execute(String fullCommand, CommandLine cl, Shell shellState) throws AccumuloException, AccumuloSecurityException, TableExistsException, TableNotFoundException, IOException {
		...
		if (cl.hasOption(createTableOptCopyConfig.getOpt())) {
			if (shellState.getConnector().tableOperations().exists(tableName)) {
			    Iterable<Entry<String, String>> configuration = shellState.getConnector().tableOperations().getProperties(cl.getOptionValue(createTableOptCopyConfig.getOpt()));
		...
		if (cl.hasOption(createTableOptEVC.getOpt())) {
			...
			if(!vcSet)
				shellState.getConnector().tableOperations().setProperty(tableName, Property.TABLE_CONSTRAINT_PREFIX.getKey()+(max+1), VisibilityConstraint.class.getName());
		}
	...
	public Options getOptions() {
		...
		createTableOptEVC = new Option("evc", "enable-visibility-constraint", false, "prevents users from writing data they can not read.  When enabling this may want to consider disabling bulk import and alter table");
