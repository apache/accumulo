...
public final class BCFile {
    ...
    static public class Writer implements Closeable {
        ...
        public class BlockAppender extends DataOutputStream {
             ...
             * 
             * @return the number of compressed bytes written to the underlying FS file. The size may be smaller than actual need to compress the all data
             *         written due to internal buffering inside the compressor.
             * @throws IOException
             */
        ...
        /**
         * Create a Meta Block and obtain an output stream for adding data into the block. There can only be one BlockAppender stream active at any time.
         * Regular Blocks may not be created after the first Meta Blocks. The caller must call BlockAppender.close() to conclude the block creation.
         * 
         * @param name
        ...
        /**
         * Create a Meta Block and obtain an output stream for adding data into the block. The Meta Block will be compressed with the same compression algorithm
         * as data blocks. There can only be one BlockAppender stream active at any time. Regular Blocks may not be created after the first Meta Blocks. The
         * caller must call BlockAppender.close() to conclude the block creation.
         * 
         * @param name
         *            The name of the Meta Block. The name must not conflict with existing Meta Blocks.
         * @return The BlockAppender stream
            ...
            public RBlockState(Algorithm compressionAlgo, FSDataInputStream fsin, BlockRegion region, Configuration conf) throws IOException {
                ...
                try {
                    this.in = compressAlgo.createDecompressionStream(
                            new BoundedRangeFileInputStream(fsin, this.region.getOffset(), this.region.getCompressedSize()), decompressor,
