...
/***
 *
 *This is a wrapper class for BCFile that includes a cache for
 *independent caches for datablocks and metadatablocks
 */
...
private CachableBlockFile(){};
	...
	public static class Reader implements BlockFileReader{
		...
		/**
		 *  It is intended that once the BlockRead object is returned to the caller, that the caller 
		 *  will read the entire block and then call close on the BlockRead class. 
		 *  
		 *  NOTE: In the case of multi-read threads: This method can do 
		 *  redundant work where an entry is read from disk and other threads 
		...
		public BlockRead getMetaBlock(String blockName) throws IOException {
			...
			/**
			 * Try to fully read block for meta data
			 * if error try to close file 
			 * 
			 */
				try {
		...
		public BlockRead getDataBlock(int blockIndex) throws IOException {
			 ...
			 */
				...
				if(( _dCache == null) || (_currBlock.getRawSize() > _dCache.getMaxSize())){
					...
					/**
					 * Try to read block fully
					 * Incase of hdfs error catch
					 *  close stream
					 */
					try {
						b = new byte[(int) _currBlock.getRawSize()];
	 ...
	 * Since We are caching blocks in the Reader class as bytearrays, 
	 * this class will wrap a DataInputStream(ByteArrayStream(cachedBlock)).
	 * 
	 *
	 */
	...
	/**
	 *  It is intended that the caller of this method will close the stream
	 *  we also only intend that this be called once per BlockRead.  
	 *  This method is provide for methods up stream that expect to receive 
