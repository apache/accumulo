...
public class ZooCache
{
    ...
    public ZooCache(String zooKeepers, int sessionTimeout, Watcher watcher)
    {
        ...
        statCache = new HashMap<String, Stat>();
    ...
    public synchronized byte[] get(final String zPath, Stat stat)
    {
    	ZooRunnable zr = new ZooRunnable(){
    		...
    		public void run(ZooKeeper zooKeeper) throws KeeperException, InterruptedException
    		{
    			 ...
    			 * caching that a node does not exist.
    			 * Once the node comes into existance, it will be added to
    			 * the cache. But this notification of a
    			 * node coming into existance will only be given if exists()
    			 * was previously called.
    			 * 
    			 * If the call to exists() is bypassed and only getData() is
    			 * called with a special case that looks for
    			 * Code.NONODE in the KeeperException, then non-existance
    			 * can not be cached.
    			 */
