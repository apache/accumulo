        ...
        try {
            ...
            try {
                ...
                InitialScan isr = client.startScan(null, scanState.credentials, extent.toThrift(), scanState.range.toThrift(),
                        ...
                        scanState.serverSideIteratorOptions, scanState.authorizations.getAuthorizationsBB(), waitForWrites, scanState.isolated);
                ...
                while (loc == null) {
                    ...
                    try {
                        ...
                        if (loc == null) {
                            ...
                            else if (Tables.getTableState(instance, scanState.tableName.toString()) == TableState.OFFLINE) throw new TableOfflineException(
                                    instance, scanState.tableName.toString());
            ...
            if (!sr.more) {
                ...
                if (loc.tablet_extent.getEndRow() == null) {
                    ...
                    opTimer.stop("Completely finished scan in %DURATION% #results=" + sr.results.size());
                        ...
                        || !scanState.range.afterEndKey(new Key(loc.tablet_extent.getEndRow()).followingKey(PartialKey.ROW))) {
                    scanState.startRow = loc.tablet_extent.getEndRow();
                    scanState.skipStartRow = true;
