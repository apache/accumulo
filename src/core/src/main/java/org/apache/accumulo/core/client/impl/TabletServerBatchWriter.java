...
public class TabletServerBatchWriter {
    ...
    private void logStats() {
        ...
        if (log.isTraceEnabled()) {
            ...
            log.trace(String.format("Total bin time       : %,10.2f secs %6.2f%s", totalBinTime.get() / 1000.0, 100.0 * totalBinTime.get()
                    / (finishTime - startTime), "%"));
            log.trace(String.format("Average bin rate     : %,10.2f mutations/sec", totalBinned.get() / (totalBinTime.get() / 1000.0)));
            log.trace(String.format("tservers per batch   : %,8.2f avg  %,6d min %,6d max", (tabletServersBatchSum / (double) numBatches),
                    minTabletServersBatch, maxTabletServersBatch));
            log.trace(String.format("tablets per batch    : %,8.2f avg  %,6d min %,6d max", (tabletBatchSum / (double) numBatches), minTabletBatch,
                    maxTabletBatch));
            log.trace("");
    ...
    private class MutationWriter {
                ...
                for (Entry<String,List<Mutation>> entry : es) {
                    ...
                    if (tableMutations != null) {
                        ...
                        if (tableFailures.size() > 0) {
                            ...
                            if (tableFailures.size() == tableMutations.size()) if (!Tables.exists(instance, entry.getKey())) throw new TableDeletedException(
                                    entry.getKey());
        ...
        private synchronized void addMutations(Map<String,TabletServerMutations> binnedMutations) {
            ...
            if (count > 0 && log.isTraceEnabled()) log.trace(String.format("Started sending %,d mutations to %,d tablet servers", count, binnedMutations
                    .keySet().size()));
        ...
        class SendTask implements Runnable {
            ...
            public void send(TabletServerMutations tsm) throws AccumuloServerException, AccumuloSecurityException {
                ...
                try {
                    ...
                    String msg = "sending " + String.format("%,d", count) + " mutations to " + String.format("%,d", mutationBatch.size()) + " tablets at "
                            + location;
                    ...
                    try {
                        ...
                        if (log.isTraceEnabled()) log.trace("sent "
                                + String.format("%,d", count)
                                + " mutations to "
                                + location
                                + " in "
                                + String.format("%.2f secs (%,.2f mutations/sec) with %,d failures", (st2 - st1) / 1000.0, count / ((st2 - st1) / 1000.0),
                                        failures.size()));
