...
public class TabletServerBatchWriter {
	...
	private void logStats() {
		...
		if (log.isTraceEnabled())
		{
			...
			log.trace(String.format("Total send time      : %,10.2f secs %6.2f%s", totalSendTime.get()/1000.0, 100.0 * totalSendTime.get()/(finishTime - startTime), "%"));
			log.trace(String.format("Average send rate    : %,10.2f mutations/sec", averageRate));
			log.trace(String.format("Total bin time       : %,10.2f secs %6.2f%s", totalBinTime.get()/1000.0, 100.0 * totalBinTime.get()/(finishTime - startTime), "%"));
			log.trace(String.format("Average bin rate     : %,10.2f mutations/sec", totalBinned.get() / (totalBinTime.get() / 1000.0)));
			log.trace(String.format("tservers per batch   : %,8.2f avg  %,6d min %,6d max", (tabletServersBatchSum/(double)numBatches), minTabletServersBatch, maxTabletServersBatch));
	 ...
	 * 
	 * @param mutationsprivate static final Logger log = Logger.getLogger(TabletServerBatchWriter.class);
	 */
	...
	private class MutationWriter {
			...
			public void send(TabletServerMutations tsm) throws AccumuloServerException, AccumuloSecurityException {
				...
				try{
					...
					try {
						...
						if (log.isTraceEnabled()) log.trace("sent "+String.format("%,d", count)+" mutations to "+location+" in "+String.format("%.2f secs (%,.2f mutations/sec) with %,d failures", (st2 - st1)/1000.0, count/((st2 - st1)/1000.0), failures.size()));
		...
		private MutationSet sendMutationsToTabletServer(String location, Map<KeyExtent, List<Mutation>> tabMuts) throws IOException, AccumuloSecurityException, AccumuloServerException {
			if(tabMuts.size() == 0){
			...
			try {
				...
				try {
				    ...
				    for(Entry<KeyExtent, List<Mutation>> entry :  tabMuts.entrySet()){
				        client.setUpdateTablet(null, usid, entry.getKey().toThrift());
				        for(Mutation mutation : entry.getValue()){
				            client.applyUpdate(null, usid, mutation.toThrift());
				        }
				    }
