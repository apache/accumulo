 ...
 * input format provides keys and values of type K and V to the Map() and
 * Reduce() functions.
 * 
 ...
 * 
 * This class includes a static class that can be used to create a RecordReader:
 * protected abstract static class RecordReaderBase<K,V> extends RecordReader<K,V>
 * 
 * Subclasses of RecordReaderBase must implement the following method:
 * public boolean nextKeyValue() throws IOException, InterruptedException
 * This method should set the following variables:
 * K currentK
 * V currentV
 * Key currentKey (used for progress reporting)
 * int numKeysRead (used for progress reporting)
 * 
...
public abstract class InputFormatBase<K,V> extends InputFormat<K,V> {
	...
	/**
	 * Sets the max # of values that may be returned for an individual Accumulo cell. By default, applied before all other 
	 * Accumulo iterators (highest priority) leveraged in the scan by the record reader.  To adjust priority use 
	 ...
	 * @param job The job in which to save the iterator configuration
	 * @param priority The priority of the iterator
	...
	/**
	 * Specify a Accumulo iterator type to manage the behavior of the underlying table scan this InputFormat's Record Reader will conduct, w/ priority dictating the order
	...
	/**
	 * Specify an option for a named Accumulo iterator, further specifying that iterator's 
	 * behavior. 
	 * 
	 * @param job the job
	...
	protected static TabletLocator getTabletLocator(JobContext job) throws TableNotFoundException {
		...
		return TabletLocator.getInstance(instance, new AuthInfo(username, ByteBuffer.wrap(password), instance.getInstanceID()), new Text(Tables.getTableId(instance, tableName)));
	}
	...
	protected abstract static class RecordReaderBase<K,V> extends RecordReader<K,V> {
        ...
        private void checkAndEnableRegex(String regex, Scanner scanner, String methodName) throws IllegalArgumentException, SecurityException, IllegalAccessException, InvocationTargetException, NoSuchMethodException, IOException {
			if (regex != null) {
	...
	/**
	 * The Class AccumuloIteratorOption. Encapsulates specifics for a Accumulo iterator's optional configuration
