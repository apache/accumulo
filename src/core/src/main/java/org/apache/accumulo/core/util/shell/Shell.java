...
public class Shell {
	...
	public void config(String... args) {
		...
		Option authTimeoutOpt = new Option(null, "auth-timeout", true, "minutes the shell can be idle without re-entering a password (default " + DEFAULT_AUTH_TIMEOUT + " min)");
	 ...
	 * easily linked up. There is some code in token to allow forward building
	 * of the command tree.
	 */
	...
	/**
	 * Command Factory is a class that holds the command table, which is a
	 * mapping from a string representing the command to a class. It has methods
	 * to build the command table from the classes in this file and a currently
	 * unused method to build the command table from XML. The classes that the
	 * table returns all extend Shell$Command class.
	 */
	 ...
	 * contains the methods to execute along with some methods to help tab
	 * completion, and return the command name, help, and usage.
	 */
	...
	public static class MasterStateCommand extends Command {
        ...
        public int execute(String fullCommand, CommandLine cl, Shell shellState) throws Exception {
            ...
            ZooUtil.putPersistentData(ZooUtil.getRoot(shellState.connector.getInstance()) + Constants.ZMASTER_GOAL_STATE, 
                                      state.getBytes(), 
	...
	public static class CreateUserCommand extends Command {
		...
		public int execute(String fullCommand, CommandLine cl, Shell shellState) throws AccumuloException, TableNotFoundException, AccumuloSecurityException, TableExistsException, IOException {
			...
			log.debug("Created user " + user + " with" + (authorizations.isEmpty() ? " no" : "") + " initial scan authorizations" + (!authorizations.isEmpty() ? " " + authorizations : ""));
			return 0;
		}
	...
	public static class GrepCommand extends ScanCommand {
		...
		public int execute(String fullCommand, CommandLine cl, Shell shellState) throws AccumuloException, AccumuloSecurityException, TableNotFoundException, IOException, MissingArgumentException {
			shellState.checkTableState();
	...
	public static class AddSplitsCommand extends Command {
		...
		public int execute(String fullCommand, CommandLine cl, Shell shellState) throws AccumuloException, AccumuloSecurityException, TableNotFoundException, MissingArgumentException, FileNotFoundException {
	...
	public static class SelectCommand extends Command {
		...
		public int execute(String fullCommand, CommandLine cl, Shell shellState) throws AccumuloException, AccumuloSecurityException, TableNotFoundException, IOException {
			...
			Authorizations authorizations = cl.hasOption(selectOptAuths.getOpt()) ? parseAuthorizations(cl.getOptionValue(selectOptAuths.getOpt())) : Constants.NO_AUTHS;
	...
	public static class InsertCommand extends Command {
		...
		public int execute(String fullCommand, CommandLine cl, Shell shellState) throws AccumuloException, AccumuloSecurityException, TableNotFoundException, IOException, ConstraintViolationException {
			...
			if (cl.hasOption(insertOptAuths.getOpt())) {
				...
				if(cl.hasOption(timestampOpt.getOpt()))
					m.put(new Text(cl.getArgs()[1]), new Text(cl.getArgs()[2]), le, Long.parseLong(cl.getOptionValue(timestampOpt.getOpt())), new Value(cl.getArgs()[3].getBytes()));
				else
					m.put(new Text(cl.getArgs()[1]), new Text(cl.getArgs()[2]), le, new Value(cl.getArgs()[3].getBytes()));
			} else
				if(cl.hasOption(timestampOpt.getOpt()))
					m.put(new Text(cl.getArgs()[1]), new Text(cl.getArgs()[2]), Long.parseLong(cl.getOptionValue(timestampOpt.getOpt())), new Value(cl.getArgs()[3].getBytes()));
				else
	...
	public static class CreateTableCommand extends Command {
		...
		public int execute(String fullCommand, CommandLine cl, Shell shellState) throws AccumuloException, AccumuloSecurityException, TableExistsException, TableNotFoundException, IOException {
			...
			if (cl.hasOption(createTableOptCopyConfig.getOpt())) {
				if (shellState.connector.tableOperations().exists(tableName)) {
					for (Entry<String, String> entry : AccumuloConfiguration.getTableConfiguration(shellState.connector.getInstance().getInstanceID(), shellState.connector.tableOperations().tableIdMap().get(cl.getOptionValue(createTableOptCopyConfig.getOpt()))))
	...
	public static class ImportDirectoryCommand extends Command {
		...
		public int execute(String fullCommand, CommandLine cl, Shell shellState) throws IOException, AccumuloException, AccumuloSecurityException, TableNotFoundException {
			...
			AssignmentStats stats = shellState.connector.tableOperations().importDirectory(shellState.tableName, dir, failureDir, numFileThreads, numAssignThreads, disableGC);
			if (cl.hasOption(verboseOption.getOpt()))
	...
	public static class GetSplitsCommand extends Command {
		...
		@Override
		public int execute(String fullCommand, CommandLine cl, final Shell shellState) throws IOException, AccumuloException, AccumuloSecurityException, TableNotFoundException {
			shellState.checkTableState();
			...
			try {
			    if (!verbose) {
			        for (Text row : maxSplits > 0 ? shellState.connector.tableOperations().getSplits(shellState.tableName, maxSplits) : shellState.connector.tableOperations().getSplits(shellState.tableName)) {
			        ...
			        for (Iterator<Entry<Key, Value>> iterator = scanner.iterator(); iterator.hasNext(); ) {
			            ...
			            if (Constants.METADATA_PREV_ROW_COLUMN.hasColumns(next.getKey())) {
			                ...
			                final String line = String.format("%-26s (%s, %s%s",
			                        obscuredTabletName(extent),
			                        pr == null ? "-inf" : pr,
			                                er == null ? "+inf" : er,
	...
	private static class ActiveScanIterator implements Iterator<String>{
			...
			while(tsIter.hasNext()){
				...
				try {
					...
					for (ActiveScan as : asl) 
						scans.add(String.format("%21s |%21s |%9s |%9s |%7s |%6s |%8s |%8s |%10s |%10s |%10s | %s"
								, tserver, as.getClient(), Duration.format(as.getAge(), ""), Duration.format(as.getLastContactTime(), ""), as.getState(), as.getType(), as.getUser(), as.getTable(), as.getColumns(), (as.getType() == ScanType.SINGLE ? as.getExtent() : "N/A"), as.getSsiList(), as.getSsio()));
		...
		ActiveScanIterator(List<String> tservers, InstanceOperations instanceOps){
			...
			String header = String.format(" %-21s| %-21s| %-9s| %-9s| %-7s| %-6s| %-8s| %-8s| %-10s| %-10s| %-10s | %s",
