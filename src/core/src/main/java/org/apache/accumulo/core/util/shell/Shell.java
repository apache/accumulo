...
public class Shell {
    ...
    /**
     * Command Factory is a class that holds the command table, which is a mapping from a string representing the command to a class. It has methods to build
     * the command table from the classes in this file and a currently unused method to build the command table from XML. The classes that the table returns all
    ...
    public static class PasswdCommand extends Command {
        ...
        public int execute(String fullCommand, CommandLine cl, Shell shellState) throws AccumuloException, AccumuloSecurityException, IOException {
            ...
            // the current user
            if (shellState.connector.whoami().equals(user)) shellState.credentials = new AuthInfo(user, pass, shellState.connector.getInstance()
                    .getInstanceID());
        ...
        public int execute(String fullCommand, CommandLine cl, Shell shellState) throws AccumuloException, AccumuloSecurityException, TableNotFoundException,
            ...
            if (cl.hasOption(deleteOpt.getOpt())) {
                ...
                if (tableName != null) {
                    ...
                    if (!Property.isValidZooPropertyKey(property)) log.warn("Invalid per-table property : " + property
                            + ", still removing from zookeeper if its there.");
                ...
                if (!property.contains("=")) throw new BadArgumentException("Missing '=' operator in set operation.", fullCommand,
    ...
    public static class DeleteManyCommand extends ScanCommand {
        ...
        public int execute(String fullCommand, CommandLine cl, Shell shellState) throws AccumuloException, AccumuloSecurityException, TableNotFoundException,
            ...
            shellState.printLines(new DeleterFormatter(writer, scanner, cl.hasOption(timestampOpt.getOpt()), shellState, cl.hasOption(forceOpt.getOpt())),
                    false);
            return 0;
        }
    ...
    public static class SelectrowCommand extends Command {
        ...
        public int execute(String fullCommand, CommandLine cl, Shell shellState) throws AccumuloException, AccumuloSecurityException, TableNotFoundException,
            ...
            Authorizations auths = cl.hasOption(selectrowOptAuths.getOpt()) ? parseAuthorizations(cl.getOptionValue(selectrowOptAuths.getOpt()))
                    : Constants.NO_AUTHS;
    ...
    public static class ImportDirectoryCommand extends Command {
        ...
        public int execute(String fullCommand, CommandLine cl, Shell shellState) throws IOException, AccumuloException, AccumuloSecurityException,
            ...
            AssignmentStats stats = shellState.connector.tableOperations().importDirectory(shellState.tableName, dir, failureDir, numFileThreads,
                    numAssignThreads, disableGC);
                ...
                if (!verbose) {
                    ...
                    for (Iterator<Entry<Key,Value>> iterator = scanner.iterator(); iterator.hasNext();) {
                        ...
                        if (Constants.METADATA_PREV_ROW_COLUMN.hasColumns(next.getKey())) {
                            ...
                            final String line = String.format("%-26s (%s, %s%s", obscuredTabletName(extent), pr == null ? "-inf" : pr,
    ...
    private static class ActiveScanIterator implements Iterator<String> {
        ...
        ActiveScanIterator(List<String> tservers, InstanceOperations instanceOps) {
            ...
            String header = String.format(" %-21s| %-21s| %-9s| %-9s| %-7s| %-6s| %-8s| %-8s| %-10s| %-10s| %-10s | %s", "TABLET SERVER", "CLIENT", "AGE",
                    "LAST", "STATE", "TYPE", "USER", "TABLE", "COLUMNS", "TABLET", "ITERATORS", "ITERATOR OPTIONS");
