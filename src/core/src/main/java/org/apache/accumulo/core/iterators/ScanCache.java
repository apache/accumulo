...
public class ScanCache implements SortedKeyValueIterator<Key,Value> {
     ...
     * entry just before the associated key that is not in the cache exitPoint is true only if there is the possibility of an entry just after the associated
     * key that is not in the cache
     */
