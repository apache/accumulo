    ...
    public static synchronized ClientService.Iface getConnection(Instance instance) throws TTransportException
    {
        ...
        for (String tserver : getZooCache(instance).getChildren(ZooUtil.getRoot(instance) + Constants.ZTSERVERS))
        {
            ...
            if (data != null && !new String(data).equals("master"))
                servers.add(new ThriftTransportKey(new ServerServices(new String(data)).getAddressString(Service.TSERV_CLIENT), instance.getConfiguration().getPort(Property.TSERV_CLIENTPORT), instance.getConfiguration().getTimeInMillis(Property.GENERAL_RPC_TIMEOUT)));
        }

        // add masters
        for (String master : instance.getMasterLocations())
            servers.add(new ThriftTransportKey(master, instance.getConfiguration().getPort(Property.MASTER_CLIENTPORT), instance.getConfiguration().getTimeInMillis(Property.GENERAL_RPC_TIMEOUT)));
