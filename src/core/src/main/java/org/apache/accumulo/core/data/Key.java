...
public class Key implements WritableComparable<Key>, Cloneable {
	...
	public Key(Text row) {
		init(row.getBytes(), 0, row.getLength(),
				EMPTY_BYTES,0,0,
				EMPTY_BYTES,0,0,
				EMPTY_BYTES,0,0,
				Long.MAX_VALUE,
	...
	public Key(byte row[], int rOff, int rLen,
			byte cf[], int cfOff, int cfLen,
			byte cq[], int cqOff, int cqLen,
			byte cv[], int cvOff, int cvLen,
			long ts)
	...
	public Key(TKey tkey) {
		this.row = tkey.row;
		this.colFamily = tkey.colFamily;
		this.colQualifier = tkey.colQualifier;
		this.colVisibility = tkey.colVisibility;
	...
	public String toString() {
		...
		String s = toPrintableString(row, 0, row.length, Constants.MAX_DATA_TO_PRINT) + " " +
					...
					toPrintableString(colQualifier, 0, colQualifier.length, Constants.MAX_DATA_TO_PRINT) + " " +
					labelString + " " +
	...
	public String toStringNoTime() {
		...
		String s = new String(row, 0, row.length) + " " +
					new String(colFamily, 0, colFamily.length) + ":" +
					new String(colQualifier, 0, colQualifier.length) + " " +
					labelString;
		return s;
	}
	...
	/**
	 * Use this to compress a list of
	 * keys before sending them via thrift.
	 * 
	 * @param param
	 */
	...
	/**
	 * Use this to decompress a list of keys 
	 * received from thrift.
	 * 
	 * @param param
	 */
