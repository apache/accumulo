...
public class GetSplitsCommand extends Command {
	...
	public int execute(String fullCommand, CommandLine cl, final Shell shellState) throws IOException, AccumuloException, AccumuloSecurityException, TableNotFoundException {
		...
		try {
			if (!verbose) {
				for (Text row : maxSplits > 0 ? shellState.getConnector().tableOperations().getSplits(tableName, maxSplits) : shellState.getConnector().tableOperations().getSplits(tableName)) {
