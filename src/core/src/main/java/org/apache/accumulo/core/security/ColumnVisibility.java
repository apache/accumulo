 ...
 * See {@link ColumnVisibility#ColumnVisibility(byte[])} for the definition of an expression.
 */
public class ColumnVisibility {
    ...
    public static class Node {
        ...
        public void add(Node child) {
        	// Profiling found the below to be a hot spot due to the use
        	// of reflection.
    ...
    public static class NodeComparator implements Comparator<Node> {
        ...
        byte[] text;
        NodeComparator(byte[] text) { this.text = text; }
        ...
        public int compare(Node a, Node b) {
            ...
            {
            case TERM:
                return WritableComparator.compareBytes(text, a.start, a.end - a.start,
     ...
     * @param expression An expression of the rights needed to see this mutation.  The expression is a sequence of characters from the set [A-Za-z0-9_-] along with the binary operators "&" and "|" indicating that both operands are necessary, or the either is necessary.
     * The following are valid expressions for visibility:
     * <pre>
     * A
     * A|B
     * (A|B)&(C|D)
