...
public class TabletServerBatchReaderIterator implements Iterator<Entry<Key,Value>> {
        ...
        for (Entry<KeyExtent,List<Range>> entry : failures.entrySet()) {
            ...
            while (iter.hasNext()) {
                ...
                while (iter2.hasNext()) {
                    ...
                    boolean gtePer = ke.getPrevEndRow() == null ? failedExtent.getPrevEndRow() == null : failedExtent.contains(ke.getPrevEndRow())
                            || (failedExtent.getPrevEndRow() != null && failedExtent.getPrevEndRow().equals(ke.getPrevEndRow()));
                    
                    boolean containsEr = ke.getEndRow() == null ? failedExtent.getEndRow() == null : failedExtent.contains(ke.getEndRow());
    ...
    private class QueryTask implements Runnable {
        ...
        QueryTask(String tsLocation, Map<KeyExtent,List<Range>> tabletsRanges, Map<KeyExtent,List<Range>> failures, ResultReceiver receiver,
                List<Column> columns) {
    ...
    static void doLookup(String server, Map<KeyExtent,List<Range>> requested, Map<KeyExtent,List<Range>> failures, Map<KeyExtent,List<Range>> unscanned,
            ResultReceiver receiver, List<Column> columns, AuthInfo credentials, ScannerOptions options, Authorizations authorizations,
            AccumuloConfiguration conf) throws IOException, AccumuloSecurityException, AccumuloServerException {
        ...
        try {
            ...
            try {
                OpTimer opTimer = new OpTimer(log, Level.TRACE).start("Starting multi scan, tserver=" + server + "  #tablets=" + requested.size()
                        + "  #ranges=" + sumSizes(requested.values()) + " ssil=" + options.serverSideIteratorList + " ssio="
