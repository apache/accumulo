...
public class TabletServerBatchReaderIterator implements Iterator<Entry<Key, Value>>{
    ...
    private static class MyEntry implements Entry<Key, Value>{
        ...
        public TabletServerBatchReaderIterator(Instance instance, AuthInfo credentials,
        		String table, Authorizations authorizations, ArrayList<Range> ranges, int numThreads,
    ...
    throws AccumuloException, AccumuloSecurityException, TableNotFoundException
    {
    	...
    	{
    		Map<String, Map<KeyExtent, List<Range>>> tabletBinnedRanges = new HashMap<String, Map<KeyExtent,List<Range>>>();
			binRanges(tabletLocator, entry.getValue(), tabletBinnedRanges);
			...
			Iterator<Map<KeyExtent, List<Range>>> iter = tabletBinnedRanges.values().iterator();
			
			while(iter.hasNext()){
				Map<KeyExtent, List<Range>> map = iter.next();
				
				Iterator<KeyExtent> iter2 = map.keySet().iterator();
				while(iter2.hasNext()){
					//check to see if extents is contained within failed extent
					KeyExtent failedExtent = entry.getKey();
					KeyExtent ke = iter2.next();
					
					boolean gtePer = ke.getPrevEndRow() == null ?
							failedExtent.getPrevEndRow() == null :
							failedExtent.contains(ke.getPrevEndRow()) || 
								(failedExtent.getPrevEndRow() != null && failedExtent.getPrevEndRow().equals(ke.getPrevEndRow()));
						
					boolean containsEr = ke.getEndRow() == null ?
							failedExtent.getEndRow() == null :
							failedExtent.contains(ke.getEndRow()) ;
					
					boolean contains =  gtePer && containsEr;
					
					if(!contains){
						iter2.remove();  //extent is not within failed extent so remove it
					}
			...
			//merge
			for(Entry<String, Map<KeyExtent, List<Range>>> entry2 : tabletBinnedRanges.entrySet()){
				Map<KeyExtent, List<Range>> tabletMap = binnedRanges.get(entry2.getKey());
				if(tabletMap == null){
					tabletMap = new HashMap<KeyExtent, List<Range>>();
					binnedRanges.put(entry2.getKey(), tabletMap);
				}
				
				if(!Collections.disjoint(tabletMap.keySet(), entry2.getValue().keySet())){
					throw new RuntimeException("Unexpected overlap in tablet sets");
				}
				
				tabletMap.putAll(entry2.getValue());
			}
    ...
    static void trackScanning(Map<KeyExtent, List<Range>> failures, Map<KeyExtent, List<Range>> unscanned, MultiScanResult scanResult) {
    	...
    	Map<KeyExtent, List<Range>> retFailures = Translator.translate(scanResult.failures, Translator.TKET, new Translator.ListTranslator<TRange, Range>(Translator.TRT));
    ...
    static void doLookup(String server, Map<KeyExtent, List<Range>> requested, Map<KeyExtent, List<Range>> failures, Map<KeyExtent, List<Range>> unscanned, ResultReceiver receiver, List<Column> columns, AuthInfo credentials, ScannerOptions options, Authorizations authorizations, AccumuloConfiguration conf)
    throws IOException, AccumuloSecurityException, AccumuloServerException {
        ...
        try {
            ...
            try {
                OpTimer opTimer = new OpTimer(log, Level.TRACE).start("Starting multi scan, tserver="+server+"  #tablets="+requested.size()+"  #ranges="+sumSizes(requested.values())+" ssil="+options.serverSideIteratorList+" ssio="+options.serverSideIteratorOptions);
                ...
                Map<TKeyExtent, List<TRange>> thriftTabletRanges = Translator.translate(requested, Translator.KET, new Translator.ListTranslator<Range, TRange>(Translator.RT)); 
                InitialMultiScan imsr = client.startMultiScan(null, credentials, thriftTabletRanges, Translator.translate(columns, Translator.CT), options.serverSideIteratorList, options.serverSideIteratorOptions, authorizations.getAuthorizations(), waitForWrites);
                if(waitForWrites)
                	ThriftScanner.serversWaitedForWrites.get(ttype).add(server);

                MultiScanResult scanResult = imsr.result;

                opTimer.stop("Got 1st multi scan results, #results="+scanResult.results.size()+( scanResult.more ? "  scanID="+imsr.scanID : "" )+" in %DURATION%");
                ...
                while(scanResult.more) {
                    ...
                    opTimer.stop("Got more multi scan results, #results="+scanResult.results.size()+( scanResult.more ? "  scanID="+imsr.scanID : "" )+" in %DURATION%");
                    for (TKeyValue kv : scanResult.results) {
                        receiver.receive(new Key(kv.key), new Value(kv.value));
                    }
                    trackScanning(failures, unscanned, scanResult);
