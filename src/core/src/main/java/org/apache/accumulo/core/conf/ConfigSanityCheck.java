...
class ConfigSanityCheck {
	...
	private static class CheckTimeDurationBetween implements CheckTimeDuration {
		long min, max;
		CheckTimeDurationBetween(long x, long y) { min = Math.min(x, y); max = Math.max(x, y); }
		@Override public boolean check(long propVal) { return propVal >= min && propVal <= max; }
		@Override public String getDescription(Property prop) { return "ensure " + min + " <= " + prop + " <= " + max; }
	}
	...
	private static class FirstGreaterThanByFactor implements CompareTwoCounts {
		int factor;
		FirstGreaterThanByFactor(int f) { factor = f; }
		...
		@Override public boolean check(int prop1val, int prop2val) { return prop1val / prop2val > factor || (prop1val / prop2val == factor && prop1val % prop2val > 0); }
		@Override public String getDescription(Property prop1, Property prop2) { return "ensure " + prop1 + " > " + prop2 + (factor != 1 ? "*" + factor : ""); }
	}
	
	private static class FirstGreaterThanEqualToByFactor implements CompareTwoCounts {
		int factor;
		FirstGreaterThanEqualToByFactor(int f) { factor = f; }
		// integer arithmetic is sufficient here to achieve greater than or equal to (avoid fp-math)
		@Override public boolean check(int prop1val, int prop2val) { return prop1val / prop2val >= factor; }
		@Override public String getDescription(Property prop1, Property prop2) { return "ensure " + prop1 + " >= " + prop2 + (factor != 1 ? "*" + factor : ""); }
	}
