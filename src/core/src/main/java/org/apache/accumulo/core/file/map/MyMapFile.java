...
public class MyMapFile {
    ...
    public static class Writer {
        ...
        public Writer(Configuration conf, FileSystem fs, String dirName, WritableComparator comparator, Class valClass,
                MySequenceFile.CompressionType compress, Progressable progress) throws IOException {
            this(conf, fs, dirName, comparator, valClass, compress, new DefaultCodec(), progress);
        }
        
        /** Create the named map using the named key comparator. */
        public Writer(Configuration conf, FileSystem fs, String dirName, WritableComparator comparator, Class valClass,
                MySequenceFile.CompressionType compress, CompressionCodec codec, Progressable progress) throws IOException {
        ...
        public synchronized void append(WritableComparable key, Writable val) throws IOException {
            ...
            /*******************************************************************
             * Instead of storing index values for every 128th key that all point back to the same compressed block, we can store one key for each compressed
             * block.
             */
    ...
    public static class Reader implements FileSKVIterator {
        ...
        /**
         * Number of index entries to skip between each entry. Zero by default. Setting this to values larger than zero can facilitate opening large map files
         * using less memory.
         */
        ...
        private static class MapFileIndexKey {
            ...
            public boolean equals(Object other) {
                return other instanceof MapFileIndexKey && ((MapFileIndexKey) other).filename.equals(filename)
                        && ((MapFileIndexKey) other).hashInfo.equals(hashInfo);
            }
        }
        ...
        /**
         * Positions the reader at the named key, or if none such exists, at the first entry after the named key. Returns true iff the named key exists in this
         * map.
         */
        ...
        /**
         * Positions the reader at the named key, or if none such exists, at the key that falls just before or just after dependent on how the
         * <code>before</code> parameter is set.
         * 
         * @param before
         *            - IF true, and <code>key</code> does not exist, position file at entry that falls just before <code>key</code>. Otherwise, position file
         *            at record that sorts just after.
         * @return 0 - exact match found < 0 - positioned at next record 1 - no more records in file
         */
        private synchronized int seekInternal(WritableComparable key, final boolean before, WritableComparable lastKey) throws IOException {
            ...
            while (data.next(nextKey)) {
                ...
                if (c <= 0) { // at or beyond desired
                    ...
                    if (timeSpent > 200) log.debug("long seek time: " + timeSpent + " seek location: " + seekLocation + " seek scans: " + scans
                            + " seeking key " + key + " from " + lastKey + " previous " + previousKey);
                    MyMapFile.mapFileSeekTimeStat.addStat(timeSpent);
                    return c;
                }
            }
            long timeSpent = System.currentTimeMillis() - startTime;
            if (timeSpent > 100) log.debug("long seek time (2): " + timeSpent + " seek location: " + seekLocation + " seek scans: " + scans + " seeking key "
                    + key + " from " + lastKey + " previous " + previousKey);
            MyMapFile.mapFileSeekTimeStat.addStat(timeSpent);
            MyMapFile.mapFileSeekScans.addStat(scans);
            return 1;
        }
        ...
        /**
         * Read the next key/value pair in the map into <code>key</code> and <code>val</code>. Returns true if such a pair exists and false when at the end of
         * the map
         */
         ...
         *            - data value if key is found
         * @param before
         *            - IF true, and <code>key</code> does not exist, return the first entry that falls just before the <code>key</code>. Otherwise, return the
