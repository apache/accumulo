...
public class MyMapFile {
	...
	public static class Reader implements FileSKVIterator {
		...
		private static class MapFileIndexKey
		{
			...
			{
				return other instanceof MapFileIndexKey 
				&& ((MapFileIndexKey)other).filename.equals(filename) 
		...
		throws IOException {
			...
			if (seekIndex != -1 // seeked before
					&& ((seekIndex + 1 < indexInfo.count && comparator.compare(key,
							indexInfo.keys[seekIndex + 1]) < 0) || seekIndex + 1 == indexInfo.count) // before
							// next
							// indexed
		...
		public void seek(Range range, Collection<ByteSequence> columnFamilies, boolean inclusive) throws IOException {
			...
			// check boundaries
			if(topKey != null && indexInfo.firstKey != null && previousKey == null && comparator.compare(key, indexInfo.firstKey) <= 0 && comparator.compare(topKey, indexInfo.firstKey) == 0)
			{
