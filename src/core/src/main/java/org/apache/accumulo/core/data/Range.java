...
public class Range implements WritableComparable<Range> {
	 ...
	 * 
	 * @param row set this to null to cover all rows
	 */
	 ...
	 * 
	 * @param startRow set this to null when negative infinity is needed
	 * @param startRowInclusive determines if the start row is skipped
	 * @param endRow  set this to null when positive infinity is needed
	 * @param endRowInclusive determines if the endRow is included
	 */
	
	public Range(Text startRow, boolean startRowInclusive, Text endRow, boolean endRowInclusive) {
		this(
				(startRow == null ? null : (startRowInclusive ? new Key(startRow) : new Key(startRow).followingKey(PartialKey.ROW))),
				true,
				(endRow == null ? null : (endRowInclusive ? new Key(endRow).followingKey(PartialKey.ROW) : new Key(endRow))),
				false
			);		
	}
	...
	public Range(CharSequence startRow, boolean startRowInclusive, CharSequence endRow, boolean endRowInclusive) {
		this(startRow == null ? null : new Text(startRow.toString()), 
				startRowInclusive,
				endRow == null ? null : new Text(endRow.toString()), 
				endRowInclusive);
	}
	 ...
	 * @param endKey            set this to null when infinity is needed
	...
	public Range clip(Range range, boolean returnNullIfDisjoint) {
		...
		}else if(afterEndKey(range.getStartKey()) || (getEndKey() != null && range.getStartKey().equals(getEndKey()) && !(range.isStartKeyInclusive() && isEndKeyInclusive()))){
		...
		}else if(beforeStartKey(range.getEndKey()) || (getStartKey() != null  && range.getEndKey().equals(getStartKey()) && !(range.isEndKeyInclusive() && isStartKeyInclusive()))){
			if(returnNullIfDisjoint)
				return null;
			throw new IllegalArgumentException("Range "+range+" does not overlap "+this);
	 ...
	 * Returns the same Range as new Range(row)
	 * 
	...
	public static Text followingPrefix(Text prefix) {
		...
		while (changeIndex >= 0 && prefixBytes[changeIndex]==(byte)0xff) changeIndex--;
