...
public class Range implements WritableComparable<Range> {
    ...
    public TRange toThrift() {
        return new TRange(start == null ? null : start.toThrift(), stop == null ? null : stop.toThrift(), startKeyInclusive, stopKeyInclusive,
    ...
    public static Range prefix(Text row, Text cf, Text cq, Text cvPrefix) {
        ...
        return new Range(new Key(row, cf, cq, cvPrefix), true, fp == null ? new Key(row, cf, cq).followingKey(PartialKey.ROW_COLFAM_COLQUAL) : new Key(row, cf,
                cq, fp), false);
    }
