...
public class AcquireNewTable {
	...
	public static void main(String[] args) throws IOException {
		...
		MyMapFile.Writer mfw = new MyMapFile.Writer(conf, fs, Constants.getMetadataTableDir() + Constants.DEFAULT_TABLET_LOCATION + "/"+MyMapFile.EXTENSION+"_00000_00000", Key.class, Value.class);
		...
		for(Entry<KeyExtent, Value> entry : data.entrySet()) {
			...
			Key key = new Key(new Text(KeyExtent.getMetadataEntry(entry.getKey().getTableId(), endRow)), Constants.METADATA_PREV_ROW_COLUMN.getColumnFamily(), Constants.METADATA_PREV_ROW_COLUMN.getColumnQualifier());
			...
			key = new Key(new Text(KeyExtent.getMetadataEntry(entry.getKey().getTableId(), endRow)), Constants.METADATA_DIRECTORY_COLUMN.getColumnFamily(), Constants.METADATA_DIRECTORY_COLUMN.getColumnQualifier());
