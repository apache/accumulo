...
public class TabletLocatorImpl extends TabletLocator {
    ...
    public List<Range> binRanges(List<Range> ranges, Map<String,Map<KeyExtent,List<Range>>> binnedRanges) throws AccumuloException, AccumuloSecurityException,
        ...
        /*
         * For this to be efficient, need to avoid fine grained synchronization and fine grained logging. Therefore methods called by this are not synchronized
         * and should not log.
         */
        ...
        if (opTimer != null) opTimer
                .stop("Binned " + ranges.size() + " ranges for table " + tableId + " to " + binnedRanges.size() + " tservers in %DURATION%");
    ...
    public TabletLocation locateTablet(Text row, boolean skipRow, boolean retry) throws AccumuloException, AccumuloSecurityException, TableNotFoundException {
        ...
        while (true) {
            ...
            if (retry && tl == null) {
                ...
                if (log.isTraceEnabled()) log.trace("Failed to locate tablet containing row " + TextUtil.truncate(row) + " in table " + tableId
                        + ", will retry...");
                continue;
            }
