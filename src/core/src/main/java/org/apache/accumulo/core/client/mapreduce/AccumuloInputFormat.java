...
public class AccumuloInputFormat extends InputFormat<Key, Value> {
	...
	public RecordReader<Key, Value> createRecordReader(InputSplit inSplit, TaskAttemptContext attempt) throws IOException, InterruptedException {
		...
		return new RecordReader<Key, Value>() {
			...
			private boolean setupRegex(TaskAttemptContext attempt, Scanner scanner) throws AccumuloException {
				try {
			...
			private void setupIterators(TaskAttemptContext attempt, Scanner scanner) throws AccumuloException {
			...
			private void setupMaxVersions(TaskAttemptContext attempt, Scanner scanner) throws AccumuloException {
				int maxVersions = getMaxVersions(attempt);
				//Check to make sure its a legit value
				if (maxVersions >= 1) {
					try {
						scanner.setScanIterators(0, org.apache.accumulo.core.iterators.VersioningIterator.class.getName(), "vers");
					} 
					catch (Exception e){
						throw new AccumuloException(e);
					}
