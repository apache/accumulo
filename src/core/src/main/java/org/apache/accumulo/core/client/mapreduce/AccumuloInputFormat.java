...
public class AccumuloInputFormat extends InputFormat<Key,Value> {
    ...
    protected static TabletLocator getTabletLocator(JobContext job) throws TableNotFoundException {
        ...
        return TabletLocator
                .getInstance(instance, new AuthInfo(username, password, instance.getInstanceID()), new Text(Tables.getTableId(instance, tableName)));
    }
    ...
    protected static void validateOptions(JobContext job) throws IOException {
        ...
        try {
            ...
            if (!c.securityOperations().hasTablePermission(getUsername(job), getTablename(job), TablePermission.READ)) throw new IOException(
