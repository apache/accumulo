...
public class TFile {
    ...
    public static class Writer implements Closeable {
         ...
         * @param comparator
         *            Leave comparator as null or empty string if TFile is not sorted. Otherwise, provide the string name for the comparison algorithm for keys.
         *            Two kinds of comparators are supported.
         *            <ul>
         ...
         *            constructor (with no parameters). Parameterized RawComparators such as {@link WritableComparator} or {@link JavaSerializationComparator}
         *            may not be directly used. One should write a wrapper class that inherits from such classes and use its default constructor to perform
         ...
         * 
         * @param length
         *            The expected length of the key. If length of the key is not known, set length = -1. Otherwise, the application must write exactly as many
         *            bytes as specified here before calling close on the returned output stream.
         ...
         * 
         * @param length
         *            The expected length of the value. If length of the value is not known, set length = -1. Otherwise, the application must write exactly as
         *            many bytes as specified here before calling close on the returned output stream. Advertising the value size up-front guarantees that the
         *            value is encoded in one chunk, and avoids intermediate chunk buffering.
         * @throws IOException
         * 
         */
        ...
        /**
         * Obtain an output stream for creating a meta block. This function may not be called when there is a key append stream or value append stream active.
         * No more key-value insertion is allowed after a meta data block has been added to TFile.
         * 
         * @param name
        ...
        /**
         * Obtain an output stream for creating a meta block. This function may not be called when there is a key append stream or value append stream active.
         * No more key-value insertion is allowed after a meta data block has been added to TFile. Data will be compressed using the default compressor as
    ...
    public static class Reader implements Closeable {
         ...
         * @param fileLength
         *            The length of TFile. This is required because we have no easy way of knowing the actual size of the input file through the File input
         *            stream.
         * @param conf
         * @throws IOException
         */
         ...
         * 
         * @return If the TFile is not sorted by keys, an empty string will be returned. Otherwise, the actual comparator string that is provided during the
         *         TFile creation time will be returned.
         */
        ...
        /**
         * Get a Comparator object to compare Entries. It is useful when you want stores the entries in a collection (such as PriorityQueue) and perform sorting
         * or comparison among entries based on the keys without copying out the key.
         * 
        ...
        /**
         * if greater is true then returns the beginning location of the block containing the key strictly greater than input key. if greater is false then
         * returns the beginning location of the block greater than equal to the input key
         * 
         * @param key
         *            the input key
         * @param greater
        ...
        /**
         * Get the location pointing to the beginning of the first key-value pair in a compressed block whose byte offset in the TFile is greater than or equal
         ...
         * @return The actual coverage of the returned scanner tries to match the specified byte-region but always round up to the compression block boundaries.
         *         It is possible that the returned scanner contains zero key-value pairs even if length is positive.
         * @throws IOException
         */
        ...
        /**
         * The TFile Scanner. The Scanner has an implicit cursor, which, upon creation, points to the first key-value pair in the scan range. If the scan range
         * is empty, the cursor will point to the end of the scan range.
         * <p>
         * Use {@link Scanner#atEnd()} to test whether the cursor is at the end location of the scanner.
         * <p>
         * Use {@link Scanner#advance()} to move the cursor to the next key-value pair (or end if none exists). Use seekTo methods (
         * {@link Scanner#seekTo(byte[])} or {@link Scanner#seekTo(byte[], int, int)}) to seek to any arbitrary location in the covered range (including
         * backward seeking). Use {@link Scanner#rewind()} to seek back to the beginning of the scanner. Use {@link Scanner#seekToEnd()} to seek to the end of
         * the scanner.
         * <p>
        ...
        public static class Scanner implements Closeable {
            ...
            /**
             * Move the cursor to the first entry whose key is greater than or equal to the input key. Synonymous to seekTo(key, 0, key.length). The entry
            ...
            /**
             * Move the cursor to the first entry whose key is strictly greater than the input key. Synonymous to upperBound(key, 0, key.length). The entry
             * returned by the previous entry() call will be invalid.
             * 
             * @param key
             *            The input key
            ...
            /**
             * Move the cursor to the first entry whose key is strictly greater than the input key. The entry returned by the previous entry() call will be
             * invalid.
             * 
             * @param key
             *            The input key
            ...
            /**
             * Close the scanner. Release all resources. The behavior of using the scanner after calling close is not defined. The entry returned by the
            ...
            public class Entry implements Comparable<RawComparable> {
                ...
                /**
                 * Copy the value into BytesWritable. The input BytesWritable will be automatically resized to the actual value size. The implementation
                 * directly uses the buffer inside BytesWritable for storing the value. The call does not require the value length to be known.
                 * 
                 * @param value
                 * @throws IOException
                 */
                ...
                /**
                 * Writing the value to the output stream. This method avoids copying value data from Scanner into user buffer, then writing to the output
                 ...
                 * @param offset
                 *            The starting offset of the user buffer where we should copy the key into. Requiring the key-length + offset no greater than the
                 *            buffer length.
                 * @return The length of the key.
                 * @throws IOException
                 */
                ...
                /**
                 * Copy value into user-supplied buffer. User supplied buffer must be large enough to hold the whole value. The value part of the key-value pair
                 * pointed by the current cursor is not cached and can only be examined once. Calling any of the following functions more than once without
                 * moving the cursor will result in exception: {@link #getValue(byte[])}, {@link #getValue(byte[], int)}, {@link #getValueStream}.
                 * 
                ...
                /**
                 * Copy value into user-supplied buffer. User supplied buffer must be large enough to hold the whole value (starting from the offset). The value
                 * part of the key-value pair pointed by the current cursor is not cached and can only be examined once. Calling any of the following functions
                 * more than once without moving the cursor will result in exception: {@link #getValue(byte[])}, {@link #getValue(byte[], int)},
                 * {@link #getValueStream}.
                 * 
                 * @return the length of the value. Does not require isValueLengthKnown() to be true.
                 * @throws IOException
                ...
                /**
                 * Stream access to value. The value part of the key-value pair pointed by the current cursor is not cached and can only be examined once.
                 * Calling any of the following functions more than once without moving the cursor will result in exception: {@link #getValue(byte[])},
                 ...
                 * 
                 * @return true if value length is known before hand. Values less than the chunk size will always have their lengths known before hand. Values
                 *         that are written out as a whole (with advertised length up-front) will always have their lengths known in read.
                 */
                ...
                /**
                 * Compare an entry with a RawComparable object. This is useful when Entries are stored in a collection, and we want to compare a user supplied
                 * key.
                 */
                @Override
