 ...
 * Mutation from the Map() and Reduce() functions.
 * 
...
public class AccumuloOutputFormat extends OutputFormat<Text, Mutation> {
	 ...
	 * @param job the Map/Reduce job object
	 * @param user the username, which must have the Table.CREATE permission to create tables
	 * @param passwd the passwd for the username
	 * @param createTables the output format will create new tables as necessary. Table names can only be alpha-numeric and underscores.
	 * @param defaultTable the table to use when the tablename is null in the write call
	 */
	...
	private static class AccumuloRecordWriter extends RecordWriter<Text, Mutation> {
		...
		/**
		 * Push a mutation into a table.  If table is null, the defaultTable will be used. If canCreateTable is set, the 
