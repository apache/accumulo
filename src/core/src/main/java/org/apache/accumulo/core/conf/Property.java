...
public enum Property {
    ...
    GENERAL_CLASSPATHS(AccumuloClassLoader.CLASSPATH_PROPERTY_NAME, AccumuloClassLoader.DEFAULT_CLASSPATH_VALUE, PropertyType.STRING,
            "A list of all of the places to look for a class. Order does matter, as it will look for the jar "
                    ...
                    + "along with accumulo lib and zookeeper directory. Supports full regex on filename alone."), // needs special treatment in accumulo start
                                                                                                                  // jar
    GENERAL_DYNAMIC_CLASSPATHS(AccumuloClassLoader.DYNAMIC_CLASSPATH_PROPERTY_NAME, AccumuloClassLoader.DYNAMIC_CLASSPATH_VALUE, PropertyType.STRING,
    ...
    TSERV_INDEXCACHE_SIZE("tserver.cache.index.size", "50M", PropertyType.MEMORY, "Specifies the size of the cache for file indices."),
    ...
    TSERV_MAJC_MAXOPEN("tserver.compaction.major.files.open.max", "90", PropertyType.COUNT, "Max number of files a major compaction can open at once. "
            + "At runtime this number is divided by the concurrent number of compactors."),
    TSERV_MAX_OPENFILES("tserver.files.open.max", "150", PropertyType.COUNT, "Maximum total map files that all tablets in a tablet server can open. "
            + "This includes major compactions. So the number of map files that can be opened for searches is: " + "<code>tserver.files.open.max - "
            + TSERV_MAJC_MAXOPEN.getKey() + "</code>"),
    ...
    LOGGER_ARCHIVE("logger.archive", "false", PropertyType.BOOLEAN, "determines if write-ahead logs are archived in hdfs"),
    LOGGER_MONITOR_FS(
            "logger.monitor.fs",
            "true",
            PropertyType.BOOLEAN,
    ...
    TABLE_MAJC_COMPACTALL_AT("table.compaction.major.everything.at", "19700101000000GMT", PropertyType.DATETIME,
    ...
    TABLE_FILE_BLOCK_SIZE("table.file.blocksize", "0B", PropertyType.MEMORY,
            "Overrides the hadoop dfs.block.size setting so that map files have better query performance. " + "The maximum value for this is "
                    + Integer.MAX_VALUE),
    ...
    TABLE_INDEXCACHE_ENABLED("table.cache.index.enable", "false", PropertyType.BOOLEAN, "Determines whether index cache is enabled."),
    ...
    private static final EnumSet<Property> fixedProperties = EnumSet.of(Property.TSERV_CLIENTPORT, Property.TSERV_NATIVEMAP_ENABLED,
            Property.TSERV_MAJC_MAXOPEN, Property.TSERV_MAX_OPENFILES, Property.TSERV_MAJC_MAXCONCURRENT, Property.TSERV_LOGGER_COUNT, Property.LOGGER_PORT,
    ...
    public static boolean isValidZooPropertyKey(String key) {
        ...
        return key.startsWith(Property.TABLE_PREFIX.getKey()) || key.startsWith(Property.TSERV_PREFIX.getKey())
                || key.startsWith(Property.LOGGER_PREFIX.getKey()) || key.startsWith(Property.MASTER_PREFIX.getKey())
