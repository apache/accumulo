...
public enum Property
{
	...
	GENERAL_DYNAMIC_CLASSPATHS(AccumuloClassLoader.DYNAMIC_CLASSPATH_PROPERTY_NAME, AccumuloClassLoader.DYNAMIC_CLASSPATH_VALUE,
			PropertyType.STRING,
	...
	MASTER_LOGGER_BALANCER("master.logger.balancer","org.apache.accumulo.server.master.balancer.SimpleLoggerBalancer",
			PropertyType.CLASSNAME,
	        "The balancer class that accumulo will use to make logger assignment decisions."),
	MASTER_RECOVERY_REDUCERS("master.recovery.reducers","10",
			PropertyType.COUNT,
			"Number of reducers to use to sort recovery logs (per log)"),
	MASTER_RECOVERY_MAXAGE("master.recovery.max.age","60m",
			PropertyType.TIMEDURATION,
			"Recovery files older than this age will be removed."),
	MASTER_RECOVERY_MAXTIME("master.recovery.time.max","30m",
			PropertyType.TIMEDURATION,
			"The maximum time to attempt recovery before giving up"),
    MASTER_RECOVERY_QUEUE("master.recovery.queue","default",
            PropertyType.STRING,
            "Priority queue to use for log recovery map/reduce jobs."),
    MASTER_RECOVERY_POOL("master.recovery.pool","recovery",
                    PropertyType.STRING,
                    "Priority queue to use for log recovery map/reduce jobs."),
	...
	TSERV_DATACACHE_SIZE("tserver.cache.data.size","100M",
					 			PropertyType.MEMORY,
								"Specifies the size of the cache for file data blocks."),		
	TSERV_INDEXCACHE_SIZE("tserver.cache.index.size","50M",
				 			PropertyType.MEMORY,
							"Specifies the size of the cache for file indices."),
	TSERV_PORTSEARCH("tserver.port.search","false",
			PropertyType.BOOLEAN,
			"if the ports above are in use, search higher ports until one is available"),
	TSERV_CLIENTPORT("tserver.port.client","9997",
			PropertyType.PORT,
	...
	TSERV_WALOG_MAX_SIZE("tserver.walog.max.size","1G",
			PropertyType.MEMORY,
	...
	TSERV_MAJC_MAXOPEN("tserver.compaction.major.files.open.max","90",
			PropertyType.COUNT,
			"Max number of files a major compaction can open at once. " +
			"At runtime this number is divided by the concurrent number of compactors."),
	TSERV_MAX_OPENFILES("tserver.files.open.max","150",
			PropertyType.COUNT,
			"Maximum total map files that all tablets in a tablet server can open. " +
			"This includes major compactions. So the number of map files that can be opened for searches is: " +
			"<code>tserver.files.open.max - "+TSERV_MAJC_MAXOPEN.getKey()+"</code>"),
	TSERV_MAX_IDLE("tserver.files.open.idle","1m",
			PropertyType.TIMEDURATION,
			"Tablet servers leave previously used map files open for future queries. " +
	...
	TSERV_NATIVEMAP_ENABLED("tserver.memory.maps.native.enabled","true",
			...
			"accumulo can hold in memory and avoids Java GC pauses."),
	TSERV_MAXMEM("tserver.memory.maps.max","1G",
			PropertyType.MEMORY,
	...
	TSERV_LOGGER_TIMEOUT("tserver.logger.timeout", "30s",
	        PropertyType.TIMEDURATION,
	        "The time to wait for a logger to respond to a write-ahead request"),
	TSERV_LOGGER_COUNT("tserver.logger.count", "2", 
			PropertyType.COUNT, 
	...
	TSERV_MONITOR_FS("tserver.monitor.fs", "true",
	        PropertyType.BOOLEAN,
	...
	TSERV_MEMDUMP_DIR("tserver.dir.memdump", "/tmp",
	        PropertyType.PATH,
	...
	TABLE_MAJC_RATIO("table.compaction.major.ratio","3",
	...
	TABLE_MAJC_COMPACTALL_AT("table.compaction.major.everything.at","19700101000000GMT",
			PropertyType.DATETIME,
			"This setting specifies a time at which all tablets in a table will major compact to one file, even " +
			"tablets with only one file. When this settings specifies a time in the future, no action is taken. " +
			"When the time is in the past any tablet having a map file older than the specified time will major " +
			"compact to one file. The time specified must conform to the yyyyMMddHHmmssz pattern. See the " +
			"Java SimpleDataFormat java doc for details about this pattern."),
	TABLE_SPLIT_THRESHOLD("table.split.threshold","1G",
			PropertyType.MEMORY,
			"When combined size of mapfiles exceeds this amount a tablet is split."),
	TABLE_MINC_LOGS_MAX("table.compaction.minor.logs.threshold","3",
			PropertyType.COUNT,
	...
	TABLE_SCAN_CACHE_ENABLED("table.scan.cache.enable","false",
			PropertyType.BOOLEAN,
			"Determines whether scan cache is enabled."),
	TABLE_SCAN_CACHE_SIZE("table.scan.cache.size","8M",
			PropertyType.MEMORY,
			"Scan cache size."),
	TABLE_SCAN_MAXMEM("table.scan.max.memory","50M",
			PropertyType.MEMORY,
	...
	TABLE_LOAD_BALANCER("table.balancer","accumulo.server.master.balancer.DefaultLoadBalancer",
			PropertyType.STRING,
	...
	TABLE_FILE_COMPRESSED_BLOCK_SIZE("table.file.compress.blocksize","100K",
			PropertyType.MEMORY,
			"Overrides the hadoop io.seqfile.compress.blocksize setting so that map files have better query performance. " +
			"The maximum value for this is "+Integer.MAX_VALUE),
	TABLE_FILE_BLOCK_SIZE("table.file.blocksize","0B",
			PropertyType.MEMORY,
			"Overrides the hadoop dfs.block.size setting so that map files have better query performance. " +
			"The maximum value for this is "+Integer.MAX_VALUE),
	TABLE_FILE_REPLICATION("table.file.replication","0",
			PropertyType.COUNT,
			"Determines how many replicas to keep of a tables map files in HDFS. " +
			"When this value is LTE 0, HDFS defaults are used."),
	TABLE_WALOG_ENABLED("table.walog.enabled","true",
			PropertyType.BOOLEAN,
			"Use the write-ahead log to prevent the loss of data."),
	TABLE_BLOOM_ENABLED("table.bloom.enabled","false",
			PropertyType.BOOLEAN,
	...
	TABLE_BLOOM_SIZE("table.bloom.size","1048576",
			PropertyType.COUNT,
	...
	TABLE_BLOOM_HASHTYPE("table.bloom.hash.type","murmur",
			PropertyType.STRING,
	...
	TABLE_LOCALITY_GROUPS("table.groups.enabled", "",
	        PropertyType.STRING,
	        "A comma separated list of locality group names to enable for this table."),
	TABLE_CONSTRAINT_PREFIX("table.constraint.",null,
			PropertyType.PREFIX,
	...
	TABLE_INDEXCACHE_ENABLED("table.cache.index.enable","false",
			 PropertyType.BOOLEAN,
	...
	TABLE_ITERATOR_PREFIX("table.iterator.",null,
			PropertyType.PREFIX,
	...
	TABLE_LOCALITY_GROUP_PREFIX("table.group.", null,
			PropertyType.PREFIX,
    ...
    private static final EnumSet<Property> fixedProperties = 
    	EnumSet.of(Property.TSERV_CLIENTPORT, 
    		Property.TSERV_NATIVEMAP_ENABLED, 
    		Property.TSERV_MAJC_MAXOPEN,
    		Property.TSERV_MAX_OPENFILES,
    		Property.TSERV_MAJC_MAXCONCURRENT,
    		Property.TSERV_LOGGER_COUNT,
    		Property.LOGGER_PORT,
