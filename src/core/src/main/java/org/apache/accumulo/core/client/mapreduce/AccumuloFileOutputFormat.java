...
public class AccumuloFileOutputFormat extends FileOutputFormat<Key, Value>
{
	...
	public RecordWriter<Key, Value> getRecordWriter(TaskAttemptContext job)
		...
		final FileSKVWriter out = FileOperations.getInstance().openWriter(file.toString(), file.getFileSystem(conf), conf, AccumuloConfiguration.getDefaultConfiguration());
		out.startDefaultLocalityGroup();

		return new RecordWriter<Key, Value>()
				{
			@Override public void write(Key key, Value value) throws IOException { out.append(key, value); }
			@Override public void close(TaskAttemptContext context) throws IOException, InterruptedException { out.close(); }
				};
			}
	...
	{
		return job.getConfiguration().getInt(BLOCK_SIZE, (int) AccumuloConfiguration.getDefaultConfiguration().getMemoryInBytes(Property.TABLE_FILE_COMPRESSED_BLOCK_SIZE));
	}
	 ...
	 * @param instanceName
	 * @param zooKeepers
	 * @deprecated since 1.4, use {@link #setBlockSize(JobContext, int)} to set block size
	 ...
	 * @return
	 * 
	 * @deprecated since 1.4, use {@link #setBlockSize(JobContext, int)}
	 * 		no other configurations are used by OutputFormat
	 */
