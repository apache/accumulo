...
public class RelativeKey implements WritableComparable<RelativeKey> {
	...
	int fastSkip(DataInput in, Key seekKey, MByteSequence value, Key pkey, Key currKey) throws IOException {
		...
		if(count > 1){	
			...
			Key tmp = new Key(trow.getBackingArray(), trow.offset(), trow.length(), 
					tcf.getBackingArray(), tcf.offset(), tcf.length(),
					tcq.getBackingArray(), tcq.offset(), tcq.length(), 
					tcv.getBackingArray(), tcv.offset(), tcv.length(), tts);
			tmp.setDeleted(pdel);
			pkey.set(tmp);
		}
		
		this.key = new Key(row.getBackingArray(), row.offset(), row.length(), 
				cf.getBackingArray(), cf.offset(), cf.length(),
				cq.getBackingArray(), cq.offset(), cq.length(), 
	...
	public void write(DataOutput out) throws IOException {
		...
		if((fieldsSame & CQ_SAME) == 0){
			...
			/*Integer id = colFams.get(key.getColumnQualifier());
			if(id == null){
				id = nextId++;
				colFams.put(key.getColumnQualifier(), id);
			}
