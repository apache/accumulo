...
public class SortedLogRecoveryTest {
    ...
    static class KeyValue implements Comparable<KeyValue> {
        ...
        KeyValue() { key = new LogFileKey(); value = new LogFileValue(); }
    ...
    private static List<Mutation> recover(Map<String, KeyValue[]> logs, KeyExtent extent) throws IOException {
        ...
        try {
            for (Entry<String, KeyValue[]> entry: logs.entrySet()) {
                ...
                Writer map = new MapFile.Writer(conf,
                                                local,
                                                path + "/log1", 
                                                LogFileKey.class, 
                                                LogFileValue.class);
    ...
    public void testCompactionCrossesLogs() throws IOException {
        ...
        KeyValue entries4[] = new KeyValue[] {
        		createKeyValue(OPEN, 0, 3, "69"),
        		createKeyValue(DEFINE_TABLET, 1, 3, extent),
        		createKeyValue(MUTATION, 2, 3, ignored),
        		createKeyValue(MUTATION, 3, 3, ignored),
        		createKeyValue(MUTATION, 4, 3, ignored),
        };
        KeyValue entries5[] = new KeyValue[] {
        		createKeyValue(OPEN, 0, 4, "70"),
        		createKeyValue(DEFINE_TABLET, 1, 4, extent),
        		createKeyValue(COMPACTION_START, 3, 4, "thisfile"),
    ...
    public void testCompactionCrossesLogs5() throws IOException {
        ...
        KeyValue entries2[] = new KeyValue[] {
                createKeyValue(OPEN, 5, -1, "2"),
                createKeyValue(DEFINE_TABLET, 6, 1, extent),
                createKeyValue(MUTATION, 7, 1, ignored),
        };
        KeyValue entries3[] = new KeyValue[] {
                createKeyValue(OPEN, 8, -1, "3"),
                createKeyValue(DEFINE_TABLET, 9, 1, extent),
                createKeyValue(COMPACTION_FINISH, 10, 1, null),
                createKeyValue(COMPACTION_START, 12, 1, "newfile"),
                createKeyValue(COMPACTION_FINISH, 13, 1, null),
                //createKeyValue(COMPACTION_FINISH, 14, 1, null),
                createKeyValue(MUTATION, 11, 1, ignored),
                createKeyValue(MUTATION, 15, 1, m),
                createKeyValue(MUTATION, 16, 1, m2),
        };
        KeyValue entries4[] = new KeyValue[] {
        		createKeyValue(OPEN, 17, -1, "4"),
        		createKeyValue(DEFINE_TABLET, 18, 1, extent),
        		createKeyValue(COMPACTION_START, 20, 1, "file"),
        		createKeyValue(MUTATION, 19, 1, m3),
    ...
    public void testCompactionCrossesLogs6() throws IOException {
        ...
        KeyValue entries[] = new KeyValue[] {
                createKeyValue(OPEN, 0, 1, "2"),
                createKeyValue(DEFINE_TABLET, 1, 1, extent),
                createKeyValue(MUTATION, 1, 1, ignored),
                createKeyValue(MUTATION, 3, 1, m),
        };
        KeyValue entries2[] = new KeyValue[] {
                createKeyValue(OPEN, 0, 1, "2"),
                createKeyValue(DEFINE_TABLET, 1, 1, extent),
                createKeyValue(COMPACTION_START, 2, 1, "somefile"),
                createKeyValue(COMPACTION_FINISH, 3, 1, "somefile"),
                createKeyValue(MUTATION, 3, 1, m2),
        };
    ...
    public void testSkipSuccessfulCompactionAcrossFiles() throws IOException {
        ...
        KeyValue entries2[] = new KeyValue[] {
                createKeyValue(OPEN, 4, -1, "1"),
                createKeyValue(DEFINE_TABLET, 5, 1, extent),
    ...
    public void testDoubleFinish() throws IOException {
        ...
        KeyValue entries[] = new KeyValue[] {
                ...
                createKeyValue(MUTATION, 5, 1, m),
                createKeyValue(MUTATION, 7, 1, m2),
        };
        Map<String, KeyValue[]> logs = new TreeMap<String, KeyValue[]>();
        logs.put("entries", entries);
        // Recover
    ...
    public void testBug1() throws IOException {
        ...
        KeyValue entries[] = new KeyValue[] {
                createKeyValue(OPEN, 0, -1, "1"),
                createKeyValue(DEFINE_TABLET, 1, 1, extent),
                createKeyValue(COMPACTION_START, 30, 1, "somefile"),
                createKeyValue(COMPACTION_FINISH, 32, 1, "somefile"),
                createKeyValue(MUTATION, 29, 1, m1),
    ...
    public void testBug2() throws IOException {
        ...
        KeyValue entries2[] = new KeyValue[] {
                createKeyValue(OPEN, 5, -1, "1"),
                createKeyValue(DEFINE_TABLET, 6, 1, extent),
                createKeyValue(COMPACTION_START, 8, 1, "somefile"),
                createKeyValue(MUTATION, 7, 1, m2),
    ...
    public void testCompactionCrossesLogs4() throws IOException {
        ...
        KeyValue entries2[] = new KeyValue[] {
                createKeyValue(OPEN, 5, -1, "2"),
                createKeyValue(DEFINE_TABLET, 6, 1, extent),
                createKeyValue(MUTATION, 7, 1, m3),
                createKeyValue(MUTATION, 8, 1, m4),
        };
        KeyValue entries3[] = new KeyValue[] {
                createKeyValue(OPEN, 9, -1, "3"),
                ...
                createKeyValue(MUTATION, 8, 1, m5),
                createKeyValue(MUTATION, 20, 1, m6),
        };
        Map<String, KeyValue[]> logs = new TreeMap<String, KeyValue[]>();
        logs.put("entries", entries);
        logs.put("entries2", entries2);
    ...
    public void testLookingForBug3() throws IOException {
        ...
        KeyValue entries[] = new KeyValue[] {
                ...
                createKeyValue(DEFINE_TABLET, 1, 1, extent),
                createKeyValue(COMPACTION_START, 2, 1, "somefile"),
                createKeyValue(COMPACTION_FINISH, 3, 1, null),
                createKeyValue(MUTATION, 1, 1, ignored),
                createKeyValue(MUTATION, 3, 1, m),
                createKeyValue(MUTATION, 3, 1, m2),
        ...
        KeyValue entries2[] = new KeyValue[] {
                createKeyValue(OPEN, 0, -1, "2"),
                createKeyValue(DEFINE_TABLET, 1, 1, extent),
                createKeyValue(COMPACTION_START, 2, 1, "somefile2"),
                createKeyValue(MUTATION, 3, 1, m4),
    ...
    public void testMultipleTabletDefinition() throws Exception {
    	...
    	KeyValue entries[] = new KeyValue[] {
                ...
                createKeyValue(COMPACTION_START, 3, 2, "somefile"),
