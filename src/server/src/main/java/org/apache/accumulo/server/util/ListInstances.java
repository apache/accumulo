...
public class ListInstances {
  ...
  public static void main(String[] args) {
    ...
    if (args.length == 1) {
      ...
      zooKeepers = AccumuloConfiguration.getSystemConfiguration().get(Property.INSTANCE_ZK_HOST);
    }
  ...
  private static String getMaster(UUID iid) {
    ...
    ZooKeeper zk = ZooSession.getSession(zooKeepers, 30000);
    
    try {
      String masterLocPath = Constants.ZROOT + "/" + iid + Constants.ZMASTER_LOCK;
      byte[] master = ZooLock.getLockData(zk, masterLocPath);
      if (master == null) {
        return null;
      }
  ...
  private static TreeMap<String,UUID> getInstanceNames() {
    ...
    try {
      names = zk.getChildren(instancesPath, false);
    } catch (Exception e) {
      handleException(e);
      return tm;
    }
    ...
    for (String name : names) {
      ...
      try {
        UUID iid = UUID.fromString(new String(zk.getData(instanceNamePath, false, null)));
        tm.put(name, iid);
      } catch (Exception e) {
        handleException(e);
        tm.put(name, null);
