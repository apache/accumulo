...
public class AlterTablePerm extends Test {
	...
	public static void alter(State state, Properties props) throws Exception
	{
		...
		if ("system".equals(sourceUser))
		{
			...
			canGive = SecurityHelper.getSysPerm(state, SecurityHelper.getSysUserName(state), SystemPermission.ALTER_USER) || SecurityHelper.getTabPerm(state, SecurityHelper.getSysUserName(state), TablePermission.GRANT);
