...
public class BulkImport extends Test {
	...
	public static class RFileBatchWriter implements BatchWriter {
		...
		public RFileBatchWriter(Configuration conf, FileSystem fs, String file) throws IOException{
			CachableBlockFile.Writer cbw = new CachableBlockFile.Writer(fs.create(new Path(file), false, conf.getInt("io.file.buffer.size", 4096), (short)conf.getInt("dfs.replication", 3), conf.getLong("dfs.block.size", 1<<26)),"gz",conf);
