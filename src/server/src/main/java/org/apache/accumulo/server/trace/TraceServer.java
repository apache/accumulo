...
public class TraceServer implements Watcher {
    ...
    static class ByteArrayTransport extends TTransport {
		...
		public byte[] get() { return out.get(); }
		public int len() { return out.len(); }
    }
    ...
    private void registerInZooKeeper(String name) throws Exception {
        String path = ZooUtil.getRoot(HdfsZooInstance.getInstance()) + Constants.ZTRACERS;
        ZooKeeper zoo = new ZooKeeper(conf.get(Property.INSTANCE_ZK_HOST),
                                      (int) conf.getTimeInMillis(Property.INSTANCE_ZK_TIMEOUT),
                                      this);
        path += "/trace-";
        path = zoo.create(path, name.getBytes(), ZooDefs.Ids.OPEN_ACL_UNSAFE, CreateMode.EPHEMERAL_SEQUENTIAL);
