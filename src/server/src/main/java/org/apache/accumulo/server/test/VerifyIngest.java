    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            while (connector == null) {
                try {
                    connector = instance.getConnector(rootCredentials.user, rootCredentials.password);
                ...
                if (ingestArgs.useGet) {
                    ...
                    for (Entry<Key,Value> entry : scanner) {
                        ...
                        if (expectedRow >= (ingestArgs.rows + ingestArgs.startRow)) {
                            log.error("expectedRow (" + expectedRow + ") >= (ingestArgs.rows + ingestArgs.startRow)  ("
                                    + (ingestArgs.rows + ingestArgs.startRow) + "), get batch returned data passed end key");
                            errors++;
                            break;
                        }
