...
public class ContinuousBatchWalker {
	...
	public static void main(String[] args) throws Exception {
		...
		if(args.length != 10){
			throw new IllegalArgumentException("usage : "+ContinuousBatchWalker.class.getName()+" [--debug <debug log>] <instance name> <zookeepers> <user> <pass> <table> <min> <max> <sleep time> <batch size> <query threads>");
		}
	...
	/*private static void runSequentialScan(Scanner scanner, List<Range> ranges) {
		Set<Text> srowsSeen = new HashSet<Text>();
		...
		for (Range range : ranges) {
			...
			for (Entry<Key,Value> entry : scanner) {
				srowsSeen.add(entry.getKey().getRow());
				scount++;
			}
		}
		...
		System.out.println("SRQ "+(st2 - st1)+" "+srowsSeen.size() +" "+scount);
	}*/
