    ...
    public long isReady(long tid, Master master) throws Exception {
        ...
        while (ri.hasNext()) {
            ...
            while (row.hasNext()) {
                ...
                if (Constants.METADATA_COMPACT_COLUMN.equals(key.getColumnFamily(), key.getColumnQualifier())) tabletCompactID = Long.parseLong(entry
                        .getValue().toString());
...
public class CompactRange extends MasterRepo {
    ...
    public CompactRange(String tableId, byte[] startRow, byte[] endRow) throws ThriftTableOperationException {
        ...
        if (this.startRow != null && this.endRow != null && new Text(startRow).compareTo(new Text(endRow)) >= 0) throw new ThriftTableOperationException(
                tableId, null, TableOperation.COMPACT, TableOperationExceptionType.BAD_RANGE, "start row must be less than end row");
    }
