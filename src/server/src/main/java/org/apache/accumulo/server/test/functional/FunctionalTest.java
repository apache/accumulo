...
public abstract class FunctionalTest {
	...
	protected Instance getInstance(){
		return new ZooKeeperInstance(getInstanceName(), AccumuloConfiguration.getSystemConfiguration().get(Property.INSTANCE_ZK_HOST));
	}
	...
	/**
	 * A utility method for use by functional test that ensures a tables
	 * has between min and max split points inclusive.  If not an exception
	 * is thrown.
	 * 
	 */
	...
	/**
	 * A utility function that checks that each tablet has an expected number 
	 * of map files.
	 * 
	 */
	...
	protected void bulkImport(FileSystem fs, String table, String dir) throws Exception {
		...
		fs.delete(new Path(dir+"_failures"), true);

		getConnector().tableOperations().importDirectory(table, dir, dir+"_failures", 4, 20, false);

		if(fs.listStatus(new Path(dir+"_failures")).length > 0){
