 ...
 * 
 * Typically, a client will read the list of tablets, and begin an operation on
 * that tablet at the location listed in the metadata table. When a tablet
 * splits, the information read from the metadata table doesn't match reality,
 * so the operation fails, and must be retried. But the operation will take
 * place either on the parent, or at a later time on the children. It won't take
 * place on just half of the tablet.
 * 
 * However, when a merge occurs, the operation may have succeeded on one section
 * of the merged area, and not on the others, when the merge occurs. There is no
 * way to retry the request at a later time on an unmodified tablet.
 * 
 * The code below uses read-write lock to prevent some operations while a merge
 * is taking place. Normal operations, like bulk imports, will grab the read
 * lock and prevent merges (writes) while they run. Merge operations will lock
 * out some operations while they run.
 */
	...
	public Repo<Master> call(long tid, Master env) throws Exception {
	    ...
	    if (start != null && end != null)
	        if (start.compareTo(end) >= 0)
	            throw new ThriftTableOperationException(tableId, null, TableOperation.MERGE, TableOperationExceptionType.BAD_RANGE, "start row must be less than end row");
