...
public class SimpleLoggerBalancer implements LoggerBalancer {
	...
	@Override
	public void balance(List<LoggerUser> current, List<String> loggers,
			Map<LoggerUser, List<String>> assignmentsOut, int loggersPerServer) {
		...
		List<Entry<String, List<LoggerUser>>> byCount = 
			new ArrayList<Entry<String, List<LoggerUser>>>(counts.entrySet());
		Collections.sort(byCount,
				new Comparator<Entry<String, List<LoggerUser>>>() {
					@Override
					public int compare(Entry<String, List<LoggerUser>> o1,
							Entry<String, List<LoggerUser>> o2) {
