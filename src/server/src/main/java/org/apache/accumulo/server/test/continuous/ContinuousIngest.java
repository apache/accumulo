...
public class ContinuousIngest {
	...
	public static void main(String[] args) throws Exception {
		...
		if(args.length != 12){
			throw new IllegalArgumentException("usage : "+ContinuousIngest.class.getName()+" [--debug <debug log>] <instance name> <zookeepers> <user> <pass> <table> <min> <max> <max colf> <max colq> <max mem> <max latency> <max threads>");
		}
		...
		while(true){
			...
			for(int index = 0; index < flushInterval; index++){
				...
				Mutation m = genMutation(rowLong, cf, cq, ingestInstanceId, count, null, r);
				count++;
				bw.addMutation(m);	
			}
			...
			for(int depth = 1; depth < maxDepth; depth++){
				for(int index = 0; index < flushInterval; index++){
					...
					prevRows[index] = rowLong;
					Mutation m = genMutation(rowLong, r.nextInt(maxColF), r.nextInt(maxColQ), ingestInstanceId, count, prevRow, r);
					count++;
					bw.addMutation(m);	
				}
			...
			for(int index = 0; index < flushInterval - 1; index++){
				Mutation m = genMutation(firstRows[index], firstColFams[index], firstColQuals[index], ingestInstanceId, count, genRow(prevRows[index+1]), r);
				count++;
				bw.addMutation(m);
			}
			lastFlushTime = flush(bw, count, flushInterval, lastFlushTime);
	...
	public static Mutation genMutation(long rowLong, int cfInt, int cqInt, byte[] ingestInstanceId, long count, byte[] prevRow, Random r) {
		...
		m.put(new Text(cfString), new Text(cqString), createValue(ingestInstanceId, count, prevRow));
		return m;
	}
	...
	private static Value createValue(byte[] ingestInstanceId, long count, byte[] prevRow) {
		byte val[] = new byte[ingestInstanceId.length + 16 + (prevRow == null ? 0 : prevRow.length) + 2];
