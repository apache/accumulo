...
public class TableLoadBalancer extends TabletBalancer {
    ...
    private TabletBalancer getBalancerForTable(String table)
    {
    ...
    private TableOperations getTableOperations()
    {
    	if(tops == null)
			try {
				tops = HdfsZooInstance.getInstance().getConnector(SecurityConstants.systemCredentials.user,SecurityConstants.systemCredentials.password).tableOperations();
