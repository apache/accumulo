...
import org.apache.accumulo.core.zookeeper.ZooLock.LockID;
...
public class MetadataConstraints implements Constraint {
  ...
  private static final HashSet<ColumnFQ> validColumnQuals = new HashSet<ColumnFQ>(Arrays.asList(new ColumnFQ[] {Constants.METADATA_PREV_ROW_COLUMN,
      ...
      Constants.METADATA_LOCK_COLUMN,}));
  
  private static boolean isValidColumn(ColumnUpdate cu) {
    if (Constants.METADATA_LOG_COLUMN_FAMILY.equals(new Text(cu.getColumnFamily()))) {
      return true;
    }
    
    if (Constants.METADATA_SCANFILE_COLUMN_FAMILY.equals(new Text(cu.getColumnFamily()))) {
      return true;
    }
    
    if (Constants.METADATA_DATAFILE_COLUMN_FAMILY.equals(new Text(cu.getColumnFamily()))) {
      return true;
    }
    
    if (Constants.METADATA_CURRENT_LOCATION_COLUMN_FAMILY.equals(new Text(cu.getColumnFamily()))) {
      return true;
    }
    
    if (Constants.METADATA_LAST_LOCATION_COLUMN_FAMILY.equals(new Text(cu.getColumnFamily()))) {
      return true;
    }
    
    if (Constants.METADATA_FUTURE_LOCATION_COLUMN_FAMILY.equals(new Text(cu.getColumnFamily()))) {
      return true;
    }
