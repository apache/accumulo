...
/**
 * This little program can be used to write a lot
 * of entries to the !METADATA table and measure the
 * performance of varying numbers of threads doing
 * !METADATA lookups using the batch scanner.
 * 
 *
 */
...
public class MetadataBatchScanTest {
	...
	public static void main(String[] args) throws Exception {
		...
		final Connector connector = new ZooKeeperInstance("acu13", "localhost").getConnector(SecurityConstants.SYSTEM_USERNAME, SecurityConstants.systemCredentials.password);
	...
	private static ScanStats runScanTest(Connector connector, int numLoop, List<Range> ranges) throws Exception {
		Scanner scanner = null;/*connector.createScanner(Constants.METADATA_TABLE_NAME, Constants.NO_AUTHS);
		ColumnFQ.fetch(scanner, Constants.METADATA_LOCATION_COLUMN);
