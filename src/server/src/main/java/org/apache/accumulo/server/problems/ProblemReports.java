...
public class ProblemReports implements Iterable<ProblemReport> {
     ...
     * 
     * make the thread pool use a bounded queue to avoid the case where problem reports are not being processed because the whole system is in a really bad
     * state (like HDFS is down) and everything is reporting lots of problems, but problem reports can not be processed
     */
    private ExecutorService reportExecutor = new ThreadPoolExecutor(0, 1, 60, TimeUnit.SECONDS, new LinkedBlockingQueue<Runnable>(500),
            new NamingThreadFactory("acu-problem-reporter"));
    ...
    public void deleteProblemReports(String table) throws Exception {
        ...
        Connector connector = HdfsZooInstance.getInstance().getConnector(SecurityConstants.SYSTEM_USERNAME, SecurityConstants.systemCredentials.password);
        Scanner scanner = connector.createScanner(Constants.METADATA_TABLE_NAME, Constants.NO_AUTHS);
        scanner.setScanIterators(1, SortedKeyIterator.class.getName(), "keys-only");
                    ...
                    if (iter1 == null) {
                        try {
                            ...
                            if (table == null || table.equals(Constants.METADATA_TABLE_ID)) {
                                children = ZooSession.getSession().getChildren(ZooUtil.getRoot(HdfsZooInstance.getInstance()) + Constants.ZPROBLEMS, false);
                            } else {
                    ...
                    if (iter2 == null) {
                        try {
                            if ((table == null || !table.equals(Constants.METADATA_TABLE_ID)) && iter1Count == 0) {
                                Connector connector = HdfsZooInstance.getInstance().getConnector(SecurityConstants.SYSTEM_USERNAME,
