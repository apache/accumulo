...
public class GarbageCollectWriteAheadLogs {
        ...
        for (final Entry<String, ArrayList<String>> serverFiles : serverToFileMap.entrySet()) {
            ...
            final List<String> files = serverFiles.getValue();
                threadPool.submit(new Runnable() {
                    ...
                    public void run() {
                        try {
                            Iface logger = ThriftUtil.getClient(new MutationLogger.Client.Factory(), server, Property.LOGGER_PORT, Property.TSERV_LOGGER_TIMEOUT, AccumuloConfiguration.getSystemConfiguration());
                            try {
                            ...
                            for (String file : files) {
                                try {
                                    for (FileStatus match : fs.globStatus(new Path(Constants.getRecoveryDir(), file + "*"))) {
    ...
    private static int removeMetadataEntries(Map<String, String> fileToServerMap, GCStatus status) throws IOException, KeeperException, InterruptedException {
        int count = 0;
        Iterator<LogEntry> iterator = MetadataTable.getLogEntries(SecurityConstants.systemCredentials);
    ...
    private static int scanServers(Map<String, String> fileToServerMap) throws Exception {
        ...
        for (String server : servers) {
            ...
            try {
                ...
                Iface logger = ThriftUtil.getClient(new MutationLogger.Client.Factory(), address, Property.LOGGER_PORT, Property.TSERV_LOGGER_TIMEOUT, AccumuloConfiguration.getSystemConfiguration());
