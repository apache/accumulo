...
public class SplitRecoveryTest extends FunctionalTest {
	...
	private void runSplitRecoveryTest(int failPoint, String mr, int extentToSplit, ZooLock zl, KeyExtent... extents) throws Exception {
		...
		for (int i = 0; i < extents.length; i++) {
			...
			String tdir = "/dir_"+i;
			MetadataTable.addTablet(extent, tdir, SecurityConstants.systemCredentials, TabletTime.LOGICAL_TIME_ID, zl);
			...
			MetadataTable.updateTabletDataFile(extent, mapFiles, SecurityConstants.systemCredentials, zl);
		}
	...
	private void splitPartiallyAndRecover(KeyExtent extent, KeyExtent high, KeyExtent low, double splitRatio, SortedMap<String, DataFileValue> mapFiles, Text midRow, String location, int steps, ZooLock zl) throws Exception {
		...
		MetadataTable.splitDatafiles(extent.getTableId(), midRow, splitRatio, new HashMap<String, FileUtil.FileInfo>(), mapFiles, lowDatafileSizes, highDatafileSizes, highDatafilesToRemove);
		...
		if(steps >= 1) MetadataTable.addNewTablet(low, "/lowDir",  instance, lowDatafileSizes, SecurityConstants.systemCredentials, TabletTime.LOGICAL_TIME_ID+"0", zl);
	...
	private void ensureTabletHasNoUnexpectedMetadataEntries(KeyExtent extent, SortedMap<String, DataFileValue> expectedMapFiles) throws Exception {
		...
		MetadataTable.getTabletAndPrevTabletKeyValues(tkv, extent, null, SecurityConstants.systemCredentials);
		...
		SortedMap<String, DataFileValue> fixedMapFiles = MetadataTable.getDataFileSizes(extent, SecurityConstants.systemCredentials);
