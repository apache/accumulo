...
public class UpgradeMetadataTable {
	...
	private static void processTabletFiles(Configuration conf, FileSystem fs, long maxTime, SortedMap<String, String> tableIds, TreeMap<Key, Value> tabletEntries, FileSKVWriter out) throws Exception {
		...
		for (Entry<Key, Value> entry : tabletEntries.entrySet()) {
			...
			if(entry.getKey().getColumnFamily().toString().equals("~tab") && 
					entry.getKey().getColumnQualifier().toString().equals("~pr"))
			{
			...
			try{
				if(key.getRow().getLength() > 0 && key.getRow().getBytes()[0] == '~'){
					...
					if(newKey.getColumnFamily().equals(Constants.METADATA_DATAFILE_COLUMN_FAMILY) && !newKey.isDeleted()){
						...
						else if(new KeyExtent(newKey.getRow(), (Text)null).getTableId().toString().equals(Constants.METADATA_TABLE_ID) &&
								per.getBytes()[0] != '~')
	...
	private static long determineMaxTime(Configuration conf, FileSystem fs) throws Exception {
		String inputDir = Constants.getMetadataTableDir();
	...
	public static void main(String[] args) throws Exception {
		...
		String rootDir = Constants.getMetadataTableDir()+"/root_tablet";
		...
		for (FileStatus fileStatus : rootFiles) {
			FileSKVIterator in = FileOperations.getInstance().openReader(fileStatus.getPath().toString(), true, fs, conf, AccumuloConfiguration.getDefaultConfiguration());
		...
		FileSKVWriter defaultTabletFile = FileOperations.getInstance().openWriter(Constants.getMetadataTableDir()+"_new"+"/default_tablet/00000_00000.rf", fs, conf, AccumuloConfiguration.getDefaultConfiguration());
		...
		FileSKVWriter rootTabletFile = FileOperations.getInstance().openWriter(Constants.getMetadataTableDir()+"_new"+"/root_tablet/00000_00000.rf", fs, conf, AccumuloConfiguration.getDefaultConfiguration());
	...
	private static void flushRow(FileSKVWriter out, TreeMap<Key, Value> tm, long maxTime) throws IOException {
		if(tm.size() > 0){
			Key logicalTimeKey = new Key(tm.lastKey().getRow(), Constants.METADATA_TIME_COLUMN.getColumnFamily(), Constants.METADATA_TIME_COLUMN.getColumnQualifier(), System.currentTimeMillis());
