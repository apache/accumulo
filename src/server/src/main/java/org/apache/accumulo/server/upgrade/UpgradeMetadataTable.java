...
public class UpgradeMetadataTable {
    ...
    private static void processTabletFiles(Configuration conf, FileSystem fs, long maxTime, SortedMap<String,String> tableIds,
            TreeMap<Key,Value> tabletEntries, FileSKVWriter out) throws Exception {
                ...
                if (key.getRow().getLength() > 0 && key.getRow().getBytes()[0] == '~') {
                    ...
                    if (newKey.getColumnFamily().equals(Constants.METADATA_DATAFILE_COLUMN_FAMILY) && !newKey.isDeleted()) {
                        ...
                        if (per == null) val = new Value(val.get(), true);
                                ...
                                && per.getBytes()[0] != '~') val = KeyExtent.encodePrevEndRow(translateRow(per, tableIds));
    ...
    public static void main(String[] args) throws Exception {
        ...
        FileSKVWriter defaultTabletFile = FileOperations.getInstance().openWriter(Constants.getMetadataTableDir() + "_new" + "/default_tablet/00000_00000.rf",
                fs, conf, AccumuloConfiguration.getDefaultConfiguration());
        ...
        FileSKVWriter rootTabletFile = FileOperations.getInstance().openWriter(Constants.getMetadataTableDir() + "_new" + "/root_tablet/00000_00000.rf", fs,
        ...
        // root's prev row
        Key rootPrevRowKey = new Key(rootExtent, Constants.METADATA_PREV_ROW_COLUMN.getColumnFamily(), Constants.METADATA_PREV_ROW_COLUMN.getColumnQualifier(),
                0);
