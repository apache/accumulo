...
public class ZooKeeperUpgrade extends Initialize
{
    ...
    static void upgradeZooKeeper() throws IOException, KeeperException, InterruptedException
    {
        ...
        ZooUtil.putPersistentData(zkInstanceRoot + Constants.ZTABLES, BigInteger.valueOf(tableNames.size() - 1).toString(Character.MAX_RADIX).getBytes(), NodeExistsPolicy.FAIL);
