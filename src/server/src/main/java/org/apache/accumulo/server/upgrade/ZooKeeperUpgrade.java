...
public class ZooKeeperUpgrade extends Initialize
{
    ...
    static void upgradeZooKeeper(ZooReaderWriter zoo) throws IOException, KeeperException, InterruptedException
    {
        ...
        zoo.putPersistentData(zkInstanceRoot + Constants.ZTABLES, BigInteger.valueOf(tableNames.size() - 1).toString(Character.MAX_RADIX).getBytes(), NodeExistsPolicy.FAIL);
