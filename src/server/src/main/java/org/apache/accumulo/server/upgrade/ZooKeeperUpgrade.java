...
public class ZooKeeperUpgrade extends Initialize {
        ...
        for (String user : users) {
            if (zk.exists(ZKUserPath + "/" + user + "/Tables", false) != null) {
                ...
                for (String table : tables) {
                    if (tableIds.containsKey(table)) move(ZKUserPath + "/" + user + "/Tables.old/" + table,
    ...
    static void upgradeZooKeeper() throws IOException, KeeperException, InterruptedException {
        ...
        for (String tableName : tableNames) {
            ...
            else ZooUtil.recursiveCopyPersistent(oldConfDir, zkInstanceRoot + Constants.ZTABLES + "/" + tableId + Constants.ZTABLE_CONF,
    ...
    private static SortedSet<String> getOldTableNames() throws IOException, KeeperException, InterruptedException {
        ...
        if (ZooUtil.exists(zkInstanceRoot + OLD_TABLE_CONF_DIR)) tableNames.addAll(ZooSession.getSession().getChildren(zkInstanceRoot + OLD_TABLE_CONF_DIR,
                false));
        return tableNames;
    }
