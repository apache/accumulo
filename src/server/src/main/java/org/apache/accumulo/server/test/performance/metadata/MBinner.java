...
public class MBinner {
	...
	static class SimpleLocationObtainer implements TabletLocationObtainer {
		...
		@Override
		public List<TabletLocation> lookupTablets(String tserver, Map<KeyExtent, List<Range>> map, TabletLocator parent) throws AccumuloSecurityException, AccumuloException {
	...
	static class TrieLocator extends TabletLocator {
		...
		public void binMutations(List<Mutation> mutations, Map<String, TabletServerMutations> binnedMutations, List<Mutation> failures) throws AccumuloException, AccumuloSecurityException, TableNotFoundException {
			...
			for (Mutation mut : mutations) {
				...
				/*if(!tl.tablet_extent.contains(row)){
					throw new RuntimeException(tl.tablet_extent+" does not contain "+row);
				}*/
				if(tl != null)
					notNull++;
			}
