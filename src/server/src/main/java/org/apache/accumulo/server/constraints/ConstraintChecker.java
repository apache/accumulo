...
public class ConstraintChecker {
    ...
    public Violations check(Environment env, Mutation m) {
        ...
        if (!env.getExtent().contains(new ComparableBytes(m.getRow()))) {
            ...
            ConstraintViolationSummary cvs = new ConstraintViolationSummary(SystemConstraint.class.getName(), (short) -1, "Mutation outside of tablet extent",
                    1);
            violations.add(cvs);
        ...
        for (Constraint constraint : constrains) {
            ...
            if (violationCodes != null) {
                for (Short vcode : violationCodes) {
                    ConstraintViolationSummary cvs = new ConstraintViolationSummary(constraint.getClass().getName(), vcode,
