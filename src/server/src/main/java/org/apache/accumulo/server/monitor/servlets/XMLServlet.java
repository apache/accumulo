...
public class XMLServlet extends BasicServlet {
	...
	protected void pageBody(HttpServletRequest req, HttpServletResponse resp, StringBuilder sb) {
		...
		for (TabletServerStatus status : Monitor.getMmi().tServerInfo) {
			...
			sb.append("<major>").append("<running>").append(summary.major.running).append("</running>").append("<queued>").append(summary.major.queued).append("</queued>").append("</major>\n");
			sb.append("<minor>").append("<running>").append(summary.minor.running).append("</running>").append("<queued>").append(summary.minor.queued).append("</queued>").append("</minor>\n");
			sb.append("</compactions>\n");
		...
		for (Entry<String, TableInfo> entry : tableStats.entrySet()) {
			...
			sb.append("<majorCompactions>").append("<running>").append(running).append("</running>").append("<queued>").append(queued).append("</queued>").append("</majorCompactions>\n");
			sb.append("</table>\n");
		}
		sb.append("\n</tables>\n");
