...
public class ConcurrencyTest extends FunctionalTest {
	...
	static class ScanTask extends Thread {
		...
		ScanTask(Connector conn, long time) throws Exception{
			...
			scanner.setScanIterators(30, SlowIterator.class.getName(), "slow");
			scanner.setScanIteratorOption("slow", "sleepTime", ""+time);
		}
	...
	public List<TableSetup> getTablesToCreate() {
		...
		TableSetup ts = new TableSetup("cct", 
				parseConfig(pre + "minc.slow=30,"+SlowIterator.class.getName(), pre+"minc.slow.opt.sleepTime=50",
							pre + "majc.slow=30,"+SlowIterator.class.getName(), pre+"majc.slow.opt.sleepTime=50",
