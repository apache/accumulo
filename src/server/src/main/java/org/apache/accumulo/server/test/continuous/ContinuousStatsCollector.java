...
public class ContinuousStatsCollector {
	...
	static class StatsCollectionTask extends TimerTask {
		...
		public StatsCollectionTask(String tableName, String instanceName, String zooHosts, String user, String pass) {
			...
			this.user = user;
			this.pass = pass;
			System.out.println("TIME TABLET_SERVERS TOTAL_ENTRIES TOTAL_INGEST TOTAL_QUERY TABLE_RECS TABLE_RECS_IN_MEM TABLE_INGEST TABLE_QUERY TABLE_TABLETS TABLE_TABLETS_ONLINE"+
		...
		private String getFSStats() throws Exception {
			...
			Path acudir = new Path(Constants.getTablesDir());
			ContentSummary contentSummary = fs.getContentSummary(acudir);
			
			Path tableDir = new Path(Constants.getTablesDir()+"/"+tableId);
		...
		private String getACUStats() throws Exception {
			...
			try{
				...
				MasterMonitorInfo stats = client.getMasterStats(null, SecurityConstants.systemCredentials);
				...
				return ""+stats.tServerInfo.size()+" "+all.recs+" "+ (long)all.ingestRate+" "+ (long)all.queryRate+" "+ti.recs+" "+ti.recsInMemory+" "+(long)ti.ingestRate+" "+(long)ti.queryRate+" "+ti.tablets+" "+ti.onlineTablets;
