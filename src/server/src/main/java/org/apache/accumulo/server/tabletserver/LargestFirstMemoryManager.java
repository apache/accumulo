...
public class LargestFirstMemoryManager implements MemoryManager {
    ...
    LargestFirstMemoryManager() {
        this(AccumuloConfiguration.getSystemConfiguration().getMemoryInBytes(Property.TSERV_MAXMEM), AccumuloConfiguration.getSystemConfiguration().getCount(
    ...
    public MemoryManagementActions getMemoryManagementActions(List<TabletState> tablets) {
        ...
        if (startMinC && largestMemTablet != null) {
            ...
            log.debug(String.format("COMPACTING %s  total = %,d ingestMemory = %,d", largestMemTablet.toString(), (ingestMemory + compactionMemory),
                    ingestMemory));
