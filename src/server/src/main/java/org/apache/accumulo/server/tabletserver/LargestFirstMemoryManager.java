...
public class LargestFirstMemoryManager implements MemoryManager {
	...
	LargestFirstMemoryManager(){
		this(AccumuloConfiguration.getSystemConfiguration().getMemoryInBytes(Property.TSERV_MAXMEM),
				AccumuloConfiguration.getSystemConfiguration().getCount(Property.TSERV_MINC_MAXCONCURRENT),
				Constants.TSERV_MINC_MAXCONCURRENT_NUMWAITING_MULTIPLIER);
	}
		...
		for (TabletState ts : tablets) {
			...
			if (mcmts==0 && mts > 0) {
				...
				if (!mincIdleThresholds.containsKey(tableId))
					mincIdleThresholds.put(tableId, AccumuloConfiguration.getTableConfiguration(instanceId, tableId.toString()).getTimeInMillis(Property.TABLE_MINC_COMPACT_IDLETIME));
