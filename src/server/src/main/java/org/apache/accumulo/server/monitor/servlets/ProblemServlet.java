...
public class ProblemServlet extends BasicServlet {
	...
	private static class ClearTableProblemsLinkType extends StringType<String> {
		...
		public String format(Object obj) {
			if (obj == null)
				return "-";
			String table = String.valueOf(obj);
			return String.format("<a href='/op?table=%s&action=clearTableProblems&redir=%s'>clear ALL %s problems</a>", table, currentPage(req), Tables.getPrintableTableNameFromId(tidToNameMap, table));
		}
	}
	...
	private static class ClearProblemLinkType extends CellType<ProblemReport> {
		...
		public String format(Object obj) {
			...
			return String.format("<a href='/op?table=%s&action=clearProblem&redir=%s&resource=%s&ptype=%s'>clear this problem</a>", p.getTableName(), currentPage(req), encode(p.getResource()), encode(p.getProblemType().name()));
		}
