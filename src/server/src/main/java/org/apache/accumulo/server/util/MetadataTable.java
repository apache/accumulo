...
public class MetadataTable extends org.apache.accumulo.core.util.MetadataTable {
	...
	throws AccumuloSecurityException
	{
		while (true){
			try {
				return ThriftScanner.getBatchFromServer(credentials, startRow, Constants.ROOT_TABLET_EXTENT, HdfsZooInstance.getInstance().getRootTabletLocation(), results, columns, skipStartRow, size, Constants.NO_AUTHS, true, AccumuloConfiguration.getSystemConfiguration());
	...
	public static void addNewTablet(KeyExtent extent, String path, TServerInstance location, Map<String, DataFileValue> datafileSizes, AuthInfo credentials, String time, ZooLock zooLock)
	{
		Mutation m = extent.getPrevRowUpdateMutation(); // 
	...
	public static void replaceDatafiles(KeyExtent extent, Set<String> datafilesToDelete, Set<String> scanFiles, String path, DataFileValue size, AuthInfo credentials, 
            String address, 
            TServerInstance lastLocation, 
            ZooLock zooLock)
	{
	...
	private static KeyExtent fixSplit(Text table, Text metadataEntry, Text metadataPrevEndRow, Value oper, double splitRatio, TServerInstance tserver, AuthInfo credentials, String time, ZooLock lock)
	...
	public static void deleteTable(String table, AuthInfo credentials, ZooLock lock)
	throws AccumuloException
	{
