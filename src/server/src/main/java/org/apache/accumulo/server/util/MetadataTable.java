...
public class MetadataTable extends org.apache.accumulo.core.util.MetadataTable {
     ...
     * 
     */
    public static void updateTabletDataFile(KeyExtent extent, String path, DataFileValue dfv, String time, AuthInfo credentials, String address,
            ZooLock zooLock, Set<String> unusedWalLogs, TServerInstance lastLocation) {
        ...
        while (true) {
            try {
                return ThriftScanner.getBatchFromServer(credentials, startRow, Constants.ROOT_TABLET_EXTENT, HdfsZooInstance.getInstance()
                        .getRootTabletLocation(), results, columns, skipStartRow, size, Constants.NO_AUTHS, true, AccumuloConfiguration
    ...
    public static void addLogEntries(AuthInfo credentials, List<LogEntry> entries, ZooLock zooLock) {
        ...
        if (entry.extent.equals(Constants.ROOT_TABLET_EXTENT)) {
            ...
            while (true) {
                try {
                    if (ZooLock.isLockHeld(ZooSession.getSession(), zooLock.getLockID())) ZooUtil.putPersistentData(root + "/" + entry.filename,
    ...
    public static void removeUnusedWALEntries(KeyExtent extent, List<LogEntry> logEntries, ZooLock zooLock) {
        for (LogEntry entry : logEntries) {
            if (entry.extent.equals(Constants.ROOT_TABLET_EXTENT)) {
                ...
                while (true) {
                    try {
                        if (ZooLock.isLockHeld(ZooSession.getSession(), zooLock.getLockID())) ZooUtil.recursiveDelete(root + "/" + entry.filename,
