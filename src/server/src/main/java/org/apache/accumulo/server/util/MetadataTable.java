...
public class MetadataTable extends org.apache.accumulo.core.util.MetadataTable {
	...
	/**
	 * new data file update function
	 * adds one data file to a tablet's list
	 * 
	...
	public static boolean getBatchFromRootTablet(AuthInfo credentials, Text startRow, SortedMap<Key, Value> results, SortedSet<Column> columns, boolean skipStartRow, int size)
	throws AccumuloSecurityException
	{
		while (true){
			try {
				return ThriftScanner.getBatchFromServer(credentials, startRow, Constants.ROOT_TABLET_EXTENT, HdfsZooInstance.getInstance().getRootTabletLocation(), results, columns, skipStartRow, size, Constants.NO_AUTHS, true, ServerConfiguration.getSystemConfiguration());
	...
	public static void addNewTablet(KeyExtent extent, String path, TServerInstance location, Map<String, DataFileValue> datafileSizes, AuthInfo credentials, String time, long lastFlushID, long lastCompactID, ZooLock zooLock)
	{
		Mutation m = extent.getPrevRowUpdateMutation(); 
		...
		for (Entry<String, DataFileValue> entry : datafileSizes.entrySet()) {
			m.put(Constants.METADATA_DATAFILE_COLUMN_FAMILY, new Text(entry.getKey()), 
	...
	public static void splitTablet(KeyExtent extent, Text oldPrevEndRow, double splitRatio, AuthInfo credentials, ZooLock zooLock)
	{
		...
		ColumnFQ.put(m, Constants.METADATA_SPLIT_RATIO_COLUMN, 
	...
	public static void finishSplit(Text metadataEntry, Map<String, DataFileValue> datafileSizes, List<String> highDatafilesToRemove, AuthInfo credentials, ZooLock zooLock) {
	...
	public static void finishSplit(KeyExtent extent, Map<String, DataFileValue> datafileSizes, List<String> highDatafilesToRemove, AuthInfo credentials, ZooLock zooLock) {
		finishSplit(extent.getMetadataEntry(), datafileSizes, highDatafilesToRemove, credentials, zooLock);
	}
	
	public static void replaceDatafiles(KeyExtent extent, Set<String> datafilesToDelete, Set<String> scanFiles, String path, Long compactionId, DataFileValue size, AuthInfo credentials, 
            String address, 
            TServerInstance lastLocation, 
            ZooLock zooLock)
	{
	...
	private static KeyExtent fixSplit(Text table, Text metadataEntry, Text metadataPrevEndRow, Value oper, double splitRatio, TServerInstance tserver, AuthInfo credentials, String time, long initFlushID, long initCompactID, ZooLock lock)
	throws AccumuloException
	{
		...
		splitDatafiles(table, metadataPrevEndRow, splitRatio, new HashMap<String, FileUtil.FileInfo>(), origDatafileSizes, lowDatafileSizes, highDatafileSizes, highDatafilesToRemove);
	...
	public static void splitDatafiles(Text table, Text midRow, double splitRatio,
			...
			SortedMap<String, DataFileValue> lowDatafileSizes,
			SortedMap<String, DataFileValue> highDatafileSizes,
	...
	public static void deleteTable(String tableId, boolean insertDeletes, AuthInfo credentials, ZooLock lock)
	throws AccumuloException
	{
    ...
    public static void addLogEntries(AuthInfo credentials, List<LogEntry> entries, ZooLock zooLock) {
	    ...
	    if (entry.extent.equals(Constants.ROOT_TABLET_EXTENT)) {
	        ...
	        m.put(Constants.METADATA_LOG_COLUMN_FAMILY, 
	              new Text(entry.server + "/" + entry.filename),
    ...
    public static Pair<List<LogEntry>, SortedMap<String, DataFileValue>> getFileAndLogEntries(AuthInfo credentials, KeyExtent extent) throws KeeperException, InterruptedException, IOException{
    ...
    private static Scanner getTabletLogScanner(AuthInfo credentials,
                                               KeyExtent extent) {
        Scanner scanner = new ScannerImpl(HdfsZooInstance.getInstance(), credentials, Constants.METADATA_TABLE_ID, Constants.NO_AUTHS);
        scanner.fetchColumnFamily(Constants.METADATA_LOG_COLUMN_FAMILY);
    ...
    static class LogEntryIterator implements Iterator<LogEntry> {
        ...
        LogEntryIterator(AuthInfo creds) throws IOException, KeeperException, InterruptedException {
            ...
            try {
                Scanner scanner = HdfsZooInstance.getInstance().getConnector(creds.user, creds.password).createScanner(Constants.METADATA_TABLE_NAME, Constants.NO_AUTHS);
                scanner.fetchColumnFamily(Constants.METADATA_LOG_COLUMN_FAMILY);
    ...
    public static void removeUnusedWALEntries(KeyExtent extent, List<LogEntry> logEntries, ZooLock zooLock) {
        for (LogEntry entry: logEntries) {
            if (entry.extent.equals(Constants.ROOT_TABLET_EXTENT)) {
                ...
                Mutation m = new Mutation(entry.extent.getMetadataEntry());
                m.putDelete(Constants.METADATA_LOG_COLUMN_FAMILY, 
                        new Text(entry.server + "/" + entry.filename));
    ...
    static int checkClone(String srcTableId, String tableId, Connector conn, BatchWriter bw) throws TableNotFoundException, MutationsRejectedException {
		TabletIterator srcIter = new TabletIterator(createCloneScanner(srcTableId, conn), new KeyExtent(new Text(srcTableId), null, null).toMetadataRange(), true, true);
