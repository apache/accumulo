...
public class CollectTabletStats {
	public static void main(String[] args) throws Exception {
		...
		if(index != 7){
			System.err.println("USAGE : "+CollectTabletStats.class+" [-i <iterations>] [-t <num threads>] [-l|-f] [-c <column fams>] <instance> <zookeepers> <user> <pass> <table> <auths> <batch size>");
			return;
		}
	...
	private static abstract class Test implements Runnable {
		...
		public void run(){
			...
			System.out.printf("\t\ttablet: "+ke.getUUID()+"  thread: "+Thread.currentThread().getId()+" count: %,d cells  time: %6.2f  rate: %,6.2f cells/sec\n", count, time, count/time);
	...
	private static List<KeyExtent> findTablets(boolean selectLocalTablets, String user, String pass, String table, ZooKeeperInstance zki, Map<KeyExtent, String> locations) throws Exception {
	...
	private static List<String> getTabletFiles(String user, String pass,
			ZooKeeperInstance zki, String tableId, KeyExtent ke) {
		...
		for (Entry<Key, Value> entry : es) {
			if(entry.getKey().compareRow(ke.getMetadataEntry()) == 0)
			{
				...
				{
					files.add(Constants.getTablesDir()+"/"+tableId+entry.getKey().getColumnQualifier());
				}
			}
		}
		return files;
	...
	private static SortedKeyValueIterator<Key, Value> createScanIterator(KeyExtent ke, Collection<SortedKeyValueIterator<Key,Value>> mapfiles, Authorizations authorizations, byte[] defaultLabels, HashSet<Column> columnSet, List<IterInfo> ssiList, Map<String, Map<String, String>> ssio, boolean useTableIterators) throws IOException {
			...
			if(useTableIterators)
				return IteratorUtil.loadIterators(IteratorScope.scan, systemIter, ke, AccumuloConfiguration.getTableConfiguration(HdfsZooInstance.getInstance().getInstanceID(), ke.getTableId().toString()), ssiList, ssio, null);
			return systemIter;
		}
	...
	private static void calcTabletStats(Connector conn, String table,
			String[] auths, int batchSize,
