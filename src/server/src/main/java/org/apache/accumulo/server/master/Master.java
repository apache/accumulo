...
/**
 * The Master is responsible for assigning and balancing tablets and loggers to
 * tablet servers.
 * 
...
public class Master implements LiveTServerSet.Listener, LoggerWatcher, TableObserver, CurrentState, JobComplete {
    ...
    public static void breakInFormatting() {
        // useless function that causes blocks above and below to be formatted
        // separately in eclipse
    }
        ...
        if (!conf.getBoolean(Property.TABLE_BLOCKCACHE_ENABLED)) {
            try {
                ...
                for (Entry<Key, Value> entry : scanner) {
                    log.error(String.format("Unable to upgrade: extent %s has log entry %s", 
                                            entry.getKey().getRow(), entry.getValue()));
                    fail = true;
                }
                if (fail)
                ...
                zset(Property.TABLE_LOCALITY_GROUP_PREFIX.getKey()+"tablet",
                        String.format("%s,%s", 
                                Constants.METADATA_TABLET_COLUMN_FAMILY.toString(),
                                Constants.METADATA_CURRENT_LOCATION_COLUMN_FAMILY.toString()));
                zset(Property.TABLE_LOCALITY_GROUP_PREFIX.getKey()+"server",
                        String.format("%s,%s,%s,%s", 
                                Constants.METADATA_DATAFILE_COLUMN_FAMILY.toString(),
                                Constants.METADATA_LOG_COLUMN_FAMILY.toString(),
                                Constants.METADATA_SERVER_COLUMN_FAMILY.toString(),
    ...
    private void checkTableName(String tableName, TableOperation operation) throws ThriftTableOperationException {
        if (!tableName.matches(Constants.VALID_TABLE_NAME_REGEX)) {
            String why = "Table names must only contain word characters (letters, digits, and underscores): "
                    + tableName;
            log.warn(why);
            throw new ThriftTableOperationException(null, tableName, operation, TableOperationExceptionType.OTHER, why);
        }
    ...
    private class MasterClientServiceHandler implements MasterClientService.Iface {
		...
		@Override
		public void waitForFlush(TInfo tinfo, AuthInfo c, String tableId, ByteBuffer startRow, ByteBuffer endRow, long flushID,  long maxLoops) throws ThriftSecurityException, ThriftTableOperationException, TException {
			verify(c, tableId, TableOperation.FLUSH, check(c, tableId, TablePermission.WRITE) || check(c, tableId, TablePermission.ALTER_TABLE));
        ...
        private void alterTableProperty(AuthInfo c, String tableName, String property, String value,
        ...
        @Override
        public void removeTableProperty(TInfo info, AuthInfo credentials, String tableName, String property)
                throws ThriftSecurityException, ThriftTableOperationException, TException {
            alterTableProperty(credentials, tableName, property, null, TableOperation.REMOVE_PROPERTY);
        }

        @Override
        public void setTableProperty(TInfo info, AuthInfo credentials, String tableName, String property, String value)
                throws ThriftSecurityException, ThriftTableOperationException, TException {
            alterTableProperty(credentials, tableName, property, value, TableOperation.SET_PROPERTY);
        }

        @Override
        public void shutdown(TInfo info, AuthInfo c, boolean stopTabletServers) throws ThriftSecurityException,
                TException {
        ...
        @Override
        public void shutdownTabletServer(TInfo info, AuthInfo c, String tabletServer, boolean force) throws ThriftSecurityException,
                TException {
            verify(c, check(c, SystemPermission.SYSTEM));
        ...
        @Override
        public void reportTabletStatus(TInfo info, 
                                       AuthInfo credentials, 
                                       String serverName, 
                                       TabletLoadState status,
                                       TKeyExtent ttablet) throws TException {
            ...
            case UNLOAD_FAILURE_NOT_SERVING:
                if (log.isTraceEnabled()) {
                    log.trace(serverName + " reports unload failed: not serving tablet, could be a split: "
                            + tablet);
                }
                break;
            case CHOPPED:
        ...
        @Override
        public void setMasterGoalState(TInfo info, AuthInfo c, MasterGoalState state) throws ThriftSecurityException,
                TException {
            verify(c, check(c, SystemPermission.SYSTEM));
		...
		@Override
		public void setSystemProperty(TInfo info, AuthInfo credentials, String property,
				String value) throws ThriftSecurityException, TException {			
        ...
        private void authenticate(AuthInfo credentials)
		...
		@Override
		public void executeTableOperation(TInfo tinfo, AuthInfo c, long opid, org.apache.accumulo.core.master.thrift.TableOperation op, List<ByteBuffer> arguments, Map<String, String> options, boolean autoCleanup) throws ThriftSecurityException, ThriftTableOperationException, TException {
			...
			case ONLINE:
			{
	            ...
	            verify(c, tableId, TableOperation.ONLINE, check(c, SystemPermission.SYSTEM) || check(c, SystemPermission.ALTER_TABLE) || check(c, tableId, TablePermission.ALTER_TABLE) );
			...
			case OFFLINE:
			{
	            ...
	            verify(c, tableId, TableOperation.OFFLINE, check(c, SystemPermission.SYSTEM) || check (c, SystemPermission.ALTER_TABLE) || check(c, tableId, TablePermission.ALTER_TABLE) );
			...
			case MERGE:
			{
	            ...
	            verify(c, tableId, TableOperation.MERGE, check(c, SystemPermission.SYSTEM) || check(c, SystemPermission.ALTER_TABLE) || check(c, tableId, TablePermission.ALTER_TABLE) );
			...
			case COMPACT:
			{
	            ...
	            verify(c, tableId, TableOperation.COMPACT, check(c, tableId, TablePermission.WRITE) || check(c, tableId, TablePermission.ALTER_TABLE) || check(c, SystemPermission.ALTER_TABLE));
	...
	public void setMergeState(MergeInfo info, MergeState state) throws IOException, KeeperException, InterruptedException {
        synchronized (mergeLock) {
            String path =  
                ZooUtil.getRoot(instance.getInstanceID()) +  
                Constants.ZTABLES + "/" +  
                info.getRange().getTableId().toString() + "/merge"; 
            info.setState(state);
            if (state.equals(MergeState.NONE)) {
                ...
                ZooReaderWriter.getInstance().putPersistentData(path,  
                        out.getData(),  
    ...
    private void setMasterGoalState(MasterGoalState state) {
        try {
            ZooReaderWriter.getInstance().putPersistentData(ZooUtil.getRoot(instance) + Constants.ZMASTER_GOAL_STATE,
                                      state.name().getBytes(), NodeExistsPolicy.OVERWRITE);
        } catch (Exception ex) {
    ...
    private class TabletGroupWatcher extends Daemon {
        ...
        private void deleteTablets(MergeInfo info) throws AccumuloException {
            ...
            try {
                ...
                Range deleteRange = new Range(KeyExtent.getMetadataEntry(range.getTableId(), start), false, 
                                              KeyExtent.getMetadataEntry(range.getTableId(), range.getEndRow()), true);
                ...
                if (followingTablet != null) {
                    ...
                    MetadataTable.addTablet(
                            new KeyExtent(range.getTableId(), null, range.getPrevEndRow()),
        ...
        private void flushChanges(SortedMap<TServerInstance, TabletServerStatus> currentTServers,
                                  ...
                                  Map<KeyExtent, TServerInstance> unassigned) throws DistributedStoreException,
                TException {
            ...
            if (!currentTServers.isEmpty()) {
                ...
                tabletBalancer.getAssignments(Collections.unmodifiableSortedMap(currentTServers),
    ...
    public void run() throws IOException, InterruptedException, KeeperException {
        ...
        final TabletStateStore stores[] = { 
                ...
                new RootTabletStateStore(this),
                new MetaDataStateStore(this) 
                };
        ...
        try {
            fate = new Fate<Master>(this, new org.apache.accumulo.server.fate.ZooStore<Master>(ZooUtil.getRoot(instance) + Constants.ZFATE, ZooReaderWriter.getInstance()), 4);
        ...
        clientService = TServerUtils.startServer(
                Property.MASTER_CLIENTPORT, 
                processor,
                "Master",
                "Master Client Service Handler", 
                null,
                Property.MASTER_MINTHREADS,
                Property.MASTER_THREADCHECK).server;
