...
public class Master implements Listener, NewLoggerWatcher, TableObserver, CurrentState {
    ...
    private void checkTableDoesNotExist(String tableName, TableOperation operation)
    ...
    private class MasterClientServiceHandler extends ClientServiceHandler implements MasterClientService.Iface {
        ...
        @Override
        public void createTable(TInfo info, AuthInfo c, String tableName, List<byte[]> splitPoints,
                                Map<String, String> aggregators, TimeType timeType) throws ThriftSecurityException,
            ...
            try {
                if (splitPoints.isEmpty()) {
                    ...
                    MetadataTable.addTablet(extent, Constants.DEFAULT_TABLET_LOCATION,
                                                    SecurityConstants.systemCredentials,
                                                    TabletTime.getTimeID(timeType), masterLock);
                    ...
                    for (Text splitpointText : splitPointsText) {
                        ...
                        MetadataTable.addTablet(local_newExtent, datafiles,
                                                        SecurityConstants.systemCredentials,
                                                        TabletTime.getTimeID(timeType), masterLock);
            ...
            for (TablePermission permission : TablePermission.values()) {
                try {
                    authenticator.grantTablePermission(SecurityConstants.systemCredentials, c.user, tableId,
                                                       permission);
                } catch (AccumuloSecurityException e) {
                    log.error(e.getMessage(), e);
                    throw e.asThriftException();
        ...
        @Override
        public void deleteTable(TInfo info, AuthInfo c, String tableName) throws ThriftSecurityException,
                ThriftTableOperationException, TException {
            final String tableId = checkTableId(tableName, TableOperation.DELETE);
            checkNotMetadataTable(tableName, TableOperation.DELETE);
            verify(c, check(c, SystemPermission.DROP_TABLE) || 
        ...
        @Override
        public void flushTable(TInfo info, AuthInfo c, String tableName) throws ThriftSecurityException,
                ThriftTableOperationException, TException {
        ...
        private void updateTableState(AuthInfo c, String tableName, TableOperation operation, TableState state)
        ...
        @Override
        public void offlineTable(TInfo info, AuthInfo credentials, String tableName) throws ThriftSecurityException,
                ThriftTableOperationException, TException {
        ...
        @Override
        public void onlineTable(TInfo info, AuthInfo credentials, String tableName) throws ThriftSecurityException,
                ThriftTableOperationException, TException {
            updateTableState(credentials, tableName, TableOperation.ONLINE, TableState.ONLINE);
        }

        @Override
        public void renameTable(TInfo info, AuthInfo c, String oldTableName, String newTableName)
        ...
        @Override
        public void shutdownTabletServer(TInfo info, AuthInfo c, String tabletServer) throws ThriftSecurityException,
                TException {
            verify(c, check(c, SystemPermission.SYSTEM));
    ...
    private void setMasterGoalState(MasterGoalState state) {
        try {
            ZooUtil.putPersistentData(ZooUtil.getRoot(instance) + Constants.ZMASTER_GOAL_STATE,
    ...
    private class TabletGroupWatcher extends Daemon {
        ...
        private void flushChanges(SortedMap<TServerInstance, TabletServerStatus> currentTServers,
            ...
            if (!assignedToDeadServers.isEmpty()) {
                ...
                log.debug(assignedToDeadServers.size() + " assigned to dead servers: " + assignedToDeadServers.subList(0, maxServersToShow) + "...");                List<TabletLocationState> recovered = new ArrayList<TabletLocationState>();
    ...
    public void run() throws IOException, InterruptedException, KeeperException {
        ...
        clientService = TServerUtils.startServer(Property.MASTER_CLIENTPORT, processor, "Master",
                                                 "Master Client Service Handler", false).server;
    ...
    private void getMasterLock(final String zMasterLoc) throws KeeperException, InterruptedException {
        ...
        final String masterClientAddress = hostname + ":"
                + AccumuloConfiguration.getSystemConfiguration().getPort(Property.MASTER_CLIENTPORT);
