...
public class FileManager {
	 ...
	 * @param fs
	 * @param maxOpen
	 * @param dataCache : underlying file can and should be able to handle a null cache
	 * @param indexCache : underlying file can and should be able to handle a null cache
	 */
	...
	private List<FileSKVIterator> reserveReaders(Text table, Collection<String> files, boolean continueOnFailure) throws IOException{
		...
		for (String file : filesToOpen) {
			try{
				...
				FileSKVIterator reader = FileOperations.getInstance().openReader(file, false, fs, conf, AccumuloConfiguration.getTableConfiguration(HdfsZooInstance.getInstance().getInstanceID(), table.toString()), dataCache, indexCache);
	...
	public class ScanFileManager {
		...
		ScanFileManager(KeyExtent tablet){
			...
			continueOnFailure = AccumuloConfiguration.getTableConfiguration(HdfsZooInstance.getInstance().getInstanceID(), tablet.getTableId().toString()).getBoolean(Property.TABLE_FAILURES_IGNORE);
		...
		private List<FileSKVIterator> openFiles(Collection<String> files) throws TooManyFilesException, IOException {
			...
			if(tabletReservedReaders.size() + files.size() >= maxOpen){
				throw new TooManyFilesException("Request to open files would exceed max open files reservedReaders.size()="+tabletReservedReaders.size()+" files.size()="+files.size()+" maxOpen="+maxOpen+" tablet = "+tablet);
			}
