	...
	private List<FileSKVIterator> reserveReaders(Text table, Collection<String> files, boolean continueOnFailure) throws IOException{
		...
		for (String file : filesToOpen) {
			try{
				...
				FileSKVIterator reader = FileOperations.getInstance().openReader(file, false, fs, conf, ServerConfiguration.getTableConfiguration(table.toString()), dataCache, indexCache);
