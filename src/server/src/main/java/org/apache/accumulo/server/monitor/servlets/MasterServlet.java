...
public class MasterServlet extends BasicServlet {
	...
	private void doLogEventBanner(StringBuilder sb) {
		if (LogService.getInstance().getEvents().size() > 0) {
			...
			banner(sb, error > 0 ? "error" : "warning", String.format("<a href='/log'>Log Events: %d Error%s, %d Warning%s, %d Total</a>", error, error == 1 ? "" : "s", warning, warning == 1 ? "" : "s", total));
		}
	}
	...
	private void doMasterStatus(HttpServletRequest req, StringBuilder sb) throws IOException {
		...
		if (Monitor.getMmi() != null) {
			...
			{
			    sb.append(
			            "<span class='warning'>Master State: " + 
			            Monitor.getMmi().state.name() +
			            " Goal: " +
			            Monitor.getMmi().goalState.name() +
			            "</span>\n");
			}
			if (Monitor.getMmi().serversShuttingDown != null && Monitor.getMmi().serversShuttingDown.size() > 0) {
			    sb.append(
                        "<span class='warning'>Servers being stopped: " + 
                        StringUtil.join(Monitor.getMmi().serversShuttingDown, ", ") +
                        "</span>\n");
			}

			int guessHighLoad = ManagementFactory.getOperatingSystemMXBean().getAvailableProcessors();
			List<String> slaves = org.apache.accumulo.server.util.getSlaves.main();
			Table masterStatus = new Table("masterStatus", "Master&nbsp;Status");
			masterStatus.addSortableColumn("#&nbsp;Online<br />Tablet&nbsp;Servers", new NumberType<Integer>((int) (slaves.size() * 0.8), slaves.size(), (int) (slaves.size() * 0.6), slaves.size()), 
			                               "Number of tablet servers currently available");
			masterStatus.addSortableColumn("#&nbsp;Total<br />Tablet&nbsp;Servers", new NumberType<Integer>(), 
			...
			masterStatus.addSortableColumn("#&nbsp;Tablets", new NumberType<Integer>(0, 300000, 2, 400000), null);
	...
	private void doRecoveryList(HttpServletRequest req, StringBuilder sb) {
		...
		boolean sortingUsesMapReduce = AccumuloConfiguration.getSystemConfiguration().getBoolean(Property.MASTER_RECOVERY_SORT_MAPREDUCE);
		if (mmi != null) {
