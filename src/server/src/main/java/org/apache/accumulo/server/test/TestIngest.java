	...
	public static void main(String[] args) {
	    ...
	    try {
			...
			if(ingestArgs.outputToMapFile){
				...
				writer = FileOperations.getInstance().openWriter(ingestArgs.outputFile+"."+MyMapFile.EXTENSION, fs, conf, AccumuloConfiguration.getDefaultConfiguration());
				writer.startDefaultLocalityGroup();
			} else if (ingestArgs.outputToRFile) {
                Configuration conf = CachedConfiguration.getInstance();
                FileSystem fs = FileSystem.get(conf);
                writer = FileOperations.getInstance().openWriter(ingestArgs.outputFile+"."+RFile.EXTENSION, fs, conf, AccumuloConfiguration.getDefaultConfiguration());
                writer.startDefaultLocalityGroup();
			} else {
