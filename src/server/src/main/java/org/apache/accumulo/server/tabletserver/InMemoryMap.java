...
public class InMemoryMap {
		...
		public void mutate(List<Mutation> mutations, int mutationCount) {
			for(Mutation m : mutations){
				for (ColumnUpdate cvp : m.getUpdates()) {
					Key newKey = new MemKey(m.getRow(), cvp.getColumnFamily(), cvp.getColumnQualifier(), cvp.getColumnVisibility(), cvp.getTimestamp(), cvp.isDeleted(), false, mutationCount);
	...
	public DataFileValue minorCompact(Configuration conf, FileSystem fs, String dirname, KeyExtent extent)
	{
		if(nextMutationCount.get() - 1 != mutationCount.get())
			throw new IllegalStateException("Memory map in unexpected state : nextMutationCount = "+nextMutationCount.get()+" mutationCount = "+mutationCount.get());
		...
		do{
			...
			try {
				...
				log.debug(String.format("MinC %,d recs in | %,d recs out | %,d recs/sec | %6.3f secs | %,d bytes ",map.size(), entriesCompacted, (int)(map.size()/((t2 - t1)/1000.0)), (t2 - t1)/1000.0, estimatedSizeInBytes()));

				if(reportedProblem){
					ProblemReports.getInstance().deleteProblemReport(extent.getTableId().toString(), ProblemType.FILE_WRITE, dirname);
				}
				
				return new DataFileValue(
						fileOps.getFileSize(dirname, fs, conf, AccumuloConfiguration.getTableConfiguration(HdfsZooInstance.getInstance().getInstanceID(), extent.getTableId().toString())),
						entriesCompacted
				);
			} catch (IOException e) {
