...
public class OfflineMetadataScanner extends ScannerOptions implements Scanner {
  ...
  private List<SortedKeyValueIterator<Key,Value>> openMapFiles(Collection<String> files, FileSystem fs, Configuration conf) throws IOException {
    ...
    for (String file : files) {
      FileSKVIterator reader = FileOperations.getInstance().openReader(file, true, fs, conf, AccumuloConfiguration.getSystemConfiguration());
      readers.add(reader);
    }
    return readers;
  }
  ...
  public OfflineMetadataScanner() throws IOException {
    ...
    FileStatus[] rootFiles = fs.listStatus(new Path(Constants.getRootTabletDir()));
    ...
    while (ssi.hasTop()) {
      if (ssi.getTopKey().compareColumnFamily(Constants.METADATA_DATAFILE_COLUMN_FAMILY) == 0) {
        allFiles.add(Constants.getMetadataTableDir() + "/" + ssi.getTopKey().getColumnQualifier().toString());
      } else if (ssi.getTopKey().compareColumnFamily(Constants.METADATA_LOG_COLUMN_FAMILY) == 0) {
        walogs++;
      }
      ssi.next();
    }
