...
public class TServersServlet extends BasicServlet {
	...
	protected void pageBody(HttpServletRequest req, HttpServletResponse response, StringBuilder sb) throws Exception {
		...
		try {
			TabletClientService.Iface client = ThriftUtil.getClient(new TabletClientService.Client.Factory(),
			                                                         address, AccumuloConfiguration.getSystemConfiguration());
			try {
	...
	private void doAllTimeTable(HttpServletRequest req, StringBuilder sb, TabletStats total, TabletStats historical, double majorQueueStdDev, double minorQueueStdDev, double totalElapsedForAll, double splitStdDev, double majorStdDev, double minorStdDev) {
		...
		opHistoryDetails.addRow("Split", historical.split.num, historical.split.fail, null, null, historical.split.num != 0 ? (long) (historical.split.elapsed / historical.split.num) : null, splitStdDev, historical.split.elapsed);
		opHistoryDetails.addRow("Major&nbsp;Compaction", total.major.num, total.major.fail, total.major.num != 0 ? (long) (total.major.queueTime / total.major.num) : null, majorQueueStdDev, total.major.num != 0 ? (long) (total.major.elapsed / total.major.num) : null, majorStdDev, total.major.elapsed);
		opHistoryDetails.addRow("Minor&nbsp;Compaction", total.minor.num, total.minor.fail, total.minor.num != 0 ? (long) (total.minor.queueTime / total.minor.num) : null, minorQueueStdDev, total.minor.num != 0 ? (long) (total.minor.elapsed / total.minor.num) : null, minorStdDev, total.minor.elapsed);
