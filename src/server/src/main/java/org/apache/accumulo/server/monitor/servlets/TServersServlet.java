...
public class TServersServlet extends BasicServlet {
    ...
    private void doAllTimeTable(HttpServletRequest req, StringBuilder sb, TabletStats total, TabletStats historical, double majorQueueStdDev,
        ...
        opHistoryDetails.addRow("Major&nbsp;Compaction", total.major.num, total.major.fail,
                total.major.num != 0 ? (long) (total.major.queueTime / total.major.num) : null, majorQueueStdDev,
                total.major.num != 0 ? (long) (total.major.elapsed / total.major.num) : null, majorStdDev, total.major.elapsed);
        opHistoryDetails.addRow("Minor&nbsp;Compaction", total.minor.num, total.minor.fail,
                total.minor.num != 0 ? (long) (total.minor.queueTime / total.minor.num) : null, minorQueueStdDev,
                total.minor.num != 0 ? (long) (total.minor.elapsed / total.minor.num) : null, minorStdDev, total.minor.elapsed);
    ...
    static void doTserverList(HttpServletRequest req, StringBuilder sb, List<TabletServerStatus> tservers, Table tServerList) {
        ...
        tServerList
                .addSortableColumn("Hosted&nbsp;Tablets", new NumberType<Integer>((int) avgHostedTablets / 4, Math.max(1, (int) avgHostedTablets * 2)), null);
