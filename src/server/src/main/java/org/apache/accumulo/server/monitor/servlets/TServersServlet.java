...
public class TServersServlet extends BasicServlet {
	...
	protected void pageBody(HttpServletRequest req, HttpServletResponse response, StringBuilder sb) throws Exception {
		...
		TabletStats total = new TabletStats(null, 
		                                    new ActionStats(),
		                                    new ActionStats(),
		                                    new ActionStats(),
		                                    0, 0, 0, 0);
	...
	private void doAllTimeTable(HttpServletRequest req, StringBuilder sb, TabletStats total, TabletStats historical, double majorQueueStdDev, double minorQueueStdDev, double totalElapsedForAll, double splitStdDev, double majorStdDev, double minorStdDev) {
		...
		opHistoryDetails.addRow("Split", historical.split.num, historical.split.fail, null, null, historical.split.num != 0 ? (historical.split.elapsed / historical.split.num) : null, splitStdDev, historical.split.elapsed);
		opHistoryDetails.addRow("Major&nbsp;Compaction", total.major.num, total.major.fail, total.major.num != 0 ? (total.major.queueTime / total.major.num) : null, majorQueueStdDev, total.major.num != 0 ? (total.major.elapsed / total.major.num) : null, majorStdDev, total.major.elapsed);
		opHistoryDetails.addRow("Minor&nbsp;Compaction", total.minor.num, total.minor.fail, total.minor.num != 0 ? (total.minor.queueTime / total.minor.num) : null, minorQueueStdDev, total.minor.num != 0 ? (total.minor.elapsed / total.minor.num) : null, minorStdDev, total.minor.elapsed);
	...
	/*
	 * omg there's so much undocumented stuff going on here.
	 * First, sumDev is a partial standard deviation computation.
	 * It is the (clue 1) sum of the squares of (clue 2) seconds of elapsed time.
	 */
        ...
        if (obit != null && !obit.isEmpty()) {
            ...
            for (DeadServer dead : obit)
                deadTServerList.addRow(TServerLinkType.displayName(dead.server), dead.lastStatus, dead.status, 
                        "<a href='/op?action=clearDeadServer&redir=" + currentPage(req) + "&server=" + encode(dead.server) + "'>clear</a>");
    ...
    static void doTserverList(HttpServletRequest req, StringBuilder sb, List<TabletServerStatus> tservers, String tableId, Table tServerList) {
		...
		tServerList.addSortableColumn("OS&nbsp;Load", new NumberType<Double>(0., guessHighLoad * 1., 0., guessHighLoad * 3.), "The Unix one minute load average. The average number of processes in the run queue over a one minute interval.");
