...
public class TServersServlet extends BasicServlet {
    ...
    protected void pageBody(HttpServletRequest req, HttpServletResponse response, StringBuilder sb) throws Exception {
        ...
        if (tserverAddress == null || tserverAddress.isEmpty() || tserverExists == false) {
            ...
            tServerList
                    .setSubCaption("Click on the <span style='color: #0000ff;'>server address</span> to view detailed performance statistics for that server.");
        ...
        try {
            TabletClientService.Iface client = ThriftUtil.getClient(new TabletClientService.Client.Factory(), address,
    ...
    private void doAllTimeTable(HttpServletRequest req, StringBuilder sb, TabletStats total, TabletStats historical, double majorQueueStdDev,
        ...
        opHistoryDetails.addRow("Major&nbsp;Compaction", total.major.num, total.major.fail, total.major.num != 0 ? (total.major.queueTime / total.major.num)
                : null, majorQueueStdDev, total.major.num != 0 ? (total.major.elapsed / total.major.num) : null, majorStdDev, total.major.elapsed);
        opHistoryDetails.addRow("Minor&nbsp;Compaction", total.minor.num, total.minor.fail, total.minor.num != 0 ? (total.minor.queueTime / total.minor.num)
                : null, minorQueueStdDev, total.minor.num != 0 ? (total.minor.elapsed / total.minor.num) : null, minorStdDev, total.minor.elapsed);
