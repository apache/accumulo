...
public class Compactor implements Callable<CompactionStats> {
	...
	Compactor(Configuration conf, FileSystem fs, Map<String, DataFileValue> files, InMemoryMap imm, String outputFile, boolean propogateDeletes, TableConfiguration acuTableConf, KeyExtent extent, CompactionEnv env){
		this.extent = extent;
		this.conf = conf;
		this.fs = fs;
	...
	public CompactionStats call() throws IOException, CompactionCanceledException {
		...
		try{
			...
			log.debug(String.format("Compaction %s %,d read | %,d written | %,6d entries/sec | %6.3f secs",
			        extent,
	...
	throws IOException,CompactionCanceledException {
		...
		try{
			...
			SortedKeyValueIterator<Key, Value> itr = iterEnv.getTopLevelIterator(IteratorUtil.loadIterators(env.getIteratorScope(), delIter, extent, acuTableConf, iterEnv));
