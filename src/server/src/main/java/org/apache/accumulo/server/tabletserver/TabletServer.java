...
public class TabletServer extends AbstractMetricsImpl implements TabletServerMBean {
        ...
        public synchronized List<ActiveScan> getActiveScans() {
            ...
            for (Entry<Long,Session> entry : sessions.entrySet()) {
                ...
                if (session instanceof ScanSession) {
                    ...
                    activeScans.add(new ActiveScan(ss.client, ss.user, ss.extent.getTableId().toString(), ct - ss.startTime, ct - ss.lastAccessTime,
                            ScanType.SINGLE, state, ss.extent.toThrift(), Translator.translate(ss.columnSet, Translator.CT), ss.ssiList, ss.ssio));
                    ...
                    activeScans.add(new ActiveScan(mss.client, mss.user, mss.threadPoolExtent.getTableId().toString(), ct - mss.startTime, ct
                            - mss.lastAccessTime, ScanType.BATCH, state, mss.threadPoolExtent.toThrift(), Translator.translate(mss.columnSet, Translator.CT),
                            mss.ssiList, mss.ssio));
                }
            }
    ...
    private class ThriftClientHandler extends ClientServiceHandler implements TabletClientService.Iface {
                ...
                try {
                    ...
                    while (iter.hasNext() && bytesAdded < maxResultsSize && (System.currentTimeMillis() - startTime) < maxScanTime) {
                        ...
                        try {
                            ...
                            lookupResult = tablet.lookup(entry.getValue(), session.columnSet, session.auths, results, maxResultsSize - bytesAdded,
                                    session.ssiList, session.ssio, interruptFlag);
                    ...
                    Map<TKeyExtent,List<TRange>> retFailures = Translator.translate(failures, Translator.KET, new Translator.ListTranslator<Range,TRange>(
                            Translator.RT));
                    ...
                    addResult(new MultiScanResult(retResults, retFailures, retFullScans, retPartScan, retPartNextKey, partNextKeyInclusive,
                            session.queries.size() != 0));
                } catch (IterationInterruptedException iie) {
                    if (!isCancelled()) {
                        log.warn("Iteration interrupted, when scan not cancelled", iie);
                        addResult(iie);
        ...
        public void closeScan(TInfo tinfo, long scanID) {
            ...
            if (ss != null) {
                ...
                log.debug(String.format("ScanSess tid %s %s %,d entries in %.2f secs, nbTimes = [%s] ", TServerUtils.clientAddress.get(), ss.extent
                        .getTableId().toString(), ss.entriesReturned, (t2 - ss.startTime) / 1000.0, ss.nbTimes.toString()));
        ...
        public void closeMultiScan(TInfo tinfo, long scanID) throws NoSuchScanIDException {
            ...
            long t2 = System.currentTimeMillis();
            log.debug(String.format("MultiScanSess %s %,d entries in %.2f secs (lookup_time:%.2f secs tablets:%,d ranges:%,d) ",
                    TServerUtils.clientAddress.get(), session.numEntries, (t2 - session.startTime) / 1000.0, session.totalLookupTime / 1000.0,
                    session.numTablets, session.numRanges));
        }
                ...
                synchronized (openingTablets) {
                    synchronized (onlineTablets) {
                        ...
                        if (unopenedOverlapping.size() > 0 || openingOverlapping.size() > 0 || onlineOverlapping.size() > 0) {
                            log.debug("Tablet " + extent + " overlaps previously assigned " + unopenedOverlapping + " " + openingOverlapping + " "
                                    + onlineOverlapping);
                            return;
                        }
        ...
        public List<ActiveScan> getActiveScans(TInfo tinfo, AuthInfo credentials) throws ThriftSecurityException, TException {
            ...
            try {
                if (!authenticator.hasSystemPermission(credentials, credentials.user, SystemPermission.SYSTEM)) throw new ThriftSecurityException(
                        credentials.user, SecurityErrorCode.PERMISSION_DENIED);
            } catch (AccumuloSecurityException e) {
                throw e.asThriftException();
            }
            ...
            synchronized (unopenedTablets) {
                synchronized (openingTablets) {
                    synchronized (onlineTablets) {
                        ...
                        if (!unopenedTablets.contains(extent) || unopenedOverlapping.size() != 1 || openingOverlapping.size() > 0
    ...
    private int startServer(Property portHint, TProcessor processor, String threadName) throws UnknownHostException {
        ServerPort sp = TServerUtils.startServer(portHint, processor, this.getClass().getSimpleName(), threadName, AccumuloConfiguration
                .getSystemConfiguration().getBoolean(Property.TSERV_PORTSEARCH));
