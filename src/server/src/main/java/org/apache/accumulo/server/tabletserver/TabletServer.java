...
public class TabletServer extends AbstractMetricsImpl implements TabletServerMBean {
	...
	private static class SessionManager {
		...
		/**
		 * while a session is reserved, it cannot be canceled
		 * or removed
		 * @param sessionId
		 */
				...
				if(session instanceof ScanSession){
					...
					activeScans.add(
						new ActiveScan(
						    ss.client,
							ss.user,
							ss.extent.getTableId().toString(), 
							ct - ss.startTime,
							ct - ss.lastAccessTime,
							ScanType.SINGLE,
							state,
							ss.extent.toThrift(), 
							Translator.translate(ss.columnSet, Translator.CT), 
							ss.ssiList, 
							ss.ssio));
					...
					activeScans.add(
							new ActiveScan(
								mss.client,
								mss.user,
								mss.threadPoolExtent.getTableId().toString(), 
								ct - mss.startTime,
								ct - mss.lastAccessTime,
								ScanType.BATCH,
								state,
								mss.threadPoolExtent.toThrift(), 
								Translator.translate(mss.columnSet, Translator.CT), 
								mss.ssiList, 
								mss.ssio));
				}
			}
    ...
    /**
     * This little class keeps track of writes in progress
     * and allows readers to wait for writes that started
     * before the read.  It assumes that the operation ids
	...
	private class ThriftClientHandler extends ClientServiceHandler implements TabletClientService.Iface {
		...
		@Override
		public InitialScan startScan(TInfo tinfo, AuthInfo credentials, TKeyExtent textent, TRange range, List<TColumn> columns, int batchSize, List<IterInfo> ssiList, Map<String, Map<String, String>> ssio, List<byte[]> authorizations, boolean waitForWrites, boolean isolated) throws NotServingTabletException, ThriftSecurityException, org.apache.accumulo.core.tabletserver.thrift.TooManyFilesException {
		...
		@Override
		public InitialMultiScan startMultiScan(TInfo tinfo, AuthInfo credentials, Map<TKeyExtent, List<TRange>> tbatch, List<TColumn> tcolumns, List<IterInfo> ssiList, Map<String, Map<String, String>> ssio, List<byte[]> authorizations, boolean waitForWrites)
		throws ThriftSecurityException
		{
		...
		public void setUpdateTablet(TInfo tinfo, long updateID, TKeyExtent tkeyExtent)
		{
			...
			if(us != null){
				...
				try {
				    ...
				    if (sameTable || authenticator.hasTablePermission(SecurityConstants.systemCredentials, us.credentials.user, keyExtent.getTableId().toString(), TablePermission.WRITE))
					{
						long t2 = System.currentTimeMillis();
		...
		@Override
		public void splitTablet(TInfo tinfo, AuthInfo credentials, TKeyExtent tkeyExtent, byte[] splitPoint) throws NotServingTabletException, ThriftSecurityException {
        ...
        @Override
        public TabletServerStatus getTabletServerStatus(TInfo tinfo, AuthInfo credentials) throws ThriftSecurityException,
                TException {
            return getStats();
        }
			...
			while(!majorCompactorDisabled){
			    try {
			        ...
			        while(iter.hasNext() && !majorCompactorDisabled) { // bail early now if we're shutting down
			            ...
			            int maxLogEntriesPerTablet = 
			                AccumuloConfiguration.getTableConfiguration(HdfsZooInstance.getInstance().getInstanceID(), tablet.getExtent().getTableId().toString()).getCount(Property.TABLE_MINC_LOGS_MAX);
	...
	private TreeMap<KeyExtent, SplitInfo> splitTablet(Tablet tablet, byte[] splitPoint) throws IOException {
		...
		newTablets[0] = new Tablet(TabletServer.this, new Text(first.getValue().dir), first.getKey(), resourceManager.createTabletResourceManager(), first.getValue().datafiles, first.getValue().time);
		
		Entry<KeyExtent, SplitInfo> last = tabletInfo.lastEntry();
		newTablets[1] = new Tablet(TabletServer.this, new Text(last.getValue().dir), last.getKey(), resourceManager.createTabletResourceManager(), last.getValue().datafiles, last.getValue().time);
        ...
        public void run() {
			...
			for (Entry<KeyExtent, Text> entry : tabletsInRange.entrySet()) {
				...
				try {
				    ...
				    ProblemReports.getInstance().report(new ProblemReport(table, 
				                                                          TABLET_LOAD, 
				                                                          extentToOpen.getUUID().toString(), 
    ...
    private int startServer(Property portHint, 
                            TProcessor processor,
                            String threadName) throws UnknownHostException {
        ServerPort sp = TServerUtils.startServer(portHint, 
                                                 processor, 
                                                 this.getClass().getSimpleName(), 
                                                 threadName, 
                                                 AccumuloConfiguration.getSystemConfiguration().getBoolean(Property.TSERV_PORTSEARCH));
	...
	public static SortedMap<KeyExtent, Text> verifyTabletInformation(KeyExtent extent, 
    ...
    public void minorCompactionStarted(CommitSession tablet, int lastUpdateSequence,
    ...
    public long getEntries() {
    	...
    	{
			long result = 0;
			for (Tablet tablet : Collections
					.unmodifiableCollection(onlineTablets.values())) {
