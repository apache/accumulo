...
public class Tablet {
    ...
    class DatafileManager {
        ...
        public void importMapFiles(long tid, Map<String,DataFileValue> pathsString, boolean setTime) throws IOException {
            ...
            for (Path tpath : paths.keySet()) {
                ...
                else if (!bulkDir.equals(tpath.getParent().toString())) throw new IllegalArgumentException("bulk files in different dirs " + bulkDir + " "
                        + tpath);
             ...
             * MetadataTable.replaceDatafiles(extent, abs2rel(paths.keySet()), new HashSet<String>(), "junk", null, zero,
             * SecurityConstants.getSystemCredentials(), tabletServer.getClientAddressString(), lastLocation, tabletServer.getLock(), false); throw new
        ...
        void unreserveMergingMinorCompactionFile(Path file) {
            if ((file == null && mergingMinorCompactionFile != null) || (file != null && mergingMinorCompactionFile == null)
                    || (file != null && mergingMinorCompactionFile != null && !file.equals(mergingMinorCompactionFile))) throw new IllegalStateException(
                    "Disagreement " + file + " " + mergingMinorCompactionFile);
            
            mergingMinorCompactionFile = null;
        }
        
        void bringMinorCompactionOnline(String tmpDatafile, String newDatafile, String absMergeFile, DataFileValue dfv, CommitSession commitSession,
                long flushId) {
        ...
        void bringMinorCompactionOnline(Path tmpDatafile, Path newDatafile, Path absMergeFile, DataFileValue dfv, CommitSession commitSession, long flushId) {
            ...
            if (absMergeFile != null) MetadataTable.addDeleteEntries(extent, Collections.singleton(abs2rel(absMergeFile)),
        ...
        void bringMajorCompactionOnline(Set<String> oldDatafiles, String tmpDatafile, String newDatafile, Long compactionId, DataFileValue dfv)
                throws IOException {
    ...
    public Tablet(TabletServer tabletServer, Text location, KeyExtent extent, TabletResourceManager trm, SortedMap<String,DataFileValue> datafiles,
            String time, long initFlushID, long initCompactID) throws IOException {
    ...
    private static long lookupFlushID(KeyExtent extent, SortedMap<Key,Value> tabletsKeyValues) {
        ...
        for (Entry<Key,Value> entry : tabletsKeyValues.entrySet()) {
            Key key = entry.getKey();
            if (key.getRow().equals(row) && Constants.METADATA_FLUSH_COLUMN.equals(key.getColumnFamily(), key.getColumnQualifier())) return Long
    ...
    private static long lookupCompactID(KeyExtent extent, SortedMap<Key,Value> tabletsKeyValues) {
        ...
        for (Entry<Key,Value> entry : tabletsKeyValues.entrySet()) {
            Key key = entry.getKey();
            if (key.getRow().equals(row) && Constants.METADATA_COMPACT_COLUMN.equals(key.getColumnFamily(), key.getColumnQualifier())) return Long
                    .parseLong(entry.getValue().toString());
        }
    ...
    private Tablet(TabletServer tabletServer, Text location, KeyExtent extent, TabletResourceManager trm, Configuration conf, FileSystem fs,
            SortedMap<Key,Value> tabletsKeyValues) throws IOException {
        this(tabletServer, location, extent, trm, conf, fs, lookupLogEntries(extent, tabletsKeyValues),
                lookupDatafiles(location, fs, extent, tabletsKeyValues), lookupTime(extent, tabletsKeyValues), lookupLastServer(extent, tabletsKeyValues),
                lookupScanFiles(extent, tabletsKeyValues), lookupFlushID(extent, tabletsKeyValues), lookupCompactID(extent, tabletsKeyValues));
    }
    ...
    private Tablet(final TabletServer tabletServer, final Text location, final KeyExtent extent, final TabletResourceManager trm, final Configuration conf,
            final FileSystem fs, final List<LogEntry> logEntries, final SortedMap<String,DataFileValue> datafiles, String time,
            final TServerInstance lastLocation, Set<String> scanFiles, long initFlushID, long initCompactID) throws IOException {
    ...
    class ScanDataSource implements DataSource {
        ...
        ScanDataSource(Authorizations authorizations, byte[] defaultLabels, HashSet<Column> columnSet, List<IterInfo> ssiList,
    ...
    boolean initiateMinorCompaction(long flushId) {
        ...
        try {
            synchronized (this) {
                ...
                if (closing || closed || majorCompactionWaitingToStart || tabletMemory.memoryReservedForMinC()
                        || tabletMemory.getMemTable().getNumEntries() == 0 || updatingFlushID) {
     ...
     * 
     * we just need to make sure major compactions aren't occurring if we have the major compactor thread decide who needs splitting we can avoid
     * synchronization issues with major compactions
     * 
     */
