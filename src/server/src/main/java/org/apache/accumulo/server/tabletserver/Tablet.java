...
public class Tablet {
    ...
    class DatafileManager {
        ...
        public void importMapFiles(Map<String,DataFileValue> paths) throws IOException {
            ...
            for (String tpath : paths.keySet()) {
                ...
                else if (!bulkDir.equals(tmpPath.getParent().toString())) throw new IllegalArgumentException("bulk files in different dirs " + bulkDir + " "
                        + tmpPath);
        ...
        synchronized void bringMajorCompactionOnline(Set<String> oldDatafiles, String tmpDatafile, String newDatafile, DataFileValue dfv) throws IOException {
            ...
            synchronized (Tablet.this) {
                ...
                if (extent.equals(Constants.ROOT_TABLET_EXTENT)) {
                    ...
                    if (!fs.rename(new Path(tmpDatafile), new Path(newDatafile))) log.warn("Rename of " + tmpDatafile + " to " + newDatafile
                            + " returned false");
    ...
    private Tablet(final TabletServer tabletServer, final Text location, final KeyExtent extent, final TabletResourceManager trm, final Configuration conf,
            ...
            final TServerInstance lastLocation, Set<String> scanFiles) throws IOException {
    ...
    private class MinorCompactionTask implements Runnable {
        ...
        public void run() {
            ...
            try {
                ...
                this.stats = minorCompact(conf, fs, tabletMemory.getMinCMemTable(), newMapfileLocation + "_tmp", newMapfileLocation, true, queued,
                        commitSession);
                span.stop();
            } catch (Throwable t) {
    ...
    private List<SortedKeyValueIterator<Key,Value>> openMapDataFiles(String lgName, Configuration conf, FileSystem fs, Set<String> mapFiles, KeyExtent extent,
        ...
        for (String mapFile : mapFiles) {
            try {
                ...
                AccumuloConfiguration tableConf = AccumuloConfiguration.getTableConfiguration(HdfsZooInstance.getInstance().getInstanceID(), extent
                        .getTableId().toString());
        ...
        try {
            ...
            if (mfw.supportsLocalityGroups()) {
                for (Entry<String,Set<ByteSequence>> entry : lGroups.entrySet()) {
                    compactLocalityGroup(entry.getKey(), entry.getValue(), true, conf, fs, filesToCompact, propogateDeletes, extent, compactTmpName, mfw,
                            majCStats);
    ...
    public TreeMap<KeyExtent,SplitInfo> split(byte[] sp) throws IOException {
        ...
        Map<String,org.apache.accumulo.core.file.FileUtil.FileInfo> firstAndLastRowsAbs = FileUtil.tryToGetFirstAndLastRows(tabletResources
                .getCopyOfMapFilePaths());
        ...
        synchronized (this) {
            ...
            if (sp == null) splitPoint = findSplitRow();
                ...
                splitPoint = new SplitRowSpec(FileUtil.estimatePercentageLTE(extent.getPrevEndRow(), extent.getEndRow(),
                        tabletResources.getCopyOfMapFilePaths(), tsp), tsp);
            }
