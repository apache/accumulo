...
public class Tablet {
	...
	class DatafileManager {
		...
		public void importMapFiles(long tid, Map<String, DataFileValue> pathsString, boolean setTime) throws IOException {
			...
			/*if(!fs.exists(new Path(bulkDir, Constants.BULK_LOAD_IN_PROGRESS))){
				DataFileValue zero = new DataFileValue(0, 0);
				MetadataTable.replaceDatafiles(extent, abs2rel(paths.keySet()), new HashSet<String>(), "junk", null, zero, SecurityConstants.getSystemCredentials(), tabletServer.getClientAddressString(), lastLocation, tabletServer.getLock(), false);
		...
		void bringMinorCompactionOnline(String tmpDatafile, String newDatafile, String absMergeFile, DataFileValue dfv, CommitSession commitSession, long flushId){
			bringMinorCompactionOnline(new Path(tmpDatafile), new Path(newDatafile), absMergeFile == null ? null : new Path(absMergeFile), dfv, commitSession, flushId);
		}
		...
		void bringMinorCompactionOnline(Path tmpDatafile, Path newDatafile, Path absMergeFile, DataFileValue dfv, CommitSession commitSession, long flushId){
			...
			do {
				try{
					if(dfv.getNumEntries() == 0){
						...
						fs.rename(tmpDatafile, newDatafile);
					}
					break;
			...
			try{
				...
				synchronized (timeLock) {
					...
					MetadataTable.updateTabletDataFile(extent, abs2rel(newDatafile), abs2rel(absMergeFile), dfv, time, creds, abs2rel(filesInUseByScans), tabletServer.getClientAddressString(), tabletServer.getLock(), unusedWalLogs, lastLocation, flushId);
				}
		...
		void bringMajorCompactionOnline(Set<Path> oldDatafiles, Path tmpDatafile, Path newDatafile, Long compactionId, DataFileValue dfv) throws IOException{
			...
			if (!extent.equals(Constants.ROOT_TABLET_EXTENT)) {
			    ...
			    MetadataTable.replaceDatafiles(extent,abs2rel(oldDatafiles), abs2rel(filesInUseByScans), abs2rel(newDatafile), compactionId, dfv, SecurityConstants.getSystemCredentials(), tabletServer.getClientAddressString(), lastLocation, tabletServer.getLock());
			    removeFilesAfterScan(filesInUseByScans);
			}
    ...
    public Tablet(TabletServer tabletServer, Text location, KeyExtent extent, TabletResourceManager trm, SortedMap<Key, Value> tabletsKeyValues) throws IOException {
        this(tabletServer, location, extent, trm, CachedConfiguration.getInstance(), tabletsKeyValues);
        splitCreationTime = 0;
    }
    
    public Tablet(TabletServer tabletServer, Text location, KeyExtent extent, TabletResourceManager trm, SortedMap<String, DataFileValue> datafiles, String time, long initFlushID, long initCompactID) throws IOException {
	...
	private Tablet(TabletServer tabletServer, Text location, KeyExtent extent, TabletResourceManager trm, Configuration conf, SortedMap<Key, Value> tabletsKeyValues) throws IOException {
		this(tabletServer, location, extent, trm, conf, TraceFileSystem.wrap(FileUtil.getFileSystem(conf, ServerConfiguration.getSiteConfiguration())), tabletsKeyValues);
	}
	
	static private final List<LogEntry> EMPTY = Collections.emptyList();
	
	private Tablet(TabletServer tabletServer, Text location, KeyExtent extent, TabletResourceManager trm, Configuration conf, SortedMap<String, DataFileValue> datafiles, String time, long initFlushID, long initCompactID) throws IOException {
		this(tabletServer, location, extent, trm, conf, TraceFileSystem.wrap(FileUtil.getFileSystem(conf, ServerConfiguration.getSiteConfiguration())), EMPTY, datafiles, time, null, new HashSet<String>(), initFlushID, initCompactID);
	}
	...
	private static SortedMap<String, DataFileValue> lookupDatafiles(Text locText, FileSystem fs, KeyExtent extent, SortedMap<Key, Value> tabletsKeyValues) throws IOException {
		...
		if(extent.equals(Constants.ROOT_TABLET_EXTENT)) { // the meta0 tablet
			...
			if(extent.getTableId().toString().equals(Constants.METADATA_TABLE_ID)) {
				...
				if(tabletsKeyValues != null && tabletsKeyValues.size() > 0){
					...
					ScannerImpl mdScanner = new ScannerImpl(HdfsZooInstance.getInstance(), SecurityConstants.getSystemCredentials(), Constants.METADATA_TABLE_ID, Constants.NO_AUTHS);
	...
	private Tablet(TabletServer tabletServer, Text location, KeyExtent extent, TabletResourceManager trm, Configuration conf, FileSystem fs, SortedMap<Key, Value> tabletsKeyValues) throws IOException {
		this(tabletServer, location, extent, trm, conf, fs, 
		     lookupLogEntries(extent, tabletsKeyValues),
		     lookupDatafiles(location, fs, extent, tabletsKeyValues), 
		     lookupTime(extent, tabletsKeyValues), 
		     lookupLastServer(extent, tabletsKeyValues),
		     lookupScanFiles(extent, tabletsKeyValues),
	 ...
	 * the Metadata table if we already know the files we need - as at split time
	 */
	private Tablet(final TabletServer tabletServer, 
				   final Text location, 
	               ...
	               final SortedMap<String, DataFileValue> datafiles, 
	               String time, 
	...
	throws IOException {
		...
		if(extent.equals(Constants.ROOT_TABLET_EXTENT)){
			...
			for(String path : datafiles.keySet()) {
				...
				FileSKVIterator reader = FileOperations.getInstance().openReader(this.location+"/"+filename, true, fs, fs.getConf(), ServerConfiguration.getTableConfiguration(Constants.METADATA_TABLE_ID));
				long maxTime = -1;
				try{
		...
		if (!logEntries.isEmpty()) {
		    ...
		    log.info("Write-Ahead Log recovery complete for " + this.extent + " (" 
		             + count[0] + " mutations applied, "+tabletMemory.getNumEntries()+" entries created)");
		}
	...
	Scanner createScanner(Range range, int num, HashSet<Column> columns, Authorizations authorizations, List<IterInfo> ssiList, Map<String, Map<String, String>> ssio, boolean isolated, AtomicBoolean interruptFlag){
		//do a test to see if this range falls within the tablet, if it does not
		//then clip will throw an exception
	...
	static class ScanOptions {
		...
		ScanOptions(int num, Authorizations authorizations, byte[] defaultLabels, HashSet<Column> columnSet, List<IterInfo> ssiList, Map<String, Map<String, String>> ssio, AtomicBoolean interruptFlag, boolean isolated){
			this.num = num;
	...
	class ScanDataSource implements DataSource {
		...
		private SortedKeyValueIterator<Key, Value> createIterator() throws IOException{
			...
			return iterEnv.getTopLevelIterator(IteratorUtil.loadIterators(IteratorScope.scan, visFilter, extent, acuTableConf, options.ssiList, options.ssio, iterEnv));
		}
	...
	private DataFileValue minorCompact(Configuration conf, FileSystem fs, InMemoryMap memTable, String tmpDatafile, String newDatafile, String mergeFile, boolean hasQueueTime, long queued, CommitSession commitSession, long flushId){
		...
		try{
			...
			datafileManager.bringMinorCompactionOnline(tmpDatafile, newDatafile, mergeFile, new DataFileValue(stats.getFileSize(), stats.getEntriesWritten()), commitSession, flushId);
			span.stop();
	...
	private class MinorCompactionTask implements Runnable {
		...
		public void run() {
		    ...
		    try {
		    	...
		    	this.stats = minorCompact(conf, fs, tabletMemory.getMinCMemTable(), newMapfileLocation+"_tmp", newMapfileLocation, mergeFile, true, queued, commitSession, flushId);
		    	span.stop();
		    } catch (Throwable t) {
	...
	boolean initiateMinorCompaction(long flushId){
		...
		try{
			synchronized(this){
			    ...
			    if(closing || closed || majorCompactionWaitingToStart || tabletMemory.memoryReservedForMinC() || tabletMemory.getMemTable().getNumEntries() == 0 || updatingFlushID){
	...
	private void closeConsistencyCheck() {
		...
		try {
			Pair<List<LogEntry>, SortedMap<String, DataFileValue>> fileLog = 
				MetadataTable.getFileAndLogEntries(SecurityConstants.getSystemCredentials(), extent);
			...
			if(extent.equals(Constants.ROOT_TABLET_EXTENT)){
				if(!fileLog.getSecond().keySet().equals(datafileManager.getDatafileSizesRel().keySet())){
					String msg = "Data file in !METADATA differ from in memory data "+extent+"  "+fileLog.getSecond().keySet()+"  "+datafileManager.getDatafileSizesRel().keySet();
					log.error(msg);
					throw new RuntimeException(msg);
				}
			}else{
				if(!fileLog.getSecond().equals(datafileManager.getDatafileSizesRel())){
					String msg = "Data file in !METADATA differ from in memory data "+extent+"  "+fileLog.getSecond()+"  "+datafileManager.getDatafileSizesRel();
					log.error(msg);
					throw new RuntimeException(msg);
				}
			}
	...
	private SplitRowSpec findSplitRow(Collection<String> files){
		...
		if(location.toString().equals(ServerConstants.getRootTabletDir()) ||
    ...
    private static int longestCommonLength(Text text, Text beforeMid) {
        int common = 0;
        while (common < text.getLength() && 
                common < beforeMid.getLength() && 
			...
			while(filesToCompact.size() > 0) {
				...
				try {
					...
					Compactor compactor = new Compactor(conf, fs, copy, null, compactTmpName, 
							filesToCompact.size() == 0 ? propogateDeletes : true, //always propagate deletes, unless last batch
									acuTableConf,
									extent,
									cenv);
	...
	public TreeMap<KeyExtent, SplitInfo> split(byte[] sp) throws IOException {
		...
		Map<String, org.apache.accumulo.core.file.FileUtil.FileInfo> firstAndLastRowsAbs = 
			FileUtil.tryToGetFirstAndLastRows(fs, ServerConfiguration.getSystemConfiguration(), datafileManager.getFiles());
		
		//convert absolute paths to relative paths
		Map<String, org.apache.accumulo.core.file.FileUtil.FileInfo> firstAndLastRows = 
			new HashMap<String, org.apache.accumulo.core.file.FileUtil.FileInfo>();
		...
		synchronized (this) {
			...
			if(sp == null)
				...
				splitPoint = new SplitRowSpec(FileUtil.estimatePercentageLTE(fs, ServerConfiguration.getSystemConfiguration(), extent.getPrevEndRow(), extent.getEndRow(), datafileManager.getFiles(), tsp), tsp);
			}
			...
			MetadataTable.splitDatafiles(extent.getTableId(), midRow, splitRatio, firstAndLastRows, datafileManager.getDatafileSizesRel(), lowDatafileSizes, highDatafileSizes, highDatafilesToRemove);
			...
			MetadataTable.addNewTablet(low, lowDirectory,  tabletServer.getTabletSession(), lowDatafileSizes, SecurityConstants.getSystemCredentials(), time, lastFlushID, lastCompactID, tabletServer.getLock());
	...
	public long totalQueries() { return this.queryCount; }
	public long totalIngest() { return this.ingestCount; }
    ...
    public synchronized int getLogCount() { return currentLogs.size(); }
