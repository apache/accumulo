...
public class Tablet {
	...
	private String getMapFilename(int gen, int seq){
		String extension = FileOperations.getNewFileExtension(AccumuloConfiguration.getTableConfiguration(HdfsZooInstance.getInstance().getInstanceID(), extent.getTableId().toString()));
	...
	class DatafileManager {
		...
		public void importMapFiles(Map<String, DataFileValue> paths) throws IOException {
			...
			if(fs.globStatus(new Path(bulkDir+"/processing_proc_*")).length == 0){
				...
				MetadataTable.replaceDatafiles(extent, relSizes.keySet(), new HashSet<String>(), "junk", zero, SecurityConstants.systemCredentials, tabletServer.getClientAddressString(), lastLocation, tabletServer.getLock(), false);
		...
		void bringMinorCompactionOnline(String tmpDatafile, String newDatafile, DataFileValue dfv, CommitSession commitSession){
			...
			try{
				...
				MetadataTable.updateTabletDataFile(extent, relativePath, dfv, time, creds, tabletServer.getClientAddressString(), tabletServer.getLock(), unusedWalLogs, lastLocation);
			}finally{
		...
		synchronized void bringMajorCompactionOnline(Set<String> oldDatafiles, String tmpDatafile, String newDatafile, DataFileValue dfv) throws IOException{
			...
			if (!extent.equals(Constants.ROOT_TABLET_EXTENT)) {
			    ...
			    MetadataTable.replaceDatafiles(extent,datafilesToDelete, abs2rel(filesInUseByScans), shortPath, dfv, SecurityConstants.systemCredentials, tabletServer.getClientAddressString(), lastLocation, tabletServer.getLock());
    ...
    public Tablet(TabletServer tabletServer, Text location, KeyExtent extent, TabletResourceManager trm, SortedMap<String, DataFileValue> datafiles, String time) throws IOException {
	...
	private Tablet(TabletServer tabletServer, Text location, KeyExtent extent, TabletResourceManager trm, Configuration conf, SortedMap<String, DataFileValue> datafiles, String time) throws IOException {
		...
		if(extent.equals(Constants.ROOT_TABLET_EXTENT)) { // the meta0 tablet
			...
			if(extent.getTableId().toString().equals(Constants.METADATA_TABLE_ID)) {
				...
				if(tabletsKeyValues != null && tabletsKeyValues.size() > 0){
					...
					ScannerImpl mdScanner = new ScannerImpl(HdfsZooInstance.getInstance(), SecurityConstants.systemCredentials, Constants.METADATA_TABLE_ID, Constants.NO_AUTHS);
	...
	throws IOException {
		...
		if(extent.equals(Constants.ROOT_TABLET_EXTENT)){
			...
			for(String path : datafiles.keySet()) {
				...
				FileSKVIterator reader = FileOperations.getInstance().openReader(this.location+"/"+filename, true, fs, fs.getConf(), AccumuloConfiguration.getTableConfiguration(HdfsZooInstance.getInstance().getInstanceID(), Constants.METADATA_TABLE_ID));
				long maxTime = -1;
				try{
	...
	private DataFileValue minorCompact(Configuration conf, FileSystem fs, InMemoryMap memTable, String tmpDatafile, String newDatafile, boolean hasQueueTime, long queued, CommitSession commitSession){
	...
	private void closeConsistencyCheck() {
		...
		try {
			Pair<List<LogEntry>, SortedMap<String, DataFileValue>> fileLog = 
				MetadataTable.getFileAndLogEntries(SecurityConstants.systemCredentials, extent);
			...
			if(extent.equals(Constants.ROOT_TABLET_EXTENT)){
				if(!fileLog.getSecond().keySet().equals(datafileManager.datafileSizes.keySet())){
					String msg = "Data file in !METADATA differ from in memory data "+extent+"  "+fileLog.getSecond().keySet()+"  "+datafileManager.datafileSizes.keySet();
					log.error(msg);
					throw new RuntimeException(msg);
				}
			}else{
	...
	private SplitRowSpec findSplitRow(){
		...
		if(location.toString().equals(Constants.getRootTabletDir()) ||
	...
	private List<SortedKeyValueIterator<Key,Value>> openMapDataFiles(String lgName,  Configuration conf, FileSystem fs, 
			Set<String> mapFiles, KeyExtent extent, ArrayList<FileSKVIterator> readers) throws IOException{
		...
		for(String mapFile : mapFiles ) {
			try{
				...
				AccumuloConfiguration tableConf = AccumuloConfiguration.getTableConfiguration(HdfsZooInstance.getInstance().getInstanceID(), extent.getTableId().toString());
	...
	private void compactLocalityGroup(String lgName, Set<ByteSequence> columnFamilies, boolean inclusive,
			...
			Set<String> filesToCompact, boolean propogateDeletes,
			KeyExtent extent, String compactTmpName, FileSKVWriter mfw,
			MajorCompactionStats majCStats) throws IOException,
			MajorCompactionCanceledException {
	...
	private MajorCompactionStats compactFiles(Configuration conf, 
	                                          FileSystem fs,
	                                          Set <String> filesToCompact, 
	                                          boolean propogateDeletes,
	                                          KeyExtent extent, 
	                                          String compactTmpName) 
	throws IOException, MajorCompactionCanceledException{
		...
		try{
			...
			log.debug(String.format("MajC %,d read | %,d written | %,6d entries/sec | %6.3f secs",
	...
	private MajorCompactionStats _majorCompact(boolean idleCompaction) throws IOException, MajorCompactionCanceledException {
		...
		while(filesToCompact.size() > 0) {
			...
			try {
			    ...
			    long size = FileOperations.getInstance().getFileSize(compactTmpName, fs, conf, AccumuloConfiguration.getTableConfiguration(HdfsZooInstance.getInstance().getInstanceID(), extent.getTableId().toString()));
	...
	public TreeMap<KeyExtent, SplitInfo> split(byte[] sp) throws IOException {
		...
		Map<String, org.apache.accumulo.core.file.FileUtil.FileInfo> firstAndLastRowsAbs = 
			FileUtil.tryToGetFirstAndLastRows(tabletResources.getCopyOfMapFilePaths());
		...
		synchronized (this) {
			...
			if(sp == null)
				...
				splitPoint = new SplitRowSpec(FileUtil.estimatePercentageLTE(extent.getPrevEndRow(), extent.getEndRow(), tabletResources.getCopyOfMapFilePaths(), tsp), tsp);
			}
			...
			MetadataTable.splitDatafiles(extent.getTableId(), midRow, splitRatio, firstAndLastRows, datafileManager.datafileSizes, lowDatafileSizes, highDatafileSizes, highDatafilesToRemove);
			...
			MetadataTable.addNewTablet(low, lowDirectory,  tabletServer.getTabletSession(), lowDatafileSizes, SecurityConstants.systemCredentials, time, tabletServer.getLock());
