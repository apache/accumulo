...
/**
 * A map only job that reads a table created by continuous ingest 
 * and creates doubly linked list.  This map reduce job test ability 
 * of a map only job to read and write to accumulo at the same time.  
 * This map reduce job mutates the table in such a way that it should 
 * not create any undefined nodes.
 *
 */
public class ContinuousMoru  extends Configured implements Tool
{
        ...
        public void map(Key key, Value data, Context context) throws IOException, InterruptedException
        {
        	...
        	if(WritableComparator.compareBytes(iiId, 0, iiId.length, data.get(), 0, iiId.length) != 0){
                ...
                if(offset > 0){
             	   ...
             	   Mutation m = ContinuousIngest.genMutation(rowLong, random.nextInt(max_cf), random.nextInt(max_cq), iiId, count++, key.getRowData().toArray(), random, true);
    ...
    public int run(String[] args) throws IOException, InterruptedException, ClassNotFoundException
    {
        ...
        // set up ranges
        try {
			AccumuloInputFormat.setRanges(job, new ZooKeeperInstance(instance, zookeepers).getConnector(user, pass.getBytes()).tableOperations().splitRangeByTablets(table, new Range(), Integer.parseInt(maxMaps)));
    ...
    /**
     * 
     * @param args instanceName zookeepers username password table columns outputpath
     * @throws Exception
     */
