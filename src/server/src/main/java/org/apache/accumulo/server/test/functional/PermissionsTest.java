...
public class PermissionsTest
{
	...
	public static class SystemPermissionsTest extends FunctionalTest
	{
		...
		@Override
		public Map<String, String> getInitialConfig() { return Collections.emptyMap(); }
		@Override
		public List<TableSetup> getTablesToCreate() { return Collections.emptyList(); }
		...
		private static void testMissingSystemPermission(Connector root_conn, Connector test_user_conn, SystemPermission perm)
		throws AccumuloException, TableExistsException, AccumuloSecurityException, TableNotFoundException
		{
			...
			case ALTER_TABLE:
				...
				try {
					...
					if (e.getErrorCode() != SecurityErrorCode.PERMISSION_DENIED
							|| AccumuloConfiguration.getTableConfiguration(HdfsZooInstance.getInstance().getInstanceID(), tableId).get(Property.TABLE_BLOOM_ERRORRATE).equals("003.14159%"))
						throw e;
				}
		...
		private static void testGrantedSystemPermission(Connector root_conn, Connector test_user_conn, SystemPermission perm)
		throws AccumuloException, AccumuloSecurityException, TableNotFoundException, TableExistsException
		{
			String tableName, tableId, user, password = "password";
			...
			case ALTER_TABLE:
				...
				tableId = Tables.getNameToIdMap(root_conn.getInstance()).get(tableName);
				test_user_conn.tableOperations().setProperty(tableName, Property.TABLE_BLOOM_ERRORRATE.getKey(), "003.14159%");
				if (!AccumuloConfiguration.getTableConfiguration(HdfsZooInstance.getInstance().getInstanceID(), tableId).get(Property.TABLE_BLOOM_ERRORRATE).equals("003.14159%"))
		...
		private static void verifyHasOnlyTheseSystemPermissions(Connector root_conn, String user, SystemPermission...perms)
		...
		private static void verifyHasNoSystemPermissions(Connector root_conn, String user, SystemPermission...perms)
		throws AccumuloException, AccumuloSecurityException
		{
	...
	public static class TablePermissionsTest extends FunctionalTest
	{
		...
		@Override
		public void run()
		throws AccumuloException, AccumuloSecurityException, TableExistsException, TableNotFoundException, MutationsRejectedException
		{
			// create the test user
		    getConnector().securityOperations().createUser(TEST_USER, TEST_PASS.getBytes(), Constants.NO_AUTHS);
			Connector test_user_conn = getInstance().getConnector(TEST_USER, TEST_PASS.getBytes());
			...
			verifyHasOnlyTheseTablePermissions(getConnector(), getConnector().whoami(), Constants.METADATA_TABLE_NAME, TablePermission.READ, TablePermission.ALTER_TABLE);
        ...
        private void createTestTable() throws AccumuloException, AccumuloSecurityException,
		...
		private static void testMissingTablePermission(Connector root_conn, Connector test_user_conn, TablePermission perm) 
		...
		private static void testGrantedTablePermission(Connector root_conn, Connector test_user_conn, TablePermission perm)
		...
		private static void verifyHasOnlyTheseTablePermissions(Connector root_conn, String user, String table, TablePermission...perms)
		throws AccumuloException, AccumuloSecurityException
		{
		...
		private static void verifyHasNoTablePermissions(Connector root_conn, String user, String table, TablePermission...perms)
		throws AccumuloException, AccumuloSecurityException
		{
