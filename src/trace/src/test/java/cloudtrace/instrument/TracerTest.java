...
import cloudtrace.instrument.receivers.SpanReceiver;
import cloudtrace.instrument.thrift.TraceWrap;
import cloudtrace.thrift.TInfo;
import cloudtrace.thrift.TestService;

public class TracerTest {
    static class SpanStruct {
        public SpanStruct(long traceId, long spanId, long parentId, long start, long stop, String description,
        ...
        public long millis() { return stop - start; }
    }
    ...
    static class TestReceiver implements SpanReceiver {
        ...
        @Override
        public void span(long traceId, long spanId, long parentId, long start, long stop, String description,
                         Map<String, String> data) {
    ...
    public void testThrift() throws Exception {
        ...
        transport.listen();
        final TServer tserver = new TThreadPoolServer(new TestService.Processor(TraceWrap.service(new Service())),
                                                      transport);
        Thread t = new Thread() {
            public void run() {
                tserver.serve();
