...
public class QueryUtil {
	...
	public static int getDepth(String path) {
		int numSlashes = 0;
		int index = -1;
		while ((index = path.indexOf("/",index+1)) >= 0) numSlashes++;
		return numSlashes;
	}
	...
	public static Text getReverseIndex(String path) {
		...
		int i = part.length()-1;
		for (byte b : part.getBytes()) rev[i--] = b;
	...
	public Iterable<Entry<Key,Value>> singleRestrictedWildCardSearch(String exp) throws Exception {
		...
		if (exp.startsWith("*")) {
			...
			exp = exp.substring(1);
			scanner.setRange(prefix(getReverseIndex(exp)));
		} else if (exp.endsWith("*")) {
			System.out.println("executing ending wildcard search for "+exp);
			exp = exp.substring(0,exp.length()-1);
			scanner.setRange(prefix(getForwardIndex(exp)));
	...
	public Iterable<Entry<Key,Value>> singleWildCardSearch(String exp) throws Exception {
		...
		if (firstPart.length()>=lastPart.length()) {
			...
			scanner.setRange(prefix(getForwardIndex(firstPart)));
		} else {
			System.out.println("executing middle wildcard search for "+regexString+" from entries ending with "+lastPart);
			scanner.setRange(prefix(getReverseIndex(lastPart)));
		}
		scanner.setColumnQualifierRegex(regexString);
		return scanner;
	}

	// code copied from 1.4 Range.followingPrefix(Text)
	...
	// code copied from 1.4 Range.prefix(Text)
