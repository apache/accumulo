	...
	public static class FileCountMapper extends Mapper<Key,Value,Key,Value> {
		...
		throws IOException, InterruptedException {
			...
			while ((slashIndex = lrf.lastIndexOf("/"))>=0) {
				...
				context.write(new Key(lrf), new Value(StringArraySummation.longArrayToStringBytes(
						parentCount==0 ? new Long[]{dirCount,fileCount,dirCount,fileCount} : new Long[]{0l,0l,dirCount,fileCount})));
				parentCount++;
			}
		}
	}
