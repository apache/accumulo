 ...
 * 
 * Params <numrows> <minkeylength> <maxkeylength> <minvaluelength> <maxvaluelength> <tablename> <instance> <zoohosts> <username> <password> [rows_per_split]
 * rows_per_split allows you specify how many rows you want each split to contain (and consequently each mapper to handle at one time)
 * 
 * 
 */
public class TeraSortIngest extends Configured implements Tool {
  ...
  static class RangeInputFormat extends InputFormat<LongWritable,NullWritable> {
    ...
    public List<InputSplit> getSplits(JobContext job) {
      ...
      int numSplits = job.getConfiguration().getInt(ROWSPERSPLIT, 1);
