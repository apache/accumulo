 ...
 * A sequence of bytes.
 */
abstract class ByteSequence implements Comparable<ByteSequence>, Serializable {
  ...
  public int compareTo(ByteSequence obs) {
    if (isBackedByArray() && obs.isBackedByArray()) {
      return Arrays.compare(getBackingArray(), offset(), offset() + length(), obs.getBackingArray(),
          obs.offset(), obs.offset() + obs.length());
