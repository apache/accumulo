...
public class BooleanLogicIterator implements SortedKeyValueIterator<Key, Value>, OptionDescriber {
    ...
    public BooleanLogicTreeNode transformTreeNode(TreeNode node) throws ParseException {
        ...
        if (node.getType().equals(ASTAndNode.class) || node.getType().equals(ASTOrNode.class)) {
            ...
            if (node.isLeaf()) {
                returnNode =
                        new BooleanLogicTreeNode(ParserTreeConstants.JJTJEXLSCRIPT);
                //NOTE: this should be cleaned up a bit.
                Multimap<String, QueryTerm> terms = node.getTerms();
                for (String fName : terms.keySet()) {
                    Collection<QueryTerm> values = terms.get(fName);
     ...
     * Specify the root or sub root of the tree you wish to view.
     *
    ...
    /*
     * Remember, the Key in the BooleanLogicTreeNode is different structurally
     * than the Key in its sub iterator because the key BooleanLogic needs to
     * return is an event key created from the index key (which is what the sub
     * iterators are looking at!)
     */
    ...
    /*
     * The incoming jump key has been formatted into the structure of an
     * index key, but the leaves are eventkeys
     */
