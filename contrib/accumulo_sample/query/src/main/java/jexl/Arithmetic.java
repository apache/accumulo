...
public class Arithmetic extends JexlArithmetic {
    ...
    /**
     * This method differs from the parent in that we are not calling
     * String.matches() because it does not match on a newline. Instead
	...
	/**
	 * This method differs from the parent class in that we are going
	 * to try and do a better job of coercing the types. As a last resort
	 * we will do a string comparison and try not to throw a NumberFormatException.
	 * The JexlArithmetic class performs coercion to a particular type if either
	 * the left or the right match a known type. We will look at the type of the 
	 * right operator and try to make the left of the same type.
	 */
	@Override
