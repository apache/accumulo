 ...
 */
public class NewMonitorSslIT extends ConfigurableMacBase {
  ...
  public void test() throws Exception {
    ...
    monitorLocation = monitorLocation.substring(0, monitorLocation.lastIndexOf(':') + 1)
        + NewMonitor.NEW_MONITOR_PORT + "/routes";
    URL url = new URL(monitorLocation);
    log.debug("Fetching web page {}", url);
    String result = FunctionalTestUtils.readWebPage(url).body();
    assertTrue(result.length() > 100);
    assertTrue(result.indexOf("/metrics") >= 0);
  }
