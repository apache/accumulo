...
public class ExternalCompactionWaitIT extends AccumuloClusterHarness {
  ...
  public void configureMiniCluster(MiniAccumuloConfigImpl cfg, Configuration coreSite) {
    ...
    cfg.setProperty(Property.COMPACTOR_MAX_JOB_WAIT_TIME, "5m");
    ...
    return Threads.createNonCriticalThread("metric-tailer", () -> {
      ...
      out: while (!stopCheckerThread.get()) {
        ...
        for (String s : statsDMetrics) {
          ...
          String process = metric.getTags().getOrDefault(MetricsInfo.PROCESS_NAME_TAG_KEY, "none");
          String rg = metric.getTags().getOrDefault(MetricsInfo.RESOURCE_GROUP_TAG_KEY, "none");
          if (process.equals("compactor") && rg.equalsIgnoreCase(GROUP1)
              && metric.getName().equals(Metric.SERVER_IDLE.getName())) {
            int value = Integer.parseInt(metric.getValue());
            log.debug("Found metric: {} {} with value: {}", metric.getName(), metric.getTags(),
                value);
            if (metric.getName().equals(Metric.SERVER_IDLE.getName())) {
              compactorIdle.set(value == 0 ? false : true);
            }
          }
        }
        UtilWaitThread.sleep(CHECKER_THREAD_SLEEP_MS);
      }
