  ...
  public void testClientConfiguration() throws Exception {
    ...
    try (AccumuloClient client = Accumulo.newClient().from(getClientProps()).build()) {
      ...
      try (Scanner scanner = client.createScanner(tableName, Authorizations.EMPTY)) {
        ...
        Wait.waitFor(() -> getCluster().getServerContext().getServerPaths().getScanServer(
            Optional.of(ScanServerSelector.DEFAULT_SCAN_SERVER_GROUP_NAME), Optional.empty()).size()
            > 0);
        ...
        Wait.waitFor(() -> getCluster().getServerContext().getServerPaths().getScanServer(
            ...
            == 1);
        Wait.waitFor(() -> getCluster().getServerContext().getServerPaths()
            .getScanServer(Optional.of("GROUP1"), Optional.empty()).size() == 1);
