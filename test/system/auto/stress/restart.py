...
import os
import signal
from TestUtils import TestUtilsMixin, ACCUMULO_HOME, ACCUMULO_DIR
...
class RestartMasterRecovery(RestartTest):
    ...
    def waitForRecovery(self):
        while 1:
            h = self.runOn(self.masterHost(), ['hadoop', 'fs', '-ls', ACCUMULO_DIR + "/recovery/*"])
            out, err = h.communicate()
            if out.find('recovery') >= 0:
                break
    ...
    def runTest(self):
        ...
        # once recovery starts, restart the master
