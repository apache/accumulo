...
class FinishCloneTable extends ManagerRepo {
  ...
  public Repo<Manager> call(FateId fateId, Manager environment) {
    ...
    if (cloneInfo.isKeepOffline()) {
      environment.getTableManager().transitionTableState(cloneInfo.getTableId(), cloneInfo.getNamespaceId(), TableState.OFFLINE,
          expectedCurrStates);
    } else {
      ...
      environment.getTableManager().transitionTableState(cloneInfo.getTableId(), cloneInfo.getNamespaceId(), ts,
          expectedCurrStates);
    }
