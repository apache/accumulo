...
class FateMetricValues {
  ...
  public static FateMetricValues getFromZooKeeper(final ServerContext context,
    ...
    try {
      ...
      List<FateTransaction> currFates = admin.getTransactionStatus(zooStore, null, null);
      ...
      for (FateTransaction tx : currFates) {

        String stateName = ReadOnlyTStore.TStatus.valueOf(tx.getStatus().name()).name();
        ReadOnlyTStore.TStatus tStatus = ReadOnlyTStore.TStatus.valueOf(tx.getStatus().name());
