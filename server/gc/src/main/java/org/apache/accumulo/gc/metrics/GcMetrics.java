...
public class GcMetrics extends Metrics {
  ...
  public GcMetrics(SimpleGarbageCollector gc) {
    ...
    Gauge walFinished = Gauge
        .builder(GC_METRIC_PREFIX + ".wal.finished", metricValues.get(),
    ...
    Gauge walCandidates = Gauge
        .builder(GC_METRIC_PREFIX + ".wal.candidates", metricValues.get(),
            v -> v.getLastWalCollect().getCandidates())
        .description("Number of files that are candidates for deletion").register(registry);
    Gauge walInUse = Gauge
        .builder(GC_METRIC_PREFIX + ".wal.in.use", metricValues.get(),
    ...
    Gauge postOpDuration = Gauge
        .builder(GC_METRIC_PREFIX + ".post.op.duration", metricValues.get(),
  ...
  protected void prepareMetrics() {
    metricValues.set(gc.getGcCycleMetrics());
