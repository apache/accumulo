...
public class TablesServlet extends BasicServlet {
  ...
  static void doProblemsBanner(StringBuilder sb) {
    ...
    if (numProblems > 0)
      banner(sb, "error",
          String.format("<a href='/problems'>Table Problems: %d Total</a>", numProblems));
  }

  static void doTableList(HttpServletRequest req, StringBuilder sb,
    ...
    tableList.addSortableColumn("#&nbsp;Offline<br />Tablets", new NumberType<>(0, 0),
        "Tablets unavailable for query or ingest.  "
            + "May be a transient condition when tablets are moved for balancing.");
    tableList.addSortableColumn("Entries", new NumberType<Long>(),
        "Key/value pairs over each instance, table or tablet.");
    ...
    if (Monitor.getMmi() != null && Monitor.getMmi().tableMap != null)
      for (Entry<String,TableInfo> te : Monitor.getMmi().tableMap.entrySet())
        tableStats.put(Tables.getPrintableTableNameFromId(tidToNameMap, te.getKey()),
            te.getValue());

    Map<String,Double> compactingByTable = TableInfoUtil.summarizeTableStats(Monitor.getMmi());
    TableManager tableManager = TableManager.getInstance();

    for (Entry<String,String> tableName_tableId : Tables
        .getNameToIdMap(Monitor.getContext().getInstance()).entrySet()) {
