...
public class DfsLogger implements Comparable<DfsLogger> {
    ...
    try {
      ...
      if (Arrays.equals(magicBuffer, magic)) {
        ...
        EncryptionStrategy encryptionStrategy = EncryptionStrategyFactory.setupReadEncryption(
            conf, cryptoStrategyClass, EncryptionStrategy.Scope.WAL);
  ...
  public synchronized void open(String address) throws IOException {
    ...
    try {
      ...
      this.encryptionStrategy = EncryptionStrategyFactory.setupConfiguredEncryption(conf.getConfiguration(),
          EncryptionStrategy.Scope.WAL);

      log.debug("Using {} for encrypting WAL {}", this.encryptionStrategy.getClass().getSimpleName(),
          filename);
