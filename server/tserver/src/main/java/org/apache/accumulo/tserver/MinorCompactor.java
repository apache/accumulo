    ...
    try {
      do {
        try {
          ...
          if (reportedProblem) {
            ProblemReports.getInstance().deleteProblemReport(getExtent().getTableId().toString(), ProblemType.FILE_WRITE, outputFileName);
          }

          return ret;
        } catch (IOException e) {
          log.warn("MinC failed (" + e.getMessage() + ") to create " + outputFileName + " retrying ...");
          ProblemReports.getInstance().report(new ProblemReport(getExtent().getTableId().toString(), ProblemType.FILE_WRITE, outputFileName, e));
          ...
          log.warn("MinC failed (" + e.getMessage() + ") to create " + outputFileName + " retrying ...", e);
          ProblemReports.getInstance().report(new ProblemReport(getExtent().getTableId().toString(), ProblemType.FILE_WRITE, outputFileName, e));
          reportedProblem = true;
