...
public class Compactor extends AbstractServer implements MetricsProducer, CompactorService.Iface {
  ...
  protected void announceExistence(HostAndPort clientAddress)
    ...
    String compactorQueuePath =
        getContext().getZooKeeperRoot() + Constants.ZCOMPACTORS + "/" + getResourceGroupName();
    ...
    try {
      for (int i = 0; i < 25; i++) {
        ...
        if (compactorLock.tryLock(lw, new ServiceLockData(compactorId, hostPort,
            ThriftService.COMPACTOR, getResourceGroupName()))) {
        ...
        new RetryableThriftCall<>(1000, RetryableThriftCall.MAX_WAIT_TIME, 0, () -> {
          ...
          try {
            ...
            return coordinatorClient.getCompactionJob(TraceUtil.traceInfo(),
                getContext().rpcCreds(), getResourceGroupName(),
  ...
  protected Runnable createCompactionJob(final TExternalCompactionJob job,
    ...
    return () -> {
      ...
      try {
        ...
        ExtCEnv cenv = new ExtCEnv(JOB_HOLDER, getResourceGroupName());
  ...
  protected long getWaitTimeBetweenCompactionChecks() {
    ...
    int numCompactors =
        ExternalCompactionUtil.countCompactors(getResourceGroupName(), getContext());
  ...
  public void run() {
    ...
    try {
      MetricsUtil.initializeMetrics(getContext().getConfiguration(), this.applicationName,
          clientAddress, getResourceGroupName());
      ...
      while (!shutdown) {
        ...
        try {
          ...
          if (!job.isSetExternalCompactionId()) {
            LOG.trace("No external compactions in queue {}", getResourceGroupName());
