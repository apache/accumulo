...
public abstract class VersionedPropCodec {
   ...
   * additional cost of decoding - with a availability of reducing data reads from the store
   * preferred. Generally reading from the store will be followed by some sort of usage which would
   * require the full decode operation anyway, so uses of this method should be narrow and limited.
   *
   * @param bytes serialized encoded versioned property byte array.
