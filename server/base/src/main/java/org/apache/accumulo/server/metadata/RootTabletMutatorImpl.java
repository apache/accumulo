...
public class RootTabletMutatorImpl extends MutationBuilder implements TabletMutator {
    ...
    try {
      ...
      context.getZooReaderWriter().mutate(context.getZooKeeperRoot() + RootTable.ZROOT_TABLET,
          new byte[0], ZooUtil.PUBLIC, currVal -> {
            ...
            // TODO remove
            log.info("Before : " + new String(currVal, UTF_8));

            RootTabletMetadata rtm = RootTabletMetadata.fromJson(new String(currVal, UTF_8));

            rtm.update(mutation);

            // TODO remove
            log.info("After : " + rtm.toJson());
