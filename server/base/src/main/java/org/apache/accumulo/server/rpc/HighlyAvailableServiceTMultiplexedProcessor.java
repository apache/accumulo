...
/**
 * NOTE: This class is a copy of the Thrift TMultiplexecProcessor class with a modification to abort
 * processing if the Accumulo server is upgrading or not active.
 *
 *
 ...
 *
 * <p>
 * To do so, you instantiate the processor and then register additional processors with it, as shown
 ...
 * </blockquote>
 */
public class HighlyAvailableServiceTMultiplexedProcessor implements TProcessor {

  private static final Logger LOG =
      LoggerFactory.getLogger(HighlyAvailableServiceTMultiplexedProcessor.class);
  ...
  public HighlyAvailableServiceTMultiplexedProcessor(HighlyAvailableService service) {
    super();
  ...
  public void process(TProtocol iprot, TProtocol oprot) throws TException {
    ...
    if (service.isUpgrading()) {
      LOG.info("Service can not be accessed while it is upgrading.");
      if (message.type == TMessageType.ONEWAY) {
        return;
      } else {
    ...
    if (!service.isActiveService()) {
      LOG.info("Denying access to RPC service as this instance is not the active instance.");
      if (message.type == TMessageType.ONEWAY) {
        return;
      } else {
        throw new ThriftNotActiveServiceException(service.getServiceName(),
    ...
    if (actualProcessor == null) {
      throw new TProtocolException(TProtocolException.NOT_IMPLEMENTED, "Service name not found: "
          + serviceName + ".  Did you forget to call registerProcessor()?");
    }
