...
public class KerberosAuthImpl extends ZKAuthImpl {
  private static final Logger log = LoggerFactory.getLogger(KerberosAuthImpl.class);
  private UserImpersonation impersonation;

  public KerberosAuthImpl(ServerContext context, ZooCache zooCache, String ZKUserPath) {
    super(context, zooCache, ZKUserPath);
  ...
  @Override
  public Authorizations getAuths(String principal) {
    return super.getAuths(Base64.getEncoder().encodeToString(principal.getBytes(UTF_8)));
  }
  ...
  @Override
  public void setAuths(String principal, Authorizations authorizations)
      throws AccumuloSecurityException {
    super.setAuths(Base64.getEncoder().encodeToString(principal.getBytes(UTF_8)), authorizations);
  }
