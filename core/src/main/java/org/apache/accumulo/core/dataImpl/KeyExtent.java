...
import static org.apache.accumulo.core.util.RangeUtil.isExclusiveKey;
import static org.apache.accumulo.core.util.RangeUtil.requireRowRange;
import static org.apache.accumulo.core.util.RangeUtil.stripZeroTail;
...
public class KeyExtent implements Comparable<KeyExtent> {
  ...
  public KeyExtent clip(Range range, boolean returnNullIfDisjoint) {
    // A standard row range only requires an exclusive key but
    // KeyExtent has a narrower definition of a row range and toDataRange()
    // will always create exclusive start keys as well,
    // so we need to restrict the range further
    requireRowRange(range);
    if (!range.isInfiniteStartKey()) {
      Preconditions.checkArgument(isExclusiveKey(range.getStartKey()),
          "Range requires an exclusive start key %s", range);
    }
