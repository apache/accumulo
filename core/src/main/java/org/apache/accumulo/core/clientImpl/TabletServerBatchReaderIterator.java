...
public class TabletServerBatchReaderIterator implements Iterator<Entry<Key,Value>> {
  ...
  public TabletServerBatchReaderIterator(ClientContext context, TableId tableId, String tableName,
    ...
    this.locator = new TimeoutTabletLocator(retryTimeout,
  ...
  private void doLookups(Map<String,Map<KeyExtent,List<Range>>> binnedRanges,
    ...
    long busyTimeout = ssd.getBusyTimeout();
    Duration scanServerSelectorDelay = ssd.getDelay();
    Map<String,ScanServerAttemptReporter> reporters = ssd.reporters;
  ...
  private ScanServerData binRangesForScanServers(TabletLocator tabletLocator, List<Range> ranges,
    ...
    List<TabletIdImpl> tabletIds =
        extentToRangesMap.keySet().stream().map(TabletIdImpl::new).collect(Collectors.toList());
    ...
    for (TabletIdImpl tabletId : tabletIds) {
      ...
      if (serverToUse == null) {
        ...
        if (serverToUse == null) {
          ...
          log.trace("For tablet {} there is no scan server or tablet server available", tabletId);
        } else {
