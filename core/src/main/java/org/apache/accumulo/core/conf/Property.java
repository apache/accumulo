...
public enum Property {
  ...
  COMPACTION_SERVICE_PREFIX(COMPACTION_PREFIX + "service.", null, PropertyType.PREFIX, """
      ...
      ```
      Additional options can be defined using the `compaction.service.<service>.opts.<option>`
      property.""".formatted("{% jlink " + RatioBasedCompactionPlanner.class.getName() + " %}"),
      "4.0.0"),
  ...
  COMPACTION_SERVICE_DEFAULT_GROUPS(COMPACTION_SERVICE_DEFAULT_PLANNER + ".opts.groups", """
      ...
      "See %s.".formatted("{% jlink " + RatioBasedCompactionPlanner.class.getName() + " %}"),
      "4.0.0"),
  ...
  RPC_PREFIX("rpc.", null, PropertyType.PREFIX, """
      ...
      See also `instance.ssl.enabled`.""", "1.6.0"),
  ...
  RPC_BACKLOG("rpc.backlog", "50", PropertyType.COUNT, """
      ...
      This property is not used for SSL type server sockets. A value of `0`
      will use the Thrift default value.""", "2.1.3"),
  ...
  RPC_SSL_KEYSTORE_PASSWORD("rpc.javax.net.ssl.keyStorePassword", "", PropertyType.STRING, """
      ...
      Leave blank to use the Accumulo instance secret.""", "1.6.0"),
  ...
  RPC_USE_JSSE("rpc.useJsse", "false", PropertyType.BOOLEAN, """
      Use JSSE system properties to configure SSL rather than the `%sjavax.net.ssl.*`
      Accumulo properties.""".formatted(RPC_PREFIX.getKey()), "1.6.0"),
  ...
  RPC_SSL_CLIENT_PROTOCOL("rpc.ssl.client.protocol", "TLSv1.3", PropertyType.STRING, """
      ...
      on the server side `rpc.ssl.server.enabled.protocols`.""", "1.6.2"),
  RPC_SASL_QOP("rpc.sasl.qop", "auth", PropertyType.STRING, """
      The quality of protection to be used with SASL. Valid values are `auth`, `auth-int`, and
      `auth-conf`.""", "1.7.0"),

  // instance properties (must be the same for every node in an instance)
  INSTANCE_PREFIX("instance.", null, PropertyType.PREFIX, """
      Properties in this category must be consistent throughout an instance.
      This is enforced and servers won't be able to communicate if these differ.""", "1.3.5"),
  ...
  INSTANCE_ZK_TIMEOUT("instance.zookeeper.timeout", "30s", PropertyType.TIMEDURATION, """
      ...
      than `%d`.""".formatted(Integer.MAX_VALUE), "1.3.5"),
  @Sensitive
  ...
  INSTANCE_VOLUMES("instance.volumes", "", PropertyType.VOLUMES, """
      ...
      encoding. For example replace commas with `%2C`.""", "1.6.0"),
  INSTANCE_VOLUME_CONFIG_PREFIX("instance.volume.config.", null, PropertyType.PREFIX, """
      ...
      For example:
      ```properties
      instance.volume.config.%s.dfs.client.hedged.read.threadpool.size=10
      ```""".formatted("hdfs\\://namespace-a\\:8020/accumulo"), "2.1.1"),
  INSTANCE_VOLUMES_REPLACEMENTS("instance.volumes.replacements", "", PropertyType.STRING, """
      ...
      example replace commas with `%2C`.""", "1.6.0"),
  ...
  INSTANCE_SECURITY_AUTHENTICATOR("instance.security.authenticator",
      "org.apache.accumulo.server.security.handler.ZKAuthenticator", PropertyType.CLASSNAME, """
          The authenticator class that accumulo will use to determine if a user has privilege
          to perform an action.""", "1.5.0"),
  @Experimental // interface uses unstable internal types, use with caution
  ...
  INSTANCE_RPC_SSL_ENABLED("instance.rpc.ssl.enabled", "false", PropertyType.BOOLEAN, """
      ...
      Mutually exclusive with SASL RPC configuration.""", "1.6.0"),
  ...
  INSTANCE_RPC_SASL_ENABLED("instance.rpc.sasl.enabled", "false", PropertyType.BOOLEAN, """
      ...
      Mutually exclusive with SSL RPC configuration.""", "1.7.0"),
  INSTANCE_RPC_SASL_ALLOWED_USER_IMPERSONATION("instance.rpc.sasl.allowed.user.impersonation", "",
      PropertyType.STRING, """
          One-line configuration property controlling which users are allowed to"
          impersonate other users.""", "1.7.1"),
  INSTANCE_RPC_SASL_ALLOWED_HOST_IMPERSONATION("instance.rpc.sasl.allowed.host.impersonation", "",
      PropertyType.STRING, """
          One-line configuration property controlling the network locations (hostnames) that are
          allowed to impersonate other users.""", "1.7.1"),
  ...
  @Experimental
  INSTANCE_CRYPTO_FACTORY("instance.crypto.opts.factory", NoCryptoServiceFactory.class.getName(),
      PropertyType.CLASSNAME,
      """
          The class which provides crypto services for on-disk file encryption.
          The default does nothing. To enable encryption, replace this classname with an
          implementation of the %s interface."""
          .formatted("{% jlink " + CryptoServiceFactory.class.getName() + " %}"),
      "2.1.0"),
  // general properties
  GENERAL_PREFIX("general.", null, PropertyType.PREFIX, """
      Properties in this category affect the behavior of accumulo overall, but do not have to be
      consistent throughout a cloud.""", "1.3.5"),

  GENERAL_CONTEXT_CLASSLOADER_FACTORY("general.context.class.loader.factory", "",
      PropertyType.CLASSNAME, """
          Name of classloader factory to be used to create classloaders for named contexts, such as
          per-table contexts set by `table.class.loader.context`.""", "2.1.0"),
  ...
  GENERAL_RPC_SERVER_TYPE("general.rpc.server.type", "", PropertyType.STRING, """
      Type of Thrift server to instantiate, see org.apache.accumulo.server.rpc.ThriftServerType
      for more information. Only useful for benchmarking thrift servers.""", "1.7.0"),
  ...
  GENERAL_KERBEROS_PRINCIPAL("general.kerberos.principal", "", PropertyType.STRING, """
      Name of the kerberos principal to use. _HOST will automatically be replaced by the
      machine's hostname in the hostname portion of the principal.
      Leave blank if not using kerberoized hdfs.""", "1.4.1"),
  GENERAL_KERBEROS_RENEWAL_PERIOD("general.kerberos.renewal.period", "30s",
      PropertyType.TIMEDURATION, """
          ...
          (which is performed at 80% of the ticket lifetime).""", "1.6.5"),
  ...
  GENERAL_ARBITRARY_PROP_PREFIX("general.custom.", null, PropertyType.PREFIX, """
      Prefix to be used for user defined system-wide properties. This may be used for
      system-wide configuration for various user-implementations of pluggable Accumulo features,
      such as the balancer or volume chooser.""", "2.0.0"),
  GENERAL_CACHE_MANAGER_IMPL("general.block.cache.manager.class",
      TinyLfuBlockCacheManager.class.getName(), PropertyType.CLASSNAME,
  ...
  GENERAL_IDLE_PROCESS_INTERVAL("general.metrics.process.idle", "5m", PropertyType.TIMEDURATION, """
      Amount of time a process must be idle before it is considered to be idle by the metrics
      system.""", "2.1.3"),
  ...
  GENERAL_LOW_MEM_DETECTOR_THRESHOLD("general.low.mem.detector.threshold", "0.05",
      PropertyType.FRACTION, """
          ...
          percentage of total memory.""", "3.0.0"),
  GENERAL_LOW_MEM_SCAN_PROTECTION("general.low.mem.protection.scan", "false", PropertyType.BOOLEAN,
      """
          Scans may be paused or return results early when the server is low on memory and this
          property is set to true. Enabling this property will incur a slight scan performance
          penalty when the server is not low on memory.""", "3.0.0"),
  GENERAL_LOW_MEM_MINC_PROTECTION("general.low.mem.protection.compaction.minc", "false",
      PropertyType.BOOLEAN, """
          Minor compactions may be paused when the server is low on memory and this property is set
          to true. Enabling this property will incur a slight compaction performance penalty when
          the server is not low on memory.""", "3.0.0"),
  GENERAL_LOW_MEM_MAJC_PROTECTION("general.low.mem.protection.compaction.majc", "false",
      PropertyType.BOOLEAN, """
          Major compactions may be paused when the server is low on memory and this property is set
          to true. Enabling this property will incur a slight compaction performance penalty when
          the server is not low on memory.""", "3.0.0"),
  ...
  GENERAL_MICROMETER_JVM_METRICS_ENABLED("general.micrometer.jvm.metrics.enabled", "false",
      PropertyType.BOOLEAN,
      """
          Enables additional JVM metrics collection and reporting using Micrometer.
          Requires property %s to be set to `true` to take effect."""
          .formatted(markdownRef(GENERAL_MICROMETER_ENABLED.getKey())),
      "2.1.0"),
  GENERAL_MICROMETER_LOG_METRICS("general.micrometer.log.metrics", "none", PropertyType.STRING,
      """
          Enables additional log metrics collection and reporting using Micrometer.
          Requires property %s to be set to `true` to take effect. Micrometer natively instruments
          Log4j2 and Logback. Valid values for this property are `none`, `log4j2` or `logback`."""
          .formatted(markdownRef(GENERAL_MICROMETER_ENABLED.getKey())),
      "2.1.4"),
  GENERAL_MICROMETER_FACTORY("general.micrometer.factory",
      LoggingMeterRegistryFactory.class.getName(), PropertyType.CLASSNAMELIST,
      "A comma separated list of one or more class names that implements %s."
          .formatted("{% jlink " + MeterRegistryFactory.class.getName() + " %}"),
      "2.1.0"),
  GENERAL_MICROMETER_USER_TAGS("general.micrometer.user.tags", "", PropertyType.STRING, """
      A comma separated list of tags to emit with all metrics from the process.

      For example:
      ```properties
      general.micrometer.user.tags=tag1=value1,tag2=value2
      ```""", "4.0.0"),
  ...
  GENERAL_PROCESS_BIND_ADDRESS("general.process.bind.addr", "0.0.0.0", PropertyType.STRING, """
      The local IP address to which this server should bind for sending and receiving network
      traffic.""", "3.0.0"),
  ...
  GENERAL_SERVER_LOCK_VERIFICATION_INTERVAL("general.server.lock.verification.interval", "2m",
      PropertyType.TIMEDURATION, """
          Interval at which the Manager and TabletServer should verify their server locks.
          A value of `0` disables this check. The default value changed from `0` to `2m`
          in 4.0.0.""", "2.1.4"),
  ...
  MANAGER_TABLET_BALANCER_TSERVER_THRESHOLD("manager.tablet.balancer.tserver.threshold", "0",
      PropertyType.COUNT, """
          Indicates the minimum number of tservers for assignment and balancing operations for
          user tables. A value of `0` (default) disables this threshold allowing assignment and
          balancing to always occur.""", "2.1.4"),
  MANAGER_TABLET_GROUP_WATCHER_INTERVAL("manager.tablet.watcher.interval", "60s",
      PropertyType.TIMEDURATION, """
          Time to wait between scanning tablet states to identify tablets that need to be assigned,
          un-assigned, migrated, etc.""", "2.1.2"),
  MANAGER_TABLET_GROUP_WATCHER_SCAN_THREADS("manager.tablet.watcher.scan.threads.max", "8",
      PropertyType.COUNT, """
          Maximum number of threads the TabletGroupWatcher will use in its BatchScanner to look for
          tablets that need maintenance.""", "2.1.4"),
  MANAGER_TABLET_REFRESH_MINTHREADS("manager.tablet.refresh.threads.minimum", "10",
      PropertyType.COUNT, """
          The Manager will notify TabletServers that a Tablet needs to be refreshed after certain
          operations are performed (e.g. Bulk Import). This property specifies the number of core
          threads in a ThreadPool in the Manager that will be used to request these refresh
          operations.""", "4.0.0"),
  MANAGER_TABLET_REFRESH_MAXTHREADS("manager.tablet.refresh.threads.maximum", "10",
      PropertyType.COUNT, """
          The Manager will notify TabletServers that a Tablet needs to be refreshed after certain
          operations are performed (e.g. Bulk Import). This property specifies the maximum number of
          threads in a ThreadPool in the Manager that will be used to request these refresh
          operations.""", "4.0.0"),
  MANAGER_TABLET_MERGEABILITY_INTERVAL("manager.tablet.mergeability.interval", "24h",
      PropertyType.TIMEDURATION, """
          ...
          auto-merged. Valid ranges will be have merge fate ops submitted.""", "4.0.0"),
  ...
  MANAGER_MINTHREADS_TIMEOUT("manager.server.threads.timeout", "0s", PropertyType.TIMEDURATION, """
      The time after which incoming request threads terminate with no work available.
      `0` will keep the threads alive indefinitely.""", "2.1.0"),
  ...
  MANAGER_RECOVERY_DELAY("manager.recovery.delay", "10s", PropertyType.TIMEDURATION, """
      When a tablet server's lock is deleted, it takes time for it to completely quit.
      This delay gives it time before log recoveries begin.""", "1.5.0"),
  ...
  MANAGER_FATE_CONDITIONAL_WRITER_THREADS_MAX("manager.fate.conditional.writer.threads.max", "3",
      PropertyType.COUNT, """
          Maximum number of threads to use for writing data to tablet servers of the FATE system
          table.""", "4.0.0"),
  ...
  MANAGER_FATE_USER_CONFIG("manager.fate.user.config", """
      { "general": {"TABLE_CREATE,TABLE_DELETE,TABLE_RENAME,TABLE_ONLINE,TABLE_OFFLINE,\
      NAMESPACE_CREATE,NAMESPACE_DELETE,NAMESPACE_RENAME,TABLE_TABLET_AVAILABILITY,\
      SHUTDOWN_TSERVER,TABLE_BULK_IMPORT2,TABLE_COMPACT,TABLE_CANCEL_COMPACT,TABLE_MERGE,\
      TABLE_DELETE_RANGE,TABLE_SPLIT,TABLE_CLONE,TABLE_IMPORT,TABLE_EXPORT,SYSTEM_MERGE": 4},\
      ...
      operations and whose value is a pool size for those operations.""", "4.0.0"),
  ...
  MANAGER_FATE_THREADPOOL_SIZE("manager.fate.threadpool.size", "64",
      ...
      """
          ...
          set instead.""".formatted(MANAGER_FATE_USER_CONFIG.getKey(),
  ...
  MANAGER_FATE_IDLE_CHECK_INTERVAL("manager.fate.idle.check.interval", "60m",
      ...
      """
          ...
          maximum value of 60m.""".formatted(MANAGER_FATE_USER_CONFIG.getKey(),
          MANAGER_FATE_META_CONFIG.getKey()),
      "4.0.0"),
  MANAGER_STATUS_THREAD_POOL_SIZE("manager.status.threadpool.size", "0", PropertyType.COUNT, """
      ...
      `0` indicates an unlimited number of threads will be used.""", "1.8.0"),
  MANAGER_METADATA_SUSPENDABLE("manager.metadata.suspendable", "false", PropertyType.BOOLEAN,
      """
          Allow tablets for the `%s` table to be suspended via %s.""".formatted(
          SystemTables.METADATA.tableName(), markdownRef("table.suspend.duration")),
      "1.8.0"),
  ...
  MANAGER_STARTUP_TSERVER_AVAIL_MAX_WAIT("manager.startup.tserver.avail.max.wait", "0",
      PropertyType.TIMEDURATION, """
          ...
          threshold is set greater than 0.""", "1.10.0"),
  ...
  SSERV_GROUP_NAME("sserver.group", ScanServerSelector.DEFAULT_SCAN_SERVER_GROUP_NAME,
      PropertyType.STRING, """
          ...
          eventual consistency scans.""", "3.0.0"),
  ...
  SSERV_CACHED_TABLET_METADATA_REFRESH_PERCENT("sserver.cache.metadata.refresh.percent", ".75",
      PropertyType.FRACTION, """
          ...
          Value must be less than 100%. Set to 0 will disable refresh.""", "2.1.3"),
  ...
  SSERV_SCAN_EXECUTORS_PREFIX("sserver.scan.executors.", null, PropertyType.PREFIX, """
      Prefix for defining named executors to service scans. See
      ...
      and an optional prioritizer can be configured.

      To configure a new executor named `NAME`, set:
      ```properties
      sserver.scan.executors.NAME.threads=NUMBER_OF_THREADS
      ; optionally, can also set the following
      sserver.scan.executors.NAME.priority=NUMBER_1_TO_10
      sserver.scan.executors.NAME.prioritizer=CLASSNAME
  ...
  SSERV_SCAN_EXECUTORS_DEFAULT_PRIORITIZER("sserver.scan.executors.default.prioritizer", "",
      PropertyType.CLASSNAME, """
          ...
          %s to configure one.""".formatted("{% jlink " + ScanPrioritizer.class.getName() + " %}"),
      "2.1.0"),
  ...
  TSERV_PORTSEARCH("tserver.port.search", "true", PropertyType.BOOLEAN,
      "if the %s ports are in use, search higher ports until one is available."
          .formatted(markdownRef(TSERV_CLIENTPORT.getKey())),
      "1.3.5"),
  ...
  TSERV_WAL_MAX_REFERENCED("tserver.wal.max.referenced", "3", PropertyType.COUNT, """
      When a tablet server has more than this many write ahead logs, any tablet referencing
      older logs over this threshold is minor compacted. Also any tablet referencing this many
      logs or more will be compacted.""", "2.1.0"),
  TSERV_WAL_MAX_SIZE("tserver.wal.max.size", "1G", PropertyType.BYTES,
      "The maximum size for each write-ahead log. See comment for property %s."
          .formatted(markdownRef("tserver.memory.maps.max")),
      "2.1.0"),
  ...
  TSERV_WAL_TOLERATED_CREATION_FAILURES("tserver.wal.tolerated.creation.failures", "50",
      PropertyType.COUNT, """
          ...
          causes the TabletServer to exit.""", "2.1.0"),
  ...
  TSERV_MAX_IDLE("tserver.files.open.idle", "1m", PropertyType.TIMEDURATION, """
      Tablet servers leave previously used RFiles open for future queries. This setting
      determines how much time an unused RFile should be kept open until it is closed.""", "1.3.5"),
  TSERV_NATIVEMAP_ENABLED("tserver.memory.maps.native.enabled", "true", PropertyType.BOOLEAN, """
      An off-heap in-memory data store for accumulo implemented in c++ that increases the amount of
      data accumulo can hold in memory and avoids Java GC pauses.""", "1.3.5"),
  TSERV_MAXMEM("tserver.memory.maps.max", "33%", PropertyType.MEMORY,
      """
          ...
          There are two other properties that can effectively limit memory usage: %s and %s.
          Ensure that `%s * %s >= this property`. This map is created in off-heap memory when
          %s is enabled.""".formatted(markdownRef(TSERV_WAL_MAX_REFERENCED.getKey()),
          markdownRef(TSERV_WAL_MAX_SIZE.getKey()), TSERV_WAL_MAX_REFERENCED.getKey(),
          TSERV_WAL_MAX_SIZE.getKey(), markdownRef(TSERV_NATIVEMAP_ENABLED.getKey())),
      "1.3.5"),
  TSERV_SESSION_MAXIDLE("tserver.session.idle.max", "1m", PropertyType.TIMEDURATION, """
      When a tablet server's SimpleTimer thread triggers to check idle sessions, this
      configurable option will be used to evaluate scan sessions to determine if they can be
      closed due to inactivity.""", "1.3.5"),
  TSERV_UPDATE_SESSION_MAXIDLE("tserver.session.update.idle.max", "1m", PropertyType.TIMEDURATION,
      """
          When a tablet server's SimpleTimer thread triggers to check idle sessions, this
          configurable option will be used to evaluate update sessions to determine if they can be
          closed due to inactivity.""", "1.6.5"),
  TSERV_SCAN_EXECUTORS_PREFIX("tserver.scan.executors.", null, PropertyType.PREFIX, """
      ...
      To configure a new executor named `NAME`, set:
      ```properties
      tserver.scan.executors.NAME.threads=NUMBER_OF_THREADS
      ; optionally, can also set the following
      tserver.scan.executors.NAME.priority=NUMBER_1_TO_10
      tserver.scan.executors.NAME.prioritizer=CLASSNAME
  ...
  TSERV_SCAN_EXECUTORS_DEFAULT_PRIORITIZER("tserver.scan.executors.default.prioritizer", "",
      ...
      """
          Prioritizer for the default scan executor. Defaults to none which
          results in FIFO priority. Set to a class that implements %s to configure one."""
          .formatted("{% jlink " + ScanPrioritizer.class.getName() + " %}"),
      "2.0.0"),
  ...
  TSERV_BLOOM_LOAD_MAXCONCURRENT("tserver.bloom.load.concurrent.max", "4", PropertyType.COUNT, """
      ...
      Setting this to `0` will make bloom filters load in the foreground.""", "1.3.5"),
  TSERV_MEMDUMP_DIR("tserver.dir.memdump", "/tmp", PropertyType.PATH, """
      ...
      file dumped to the local dir is an exact copy of what was in memory.""", "1.3.5"),
  ...
  TSERV_LOG_BUSY_TABLETS_COUNT("tserver.log.busy.tablets.count", "0", PropertyType.COUNT,
      """
          Number of busiest tablets to log. Logged at interval controlled by %s. If `<= 0`,
          logging of busy tablets is disabled."""
          .formatted(markdownRef(TSERV_LOG_BUSY_TABLETS_INTERVAL.getKey())),
      "1.10.0"),
  TSERV_HOLD_TIME_SUICIDE("tserver.hold.time.max", "5m", PropertyType.TIMEDURATION, """
      ...
      assume there is some failure local to its node, and quit. A value of `0`
      is equivalent to forever.""", "1.4.0"),
  TSERV_WAL_BLOCKSIZE("tserver.wal.blocksize", "0", PropertyType.BYTES,
      """
          The size of the HDFS blocks used to write to the Write-Ahead log.
          If `0`, it will be `110%%` of %s (that is, try to use just one block)."""
          .formatted(markdownRef(TSERV_WAL_MAX_SIZE.getKey())),
      "1.5.0"),
  TSERV_WAL_REPLICATION("tserver.wal.replication", "0", PropertyType.COUNT, """
      The replication to use when writing the Write-Ahead log to HDFS.
      If `0`, it will use the HDFS default replication setting.""", "1.5.0"),
  ...
  TSERV_WAL_SORT_FILE_PREFIX("tserver.wal.sort.file.", null, PropertyType.PREFIX, """
      The rfile properties to use when sorting logs during recovery.
      Most of the properties that begin with `table.file` can be used here.

      For example, to set the compression of the sorted recovery files to snappy use:
      ```properties
      tserver.wal.sort.file.compress.type=snappy
      ```""", "2.1.0"),
  TSERV_WAL_SYNC("tserver.wal.sync", "true", PropertyType.BOOLEAN, """
      Use the SYNC_BLOCK create flag to sync WAL writes to disk.
      Prevents problems recovering from sudden system resets.""", "1.5.0"),
  TSERV_ASSIGNMENT_DURATION_WARNING("tserver.assignment.duration.warning", "10m",
      PropertyType.TIMEDURATION, """
          The amount of time an assignment can run before the server will print a warning along
          with the current stack trace. Meant to help debug stuck assignments.""", "1.6.2"),
  ...
  TSERV_SLOW_FLUSH_MILLIS("tserver.slow.flush.time", "100ms", PropertyType.TIMEDURATION, """
      ...
      information will written, and may result in a log rollover.""", "1.8.0"),
  TSERV_SLOW_FILEPERMIT_MILLIS("tserver.slow.filepermit.time", "100ms", PropertyType.TIMEDURATION,
      """
          ...
          debugging information will be written.""", "1.9.3"),
  TSERV_SUMMARY_PARTITION_THREADS("tserver.summary.partition.threads", "10", PropertyType.COUNT, """
      ...
      broken into partitions of 100k files. This setting determines how many of these groups of 100k
      RFiles will be processed concurrently.""", "2.0.0"),
  TSERV_SUMMARY_REMOTE_THREADS("tserver.summary.remote.threads", "128", PropertyType.COUNT, """
      For a partitioned group of 100k RFiles, those files are grouped by tablet server. Then, a
      remote tablet server is asked to gather summary data. This setting determines how many such
      requests are made concurrently per partition.""", "2.0.0"),
  TSERV_SUMMARY_RETRIEVAL_THREADS("tserver.summary.retrieval.threads", "10", PropertyType.COUNT, """
      The number of threads on each tablet server available to retrieve summary data, that is
      not currently in cache, from RFiles.""", "2.0.0"),
  ...
  TSERV_GROUP_NAME("tserver.group", Constants.DEFAULT_RESOURCE_GROUP_NAME, PropertyType.STRING, """
      Resource group name for this TabletServer. Resource groups can be defined to dedicate
      resources to specific tables(e.g. balancing tablets for table(s) within a group,
      see TableLoadBalancer).""", "4.0.0"),
  ...
  GC_CYCLE_DELAY("gc.cycle.delay", "5m", PropertyType.TIMEDURATION, """
      Time between garbage collection cycles. In each cycle, old RFiles or write-ahead logs no
      longer in use are removed from the filesystem.""", "1.3.5"),
  ...
  GC_USE_FULL_COMPACTION("gc.post.metadata.action", "flush", PropertyType.GC_POST_ACTION, """
      ...
      completion, to force the changes to be written to disk, the metadata and
      root tables can be flushed and possibly compacted. Legal values are:
      `compact` which both flushes and compacts the metadata; `flush` which
      flushes only (compactions may be triggered if required); or `none`.""", "1.10.0"),
  ...
  MONITOR_SSL_KEYPASS("monitor.ssl.keyPassword", "", PropertyType.STRING, """
      Optional: the password for the private key in the keyStore. When not provided,
      this defaults to the keystore password.""", "1.9.3"),
  ...
  MONITOR_SSL_INCLUDE_CIPHERS("monitor.ssl.include.ciphers", "", PropertyType.STRING,
      "A comma-separated list of allows SSL Ciphers, see %s to disallow ciphers."
          .formatted(markdownRef("monitor.ssl.exclude.ciphers")),
      "1.6.1"),
  MONITOR_SSL_EXCLUDE_CIPHERS("monitor.ssl.exclude.ciphers", "", PropertyType.STRING,
      "A comma-separated list of disallowed SSL Ciphers, see %s to allow ciphers."
          .formatted(markdownRef(MONITOR_SSL_INCLUDE_CIPHERS.getKey())),
      "1.6.1"),
  ...
  MONITOR_RESOURCES_EXTERNAL("monitor.resources.external", "", PropertyType.JSON, """
      A JSON Map of Strings. Each String should be an HTML tag of an external resource (JS or CSS)
      to be imported by the Monitor. If this value is set, all of the external resources
      ...
      `accumulo/server/monitor/src/main/resources/templates/default.ftl`.""", "2.0.0"),
  MONITOR_FETCH_TIMEOUT("monitor.fetch.timeout", "5m", PropertyType.TIMEDURATION, """
      ...
      to respond or for scans to complete.""", "4.0.0"),
  MONITOR_DEAD_LIST_RG_EXCLUSIONS("monitor.dead.server.rg.exclusions", "", PropertyType.STRING, """
      ...
      then it will be suppressed from the dead servers list in the monitor.""", "4.0.0"),
  MONITOR_ROOT_CONTEXT("monitor.root.context", "/", PropertyType.STRING, """
      ...
      monitor application will be hosted using this context. As an example, setting this to
      `/accumulo` would cause all `/rest/` endpoints to be hosted at `/accumulo/rest/*`.""",
      "2.1.4"),
  ...
  TABLE_PREFIX("table.", null, PropertyType.PREFIX, """
      ...
      to change properties in zookeeper that are set on a table.""", "1.3.5"),
  ...
  TABLE_COMPACTION_INPUT_DROP_CACHE_BEHIND("table.compaction.input.drop.cache", "ALL",
      PropertyType.DROP_CACHE_SELECTION, """
          ...
          and will be compacted at different times.""", "2.1.4"),
  TABLE_MINC_OUTPUT_DROP_CACHE("table.compaction.minor.output.drop.cache", "false",
      PropertyType.BOOLEAN, """
          Setting this property to true will call `FSDataOutputStream.setDropBehind(true)` on the
          minor compaction output stream.""", "2.1.1"),
  TABLE_MAJC_OUTPUT_DROP_CACHE("table.compaction.major.output.drop.cache", "false",
      PropertyType.BOOLEAN, """
          Setting this property to true will call `FSDataOutputStream.setDropBehind(true)` on the
          major compaction output stream.""", "2.1.1"),
  ...
  TABLE_MINC_COMPACT_MAXAGE("table.compaction.minor.age", "10m", PropertyType.TIMEDURATION,
      """
          Key values written to a tablet are temporarily stored in a per tablet in memory map.
          When the age of the oldest key value in a tablets in memory map exceeds this
          configuration, then a minor compaction may be initiated. This determines the maximum
          amount of time new data can be buffered in memory before being flushed to a file.
          This is useful when using scan servers in conjunction with the property %s.

          These two properties together can be used to control that amount of time it takes for a
          scan server to see a write to a tablet server.

          The default value of this property is set
          to such a high value that is should never cause a minor compaction."""
  ...
  TABLE_ONDEMAND_UNLOADER("tserver.ondemand.tablet.unloader",
      LastAccessTimeOnDemandTabletUnloader.class.getName(), PropertyType.CLASSNAME,
  ...
  TABLE_MAX_MERGEABILITY_THRESHOLD("table.mergeability.threshold", ".25", PropertyType.FRACTION, """
      A range of tablets are eligible for automatic merging until the combined size of RFiles
      reaches this percentage of the split threshold.""", "4.0.0"),
  ...
  TABLE_SCAN_DISPATCHER("table.scan.dispatcher", SimpleScanDispatcher.class.getName(),
      ...
      """
          ...
          table. The metadata table always dispatches to a scan executor named `meta`.""".formatted(
          "{% jlink " + ScanDispatcher.class.getName() + " %}",
  ...
  TABLE_SCAN_MAXMEM("table.scan.max.memory", "512k", PropertyType.BYTES, """
      ...
      Once this limit is reached, the buffered data is sent to the client.""", "1.3.5"),
  TABLE_BULK_MAX_TABLETS("table.bulk.max.tablets", "100", PropertyType.COUNT, """
      The maximum number of tablets allowed for one bulk import file.
      A value of `0` means unlimited.""", "2.1.0"),
  TABLE_BULK_MAX_TABLET_FILES("table.bulk.max.tablet.files", "100", PropertyType.COUNT, """
      The maximum number of files a bulk import can add to a single tablet. When this property is
      exceeded for any tablet the entire bulk import operation will fail before making any changes.
      A value of `0` means unlimited.""", "4.0.0"),
  TABLE_FILE_TYPE("table.file.type", RFile.EXTENSION, PropertyType.FILENAME_EXT,
      "Change the type of file a table writes.", "1.3.5"),
  TABLE_LOAD_BALANCER("table.balancer", SimpleLoadBalancer.class.getName(), PropertyType.CLASSNAME,
      """
          This property can be set to allow the %s to change which %s is used for this table."""
          .formatted("{% jlink " + TableLoadBalancer.class.getName() + " %}",
              "{% jlink " + TabletBalancer.class.getName() + " %}"),
      "1.3.5"),
  TABLE_FILE_COMPRESSION_TYPE("table.file.compress.type", "gz", PropertyType.STRING, """
      Compression algorithm used on index and data blocks before they are written.
      Possible values: `zstd`, `gz`, `snappy`, `bzip2`, `lzo`, `lz4`, `none`.""", "1.3.5"),
  ...
  TABLE_FILE_BLOCK_SIZE("table.file.blocksize", "0B", PropertyType.BYTES, """
      The HDFS block size used when writing RFiles. When set to `0B`, the value/defaults of HDFS
      property `dfs.block.size` will be used.""", "1.3.5"),
  TABLE_FILE_REPLICATION("table.file.replication", "0", PropertyType.COUNT, """
      The number of replicas for a table's RFiles in HDFS.
      When set to `0`, HDFS defaults are used.""", "1.3.5"),
  TABLE_FILE_MAX("table.file.max", "15", PropertyType.COUNT, """
      ...
      be more aggressive for compacting tablets that exceed this limit.

      The %s will lower the compaction ratio and increase the priority for tablets
      that exceed this limit.

      When adjusting this property, you may want to consider also adjusting %s.

      Setting this property to `0` will make it default to one less than %s, which
      which will prevent a tablet from having more RFiles than can be opened by a scan."""
      .formatted("{% jlink " + RatioBasedCompactionPlanner.class.getName() + " %}",
          markdownRef(TABLE_MAJC_RATIO.getKey()), markdownRef(TSERV_SCAN_MAX_OPENFILES.getKey())),
      "1.4.0"),
  TABLE_FILE_PAUSE("table.file.pause", "100", PropertyType.COUNT,
      """
          ...
          prior to reaching the pause point. A value of `0` means unlimited.""".formatted(
          markdownRef(TABLE_BULK_MAX_TABLET_FILES.getKey()), markdownRef(TABLE_FILE_MAX.getKey())),
      "4.0.0"),
  TABLE_MERGE_FILE_MAX("table.merge.file.max", "10000", PropertyType.COUNT, """
      ...
      be 1000 and the merge will only proceed if this property is greater than 1000.""", "4.0.0"),
  ...
  TABLE_BLOOM_LOAD_THRESHOLD("table.bloom.load.threshold", "1", PropertyType.COUNT, """
      This number of seeks that would actually use a bloom filter must occur before a RFile's
      bloom filter is loaded.

      Set this to `0` to initiate loading of bloom filters when a RFile is opened.""", "1.3.5"),
  ...
  TABLE_BLOOM_KEY_FUNCTOR("table.bloom.key.functor", RowFunctor.class.getName(),
      PropertyType.CLASSNAME,
      """
          A function that can transform the key prior to insertion and check of
          bloom filter.

          %s, %s, and %s are allowable values.

          One can extend any of these classes to perform specialized parsing of the key."""
          .formatted("{% jlink -f " + RowFunctor.class.getName() + " %}",
              "{% jlink -f " + ColumnFamilyFunctor.class.getName() + " %}",
              "{% jlink -f " + ColumnQualifierFunctor.class.getName() + " %}"),
      "1.3.5"),
  ...
  TABLE_BULK_SKIP_THRESHOLD("table.bulk.metadata.skip.distance", "0", PropertyType.COUNT, """
      ...
      of tablets of a large table then the tablet metadata information for most tablets will be
      needed.

      However, when importing files into a small number of non-contiguous tablets in a large table,
      ...
      a new scanner is analogous to performing a seek in an iterator, but it has a cost.

      A value of `0` (default) disables this feature.
      A non-zero value enables this feature and the Manager will setup a new scanner
      when the tablet metadata distance is above the supplied value.""", "2.1.4"),
  TABLE_DURABILITY("table.durability", "sync", PropertyType.DURABILITY, """
      The durability used to write to the write-ahead log.

      Legal values are:
      * `none`, which skips the write-ahead log,
      ...
      * `sync`, which ensures the data is written to disk.""", "1.7.0"),
  ...
  TABLE_DEFAULT_SCANTIME_VISIBILITY("table.security.scan.visibility.default", "",
      PropertyType.STRING, """
          ...
          Note: An empty security label is displayed as `[]` in the shell.
          The scan results will show an empty visibility even if the visibility from this
          ...
          **CAUTION**: If a particular key has an empty security label *AND* its
          ...
          will be interpreted with the new label on the next scan.""", "1.3.5"),
  ...
  TABLE_ITERATOR_PREFIX("table.iterator.", null, PropertyType.PREFIX, """
      ...
      `table.iterator.scan.vers`, or `table.iterator.scan.custom`.

      The values for these properties are a number indicating the ordering in which it is
  ...
  TABLE_LOCALITY_GROUP_PREFIX("table.group.", null, PropertyType.PREFIX, """
      ...
      For example, the following sets the column families for a group called `group1`:
      ```properties
      table.group.group1=x,y,z
      ```

      Once configured, `group1` can be enabled by adding it to the list of groups in the %s
      property.

      Additional group options may be specified for a named group by setting properties like:
      ```properties
      table.group.NAME.opt.KEY=VALUE
      ```""".formatted(TABLE_LOCALITY_GROUPS.getKey()), "1.3.5"),
  TABLE_FORMATTER_CLASS("table.formatter", DefaultFormatter.class.getName(), PropertyType.CLASSNAME,
  ...
  TABLE_CLASSLOADER_CONTEXT("table.class.loader.context", "", PropertyType.STRING, """
      The context to use for loading per-table resources, such as iterators from the configured
      factory in %s.""".formatted(markdownRef(GENERAL_CONTEXT_CLASSLOADER_FACTORY.getKey())),
      "2.1.0"),
  TABLE_SAMPLER("table.sampler", "", PropertyType.CLASSNAME, """
      The name of a class that implements %s.
      ...
      sample data.""".formatted("{% jlink " + Sampler.class.getName() + " %}"), "1.8.0"),
  TABLE_SAMPLER_OPTS("table.sampler.opt.", null, PropertyType.PREFIX, """
      The property is used to set options for a sampler. If a sample had two options like hasher
      and modulous, then the two properties would look like:
      ```properties
      table.sampler.opt.hasher=${hash algorithm}
      table.sampler.opt.modulous=${mod}
      ```""", "1.8.0"),
  TABLE_SUSPEND_DURATION("table.suspend.duration", "0s", PropertyType.TIMEDURATION, """
      For tablets belonging to this table: When a tablet server dies, allow the tablet server
      this duration to revive before reassigning its tablets to other tablet servers.""", "1.8.0"),
  TABLE_SUMMARIZER_PREFIX("table.summarizer.", null, PropertyType.PREFIX, """
      ...
      `table.summarizer.<unique id>.opt.<key>=<value>`.""", "2.0.0"),
  @Experimental
  TABLE_DELETE_BEHAVIOR("table.delete.behavior",
      ...
      """
          ...
          need fast seeks within the timestamp range of a column.

          When setting this to fail, also consider configuring the %s constraint."""
          .formatted("{% jlink -f " + NoDeleteConstraint.class.getName() + " %}"),
      "2.0.0"),
  TABLE_ENABLE_ERASURE_CODES("table.file.ec", "inherit", PropertyType.EC,
      """
          ...
          When setting this to `enable` must also set %s and that policy will
          always be used regardless of DFS directory settings.

          When set to `disable`, replication will always be used regardless of DFS directory
          settings.

          When set to `inherit`, the settings from the directory in dfs will be used.

          Enabling erasure coding on a volume that does not support it is a noop."""
          .formatted(markdownRef("table.file.ec.policy")),
      "2.1.4"),

  TABLE_ERASURE_CODE_POLICY("table.file.ec.policy", "", PropertyType.STRING,
      """
          The name of the erasure code policy to be used. Policy must be available and enabled in
          hdfs.

          To view if policy is enabled, check `hdfs ec -listPolicies`.

          This setting is only used when %s is set to enable."""
          .formatted(markdownRef(TABLE_ENABLE_ERASURE_CODES.getKey())),
      "2.1.4"),
  ...
  COMPACTOR_CANCEL_CHECK_INTERVAL("compactor.cancel.check.interval", "5m",
      PropertyType.TIMEDURATION, """
          ...
          This checks for situations like was the tablet deleted (split and merge do this),
          was the table deleted, was a user compaction canceled, etc.""", "2.1.4"),
  ...
  COMPACTOR_MIN_JOB_WAIT_TIME("compactor.wait.time.job.min", "1s", PropertyType.TIMEDURATION,
      """
          ...
          exponentially until %s is reached."""
          .formatted(markdownRef(COMPACTOR_MAX_JOB_WAIT_TIME.getKey())),
      "2.1.3"),
  COMPACTOR_FAILURE_BACKOFF_THRESHOLD("compactor.failure.backoff.threshold", "3",
      PropertyType.COUNT, """
          The number of consecutive failures that must occur before the Compactor starts to back
          off processing compactions.""", "2.1.4"),
  COMPACTOR_FAILURE_BACKOFF_INTERVAL("compactor.failure.backoff.interval", "0",
      PropertyType.TIMEDURATION,
      """
          The time basis for computing the wait time for compaction failure backoff. A value of `0`
          disables the backoff feature. When a non-zero value is supplied, then after
          %s failures have occurred, the compactor will wait this amount of time for each failure
          before executing the next compaction.

          For example, if this value is `10s`, then after `3` failures, the Compactor will wait
          `30s` before starting the next compaction. If the compaction fails again, then it will
          wait `40s` before starting the next compaction."""
          .formatted(markdownRef(COMPACTOR_FAILURE_BACKOFF_THRESHOLD.getKey())),
      "2.1.4"),
  COMPACTOR_FAILURE_BACKOFF_RESET("compactor.failure.backoff.reset", "10m",
      PropertyType.TIMEDURATION, """
          The maximum amount of time that the compactor will wait before executing the next
          compaction. When this time limit has been reached, the failures are cleared.""", "2.1.4"),
  COMPACTOR_FAILURE_TERMINATION_THRESHOLD("compactor.failure.termination.threshold", "0",
      PropertyType.COUNT, """
          The number of consecutive failures at which the Compactor exits and the process
          terminates. A value of `0` disables this feature.""", "2.1.4"),
  ...
  COMPACTOR_MINTHREADS_TIMEOUT("compactor.threads.timeout", "0s", PropertyType.TIMEDURATION, """
      The time after which incoming request threads terminate with no work available.
      A value of `0` will keep the threads alive indefinitely.""", "2.1.0"),
  ...
  COMPACTION_COORDINATOR_PREFIX("compaction.coordinator.", null, PropertyType.PREFIX, """
      Properties in this category affect the behavior of the accumulo compaction coordinator
      services in the manager.""", "2.1.0"),
  ...
  COMPACTION_COORDINATOR_RESERVATION_THREADS_META("compaction.coordinator.reservation.threads.meta",
      "1", PropertyType.COUNT,
      "The number of threads used to reserve files for compaction in a tablet for %s tablets."
          .formatted(SystemTables.METADATA.name()),
      "4.0.0"),
  ...
  // create a markdown link to another property
  static String markdownRef(String propKey) {
    boolean isPrefix = propKey.endsWith(".");
    return "[%s](#%s)".formatted(isPrefix ? propKey + "\\*" : propKey,
        (isPrefix ? propKey + "prefix" : propKey).replace('.', '_'));
  }
