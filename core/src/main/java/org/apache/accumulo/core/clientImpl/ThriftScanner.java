...
public class ThriftScanner {
  ...
  public static class ScanState {
    ...
    ScannerBase.ConsistencyLevel getConsistencyLevel() {
      return runOnScanServer ? ScannerBase.ConsistencyLevel.EVENTUAL
          : ScannerBase.ConsistencyLevel.IMMEDIATE;
    }
  }
  ...
  private static ScanAddress getScanServerAddress(ClientContext context, ScanState scanState,
    ...
    if (scanState.scanID != null && scanState.prevLoc != null
      ...
      } else if (loc.hasTserverLocation()) {
  ...
  static ScanAddress getNextScanAddress(ClientContext context, ScanState scanState, long timeOut,
      ...
      AccumuloServerException, InterruptedException {
    ...
    while (addr == null) {
      ...
      if ((currentTime - startTime) / 1000.0 > timeOut) {
        throw new TimedOutException("Failed to find server to process scan before timeout elapsed");
      }
      ...
      try (Scope locateSpan = child1.makeCurrent()) {
        loc = TabletCache.getInstance(context, scanState.tableId, scanState.getConsistencyLevel())
    ...
    try (Scope scope = parent.makeCurrent()) {
      while (results == null && !scanState.finished) {
        ...
        if ((System.currentTimeMillis() - startTime) / 1000.0 > timeOut) {
          throw new TimedOutException("Failed to get next batch of data for scan before timeout");
        }
        ...
        try (Scope scanLocation = child2.makeCurrent()) {
          ...
          TabletCache.getInstance(context, scanState.tableId, scanState.getConsistencyLevel())
