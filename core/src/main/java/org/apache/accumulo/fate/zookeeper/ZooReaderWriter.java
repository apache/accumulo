...
public class ZooReaderWriter extends ZooReader {
  ...
  public byte[] mutate(String zPath, byte[] createValue, List<ACL> acl, Mutator mutator)
    ...
    return retryLoopMutator(zk -> {
      ...
      // Skip incrementing the retry counter and loop if BADVERSION;
      // this preserves the infinite loop behavior in earlier code,
      // which may not be desirable.
      return e.code() == Code.BADVERSION ? null : e;
    });
  }
  ...
  private void putData(String zPath, byte[] data, CreateMode mode, NodeExistsPolicy policy,
    ...
    retryLoopPutData(zk -> {
      ...
      return e.code() == Code.NONODE && policy == NodeExistsPolicy.OVERWRITE ? null : e;
    });
  }
}
