...
public abstract class TestColumnSlice {
    ...
    public void testAllRowsSlice() throws Exception {
        ...
        opts.put(ColumnSliceOpts.OPT_MIN_CF, new String(LONG_LEX.encode(sliceMinCf), UTF_8));
        opts.put(ColumnSliceOpts.OPT_MIN_CQ, new String(LONG_LEX.encode(sliceMinCq), UTF_8));
        opts.put(ColumnSliceOpts.OPT_MAX_CF, new String(LONG_LEX.encode(sliceMaxCf), UTF_8));
        opts.put(ColumnSliceOpts.OPT_MAX_CQ, new String(LONG_LEX.encode(sliceMaxCq), UTF_8));
        BatchScanner bs = newConnector().createBatchScanner(TABLE_NAME, new Authorizations(), 5);
    ...
    public void testSingleColumnSliceByExclude() throws Exception {
        ...
        opts.put(ColumnSliceOpts.OPT_MAX_CF, new String(LONG_LEX.encode(sliceMaxCf), UTF_8));
        opts.put(ColumnSliceOpts.OPT_MAX_CQ, new String(LONG_LEX.encode(sliceMaxCq), UTF_8));
        opts.put(ColumnSliceOpts.OPT_MAX_INCLUSIVE, "false");
        opts.put(ColumnSliceOpts.OPT_MIN_INCLUSIVE, "false");
        BatchScanner bs = newConnector().createBatchScanner(TABLE_NAME, new Authorizations(), 5);
        bs.addScanIterator(new IteratorSetting(50, "ColumnSliceFilter", ColumnSliceFilter.class, opts));
        bs.setRanges(INFINITY);
        loadKvs(foundKvs, bs);
