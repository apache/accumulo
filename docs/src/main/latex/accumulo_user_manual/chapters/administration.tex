...
\begin{verbatim}
$ACCUMULO_HOME/bin/accumulo admin start <hosts> {<host> ...}
\end{verbatim}

Make sure the host in question has the new configuration, else the tablet server won't start.
...
\begin{verbatim}
$ACCUMULO_HOME/bin/accumulo admin stop <hosts> {<host> ...}
\end{verbatim}
...
slaves file automatically, so will report the decomissioned servers as down.
